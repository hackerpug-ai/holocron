...
`packages/platform/deploy/launchd/README.md` for this path (they would double-
...
```sh
SOURCE_REVISION="$(git rev-parse HEAD)"
docker build --file packages/platform/Dockerfile \
...
holo deploy:preflight --target holocron --port 44111 --json
holo deploy:rollback-preflight --lock packages/platform/deploy/compose/image-lock.json
docker compose -f packages/platform/deploy/compose/compose.yaml \
  --env-file packages/platform/deploy/compose/production.env.example config --quiet
tailscale serve status --json
holo deploy:verify --portable --json
```
