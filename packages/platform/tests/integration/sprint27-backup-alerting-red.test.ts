 ...
 *   (`packages/platform/src/backup/alerting.ts` + `holo verify:backup` / alert sweep)
 *
 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/tests/integration/sprint27-backup-alerting-red.test.ts
 *
...
const BACKUP_ALERTING_MODULE = resolve(REPO_ROOT, 'packages/platform/src/backup/alerting.ts');
