 ...
 *     packages/platform/tests/integration/sprint30-security-review.test.ts
 */
...
describe('D07-05 Security review: rollback switch + PONR immutability', () => {
  ...
  it('AC-4 fence coverage blind spot', async () => {
    ...
    const exemptionLines = grepLines(
      ...
      'packages/platform/src/cutover/convex-fence-client.ts'
    );
    ...
    await withCutoverSharedLock(async () => {
      ...
      const authGrep = spawnSync(
        ...
        [
          ...
          'packages/platform/src/cutover/rollback-repoint.ts',
        ],
      ...
      recordFinding({
        ...
        classification:
          'Sole gate is resolveSecretsPathFromEnv() (packages/platform/src/config/secrets.ts:82-92); no credential/approval middleware',
        observed: {
  ...
  it('AC-7 CONVEX_DEPLOY_KEY disarm inventory', async () => {
    ...
    const deployKeyLiteralGrep = grepCount(
      'CONVEX_DEPLOY_KEY',
      'packages/platform/src/config/verify-no-convex-env.ts'
    );
    const verifySrc = readFileSync(
      resolve(REPO_ROOT, 'packages/platform/src/config/verify-no-convex-env.ts'),
      'utf8'
    );
    ...
    const clientSrc = readFileSync(
      resolve(REPO_ROOT, 'packages/platform/src/cutover/convex-fence-client.ts'),
      'utf8'
    );
    ...
    const selfSrc = readFileSync(
      resolve(REPO_ROOT, 'packages/platform/tests/integration/sprint30-security-review.test.ts'),
      'utf8'
    );
