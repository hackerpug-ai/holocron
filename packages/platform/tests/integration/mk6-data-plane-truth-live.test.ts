...
beforeAll(async () => {
  ...
  const imageDigest = `sha256:${(await execFileAsync('shasum', ['-a', '256', resolve(repoRoot, 'packages/platform/src/http/hono-app.ts')])).stdout.split(' ')[0]}`;
  const composeSha = (
    await execFileAsync('shasum', [
      '-a',
      '256',
      resolve(repoRoot, 'packages/platform/deploy/nonprod/mk6-verification.compose.yaml'),
    ])
  ).stdout.split(' ')[0];
  hono = spawn(
    'bun',
    [resolve(repoRoot, 'packages/platform/src/etl/composite-corpus.ts'), '--serve-isolated'],
    {
      cwd: repoRoot,
      env: {
        ...process.env,
