...
const DOCKERFILE_PATH = resolve(REPO_ROOT, 'packages/platform/Dockerfile');
const ROOT_DOCKERIGNORE = resolve(REPO_ROOT, '.dockerignore');
const LOCK_PATH = resolve(REPO_ROOT, 'packages/platform/deploy/compose/image-lock.json');
const CANDIDATE =
...
describe('Sprint 29 D06-06 OCI and Compose contract', () => {
  ...
  it('TC-3: laptop topology parity preserves the exact twelve services and application identity', () => {
    ...
    const laptop = parseYaml(
      readFileSync(resolve(REPO_ROOT, 'packages/platform/deploy/compose/compose.dev.yaml'), 'utf8')
  ...
  it('uses a root-context denylist that excludes operator state before Docker is invoked', () => {
    ...
    expect(ignore).toContain('!packages/platform/src/**');
  ...
  it('requires the committed fleet role manifest at the runtime path in the production image', () => {
    const dockerfile = readFileSync(DOCKERFILE_PATH, 'utf8');
    expect(dockerfile).toContain(
      'COPY --chown=bun:bun packages/platform/fleet/manifest.json ./fleet/manifest.json'
    );
    expect(
  ...
  it('accepts Docker Compose long-form bind mounts for blob storage', () => {
    ...
    const confBind = {
      type: 'bind',
      source: '/Users/holocron/Projects/holocron/packages/platform/deploy/compose/pgbackrest.conf',
    ...
    () => {
      ...
      try {
        ...
        const forbidden = entries.filter(
          (entry) =>
            /(^|\/)\.env($|\.)/.test(entry) ||
            entry.endsWith('/packages/platform/config/secrets.yaml') ||
  ...
  dockerIt(
    ...
    () => {
      ...
      try {
        ...
        expect(
          archiveEntries.some((entry) => entry.endsWith('/packages/platform/config/secrets.yaml'))
        ).toBe(false);
