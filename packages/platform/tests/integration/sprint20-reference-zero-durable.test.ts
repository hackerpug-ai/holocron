 ...
 *
 *   PLATFORM_IT=1 COLDBOOT_IT=1 pnpm vitest run packages/platform/tests/integration/sprint20-reference-zero-durable.test.ts
 */
...
function psqlAgentRows(
  convId: string
): Array<{ id: string; contentLen: number; createdAt: string }> {
  ...
  return out
    ...
    .map((line) => {
      ...
      if (!id || !createdAt) throw new Error(`malformed psql agent row: ${line}`);
...
function psqlAgentRow(convId: string): { id: string; contentLen: number } | null {
  ...
  if (!latest) throw new Error('agent row list became empty unexpectedly');
...
async function postReferenceChatRun(): Promise<string> {
  ...
  const create = await fetch(`${PLATFORM_URL}/api/chat-runs`, {
    ...
    body: JSON.stringify({
      requestId,
      msg: 'REDHAT-FIX-H5 durable zero read probe',
      conversationId: CONV_ID,
    }),
  });
  if (!create.ok)
    throw new Error(`chat-runs create failed: ${create.status} ${await create.text()}`);
  const body = (await create.json()) as { runId?: string };
  if (!body.runId) throw new Error('chat-runs response omitted runId');
  return body.runId;
}

async function waitForNewAgentRow(
  exclude: Set<string>,
  timeoutMs = 90_000
): Promise<{ id: string; contentLen: number }> {
...
function readZeroViaCli(
  ...
  if (!raw) {
    return {
      ...
      agentContentLen: 0,
      agentIds: [],
      error: `no JSON from CLI (exit=${res.status}); stderr=${(res.stderr ?? '').slice(-300)}`,
  ...
  try {
    ...
    return {
      ...
      agentPresent: !!parsed.agentPresent,
      agentContentLen: parsed.agentContentLen ?? 0,
      agentIds,
      error: parsed.error,
...
describe.skipIf(!canRun)('REDHAT-FIX-H5 — durable Zero-synced agent message', () => {
  it('AC-1 [PRIMARY]: the live zero-cache returns the same agent row Postgres has', async () => {
    ...
    expect(
      before.has(pg.id),
      'the awaited agent row must be NEW (not present before the run)'
    ).toBe(false);
    ...
    for (
      let attempt = 1;
      attempt < 8 && !(read.ok && read.agentIds.includes(pg.id));
      attempt += 1
    ) {
    ...
    expect(
      read.ok,
      `zero read did not complete: ${read.error ?? `timedOut=${read.timedOut}`}`
    ).toBe(true);
    expect(
      read.agentPresent,
      'zero-cache returned no agent row after replication-wait retries'
    ).toBe(true);
    // Durability proof: the NEW Postgres agent row id is present among the rows Zero returned.
    expect(
      read.agentIds.includes(pg.id),
      `zero agent ids ${JSON.stringify(read.agentIds)} do not include the new Postgres agent id ${pg.id}`
    ).toBe(true);
  }, 120_000);
  ...
  it('AC-2: with ZERO_CACHE_DISABLED=1 the read fails (negative control)', () => {
    ...
    expect(read.timedOut || !!read.error, 'negative control must record a timeout/error').toBe(
      true
    );
  }, 15_000);
...
describe.skipIf(canRun)('REDHAT-FIX-H5 (skipped: no live substrate)', () => {
  it('skips with reason when PLATFORM_IT=1 COLDBOOT_IT=1 + holocron_nonprod + RN key are unset', () => {
    ...
    console.warn(
      `[REDHAT-FIX-H5] SKIPPED: set ${missing.join(' ')} to drive the real durable read`
    );
