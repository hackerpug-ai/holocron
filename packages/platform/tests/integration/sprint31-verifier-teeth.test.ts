 ...
 *     packages/platform/tests/integration/sprint31-verifier-teeth.test.ts
 */
...
const FIXTURES = resolve(REPO_ROOT, 'packages/platform/tests/fixtures/verifier-teeth');
const EVIDENCE_DIR = resolve(REPO_ROOT, '.tmp/S31-08');
const REAL_EXECUTOR = resolve(REPO_ROOT, 'packages/platform/src/mcp/executor.ts');
const FK_ZERO_FIXTURE = resolve(FIXTURES, 'fk_audit_zero_constraints');
const ETL_VALID_EXPORT = resolve(REPO_ROOT, 'packages/platform/tests/fixtures/etl-valid-export');
...
describe('S31-08 verifier teeth — fail closed on seeded violations', () => {
  ...
  it('verifyNoShellsRefusesShellResidue', () => {
    ...
    expect(existsSync(join(repoRoot, 'packages/platform/src/whatsnew'))).toBe(true);
