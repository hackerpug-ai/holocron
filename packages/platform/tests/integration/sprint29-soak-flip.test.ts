...
const MAPPED_EXPORT_DIR = resolve(REPO_ROOT, 'packages/platform/tests/fixtures/etl-valid-export');
...
const IMMUTABLE_ARTICLE_BASELINE_FIXTURE = resolve(
  REPO_ROOT,
  'packages/platform/tests/fixtures/sprint29/article-baseline-pre-freeze.json'
);
...
describe('Sprint 29 D06-05 soak flip + verify gates', () => {
  ...
  it('REDHAT-FIX-S29-C02: cross-process child without env inject still fences writes', async () => {
    ...
    const childScript = `
      ...
      import { isMigrationReadOnly, MIGRATION_READ_ONLY_BODY } from ${JSON.stringify(resolve(REPO_ROOT, 'packages/platform/src/cutover/soak-fence.ts'))};
      import { runJob } from ${JSON.stringify(resolve(REPO_ROOT, 'packages/platform/src/queue/jobs-runner.ts'))};
      import { MIGRATED_JOBS } from ${JSON.stringify(resolve(REPO_ROOT, 'packages/platform/src/queue/jobs-registry.ts'))};
      import { executePostgresMcpTool } from ${JSON.stringify(resolve(REPO_ROOT, 'packages/platform/src/mcp/executor.ts'))};
  ...
  it('R2-H03/AC-2 static: suite does not author baseline from post-fence SUT fetch', () => {
    const src = readFileSync(
      resolve(REPO_ROOT, 'packages/platform/tests/integration/sprint29-soak-flip.test.ts'),
      'utf8'
    );
