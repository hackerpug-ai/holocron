 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/tests/integration/sprint27-backup-span-export.test.ts
 * Optional configured path:
 *   PLATFORM_IT=1 LANGFUSE_PUBLIC_KEY=... LANGFUSE_SECRET_KEY=... LANGFUSE_BASE_URL=... \
 *     pnpm vitest run packages/platform/tests/integration/sprint27-backup-span-export.test.ts -t "configured"
 */
...
const SPAN_SOURCE = resolve(REPO_ROOT, 'packages/platform/src/backup/span.ts');
...
describe('REDHAT-FIX-S27-13 honest Langfuse backup span export', () => {
  ...
  it('AC-4 honesty: no const exportOk = true; D04-03 admits local-only mode', () => {
    ...
    const rg = spawnSync(
      'rg',
      ['-n', 'const exportOk = true', 'packages/platform/src/backup/span.ts'],
