...
async function importMutatedChatRuns(
  ...
  const sourceRoot = join(root, name, 'packages/platform/src');
  ...
  const actualSourceRoot = join(process.cwd(), 'packages/platform/src');
    ...
    if (!Array.isArray(accountingData.telemetryRowIds)) {
      console.info(
        ...
        JSON.stringify({
          ...
          testFile: 'packages/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts',
          requestId,
          runId: created.runId,
