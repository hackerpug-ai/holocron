 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/tests/integration/redhat-fix-h4-sacrificial-credential-negative.test.ts
 */
...
const FRESH_TARGET_MD = resolve(REPO_ROOT, 'packages/platform/src/backup/fresh-target.md');
...
const BACKUP_CONFIG = resolve(REPO_ROOT, 'packages/platform/src/backup/config.ts');
