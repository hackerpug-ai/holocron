...
const SOURCE_BACKUP = {
  restore: resolve(REPO_ROOT, 'packages/platform/src/backup/restore.ts'),
  r2: resolve(REPO_ROOT, 'packages/platform/src/backup/r2-provision.ts'),
  baseline: resolve(REPO_ROOT, 'packages/platform/src/backup/recovery-baseline.ts'),
};
...
describe('GATE-FIX-S28R3-QA25 CRITICAL3 successful disposable production-boundary', () => {
  it('reaches real fire-drill FD launcher + credentialed child; races; scans artifacts', async () => {
    ...
    const secretsPath = '/Users/inference1/Projects/holocron/packages/platform/config/secrets.yaml';
          ...
          phases: [
            {
              ...
              command:
                'pnpm exec vitest run packages/platform/tests/integration/sprint28-*.test.ts',
              exit_code: 0,
...
`,
  ...
  it('validator rejects product-code paths after bound git_sha (allowlist)', () => {
    const record = resolve(EVIDENCE, 'full-suite-live-sequence.json');
    expect(existsSync(record), 'durable sequence record required for mutation').toBe(true);
    // Pre-product QA25 base: range to HEAD includes packages/platform/src/** and product scripts.
