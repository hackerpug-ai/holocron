...
export function makeHarness(repoRoot: string, evidenceDir: string): HarnessPaths {
  ...
  const fixtureCurl = resolve(repoRoot, 'packages/platform/tests/integration/fixtures/bin/curl');
...
export function baseHarnessEnv(repoRoot: string, extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  const fixBin = resolve(repoRoot, 'packages/platform/tests/integration/fixtures/bin');
  const mockMode =
