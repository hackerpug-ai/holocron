 ...
 *     packages/platform/tests/integration/sprint31-registry-execute.test.ts
 */
...
describe('S31-05 registry execute + composition root', () => {
  ...
  itLive(
    ...
    async () => {
      ...
      const indexSrc = readFileSync(resolve(REPO_ROOT, 'packages/platform/src/index.ts'), 'utf8');
  ...
  itLive('AC-6 deferred execute residue is gone', async () => {
    const registryPath = resolve(REPO_ROOT, 'packages/platform/src/tools/registry.ts');
    ...
    const proc = spawnSync(
      'bun',
      ['packages/platform/src/cli/holo.ts', 'mcp:verify-rehost', '--json'],
      {
        cwd: REPO_ROOT,
