 ...
 *     packages/platform/tests/integration/sprint29-rollback-repoint.test.ts
 */
...
// cwd is often packages/platform under bun test and would double packages/platform/.
...
function holo(
  ...
  const r = spawnSync('bun', ['packages/platform/src/cli/holo.ts', ...args], {
    cwd: REPO_ROOT,
    encoding: 'utf8',
    timeout: 120_000,
    env,
  ...
  it('R2-C04 / R3-H03 live-ack: pre-existing serving /health observes data_plane convex', async () => {
    ...
    const rg = spawnSync(
      ...
      [
        ...
        'packages/platform/src/http/health.ts',
        'packages/platform/src/cutover/soak-fence.ts',
      ],
