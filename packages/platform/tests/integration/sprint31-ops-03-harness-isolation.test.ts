 ...
 *       (mtime unchanged). NEVER touches real packages/platform/config/pgbackrest/pgbackrest.conf.
 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm test:integration -- packages/platform/tests/integration/sprint31-ops-03-harness-isolation.test.ts
 */
 ...
 * NEVER the real operator file at packages/platform/config/pgbackrest/pgbackrest.conf.
 */
...
describe('S31-OPS-03 harness isolation (always — pure path guards)', () => {
  ...
  it('classifies production pgbackrest conf and secrets paths (including synthetic suffix)', () => {
    ...
    expect(
      isProductionPgbackrestConfPath(
        '/Users/inference1/Projects/holocron/packages/platform/config/pgbackrest/pgbackrest.conf',
        REPO_ROOT
      )
    ).toBe(true);
    ...
    expect(
      isEphemeralHarnessPath(
        resolve(REPO_ROOT, 'packages/platform/deploy/nonprod/pgbackrest.conf'),
        REPO_ROOT
      )
    ).toBe(true);
  });
...
describe('S31-OPS-03 AC-2 harnessSecretsPathEphemeral', () => {
  itLive('harness secrets stay ephemeral — production path refused', () => {
    ...
    const nonprodSecrets = resolve(
      REPO_ROOT,
      'packages/platform/deploy/nonprod/secrets.harness.example.yaml'
    );
...
describe('S31-OPS-03 AC-3 backupTestsUseIsolatedRoots', () => {
  itLive(
    ...
    () => {
      const integrationDir = resolve(REPO_ROOT, 'packages/platform/tests/integration');
      ...
      const nonprodTemplate = resolve(
        REPO_ROOT,
        'packages/platform/deploy/nonprod/pgbackrest.conf.example'
      );
  ...
  it('repo root resolution is stable for path guards', () => {
    ...
    expect(existsSync(resolve(root, 'packages/platform'))).toBe(true);
