 ...
 *     packages/platform/tests/integration/observability-otel-v4.test.ts
 */
...
const DEPLOY_OTEL_DIR = resolve(REPO_ROOT, 'packages/platform/deploy/otel');
...
function scanLegacyPaths(): {
  ...
  const srcRoot = resolve(REPO_ROOT, 'packages/platform/src');
...
describe('OBS-02 adopt Mastra/Langfuse OTLP v4', { sequential: true }, () => {
  ...
  it('AC-1: real mission/model call yields identical local Postgres and Langfuse v2 identities', async () => {
    ...
    expect(
      ...
      'packages/platform/deploy/otel/otel-collector-config.yaml must exist'
    ).toBe(true);
    expect(
      existsSync(resolve(DEPLOY_OTEL_DIR, 'compose.yaml')),
      'packages/platform/deploy/otel/compose.yaml must exist'
    ).toBe(true);
