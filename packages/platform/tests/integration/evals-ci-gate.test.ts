 ...
 *     pnpm vitest run packages/platform/tests/integration/evals-ci-gate.test.ts
 */
...
describe('obs-4 evals CI gate (threshold + deterministic invariants)', () => {
  ...
  itLive('AC-3 / TC-3: deterministic invariant blocks independently of judge score', async () => {
    ...
    const fixturePath = resolve(
      REPO_ROOT,
      'packages/platform/evals/fixtures/deterministic-invariant-regression.jsonl'
    );
  ...
  it('AC-1 / TC-1-unit: finalizeJudgeScore never floors sub-threshold scores', () => {
    ...
    const src = readFileSync(resolve(REPO_ROOT, 'packages/platform/src/evals/scorers.ts'), 'utf8');
