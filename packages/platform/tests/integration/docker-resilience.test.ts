...
describe('Docker disk resilience contracts', () => {
  ...
  it('keeps the Docker daemon build cache and default logs bounded', () => {
    const policy = JSON.parse(
      readRepoFile('packages/platform/deploy/docker/daemon-resilience.json')
  ...
  it('installs a five-minute guard that only sweeps labeled ephemeral volumes', () => {
    const plistPath = resolve(
      ROOT,
      'packages/platform/deploy/launchd/holocron-docker-disk-guard.plist'
    );
    ...
    const lifecycle = readRepoFile(
      'packages/platform/tests/integration/helpers/docker-lifecycle.ts'
    );
  ...
  it('cleans QA25 and D06 namespaces explicitly after every test run', () => {
    const qa25 = readRepoFile(
      'packages/platform/tests/integration/sprint28-s28r3-qa25-gate-fix.test.ts'
    );
...
describe('D08-08 cold-host bootstrap and managed lifecycle', () => {
  it('IMP-AC-16 documented cold-host bootstrap (ARM64, four services, no client assets, 44111)', () => {
    const composeReadme = readRepoFile('packages/platform/deploy/compose/README.md');
    const launchdReadme = readRepoFile('packages/platform/deploy/launchd/README.md');
    const composeYaml = parse(readRepoFile('packages/platform/deploy/compose/compose.yaml')) as {
      services?: Record<string, ComposeService>;
      volumes?: Record<string, { name?: string }>;
    };
    const dockerfile = readRepoFile('packages/platform/Dockerfile');
  ...
  it('IMP-AC-17 managed lifecycle: stop/restart, Serve restore, sentinels, zero volume deletions', () => {
    ...
    try {
      ...
      const compose = parse(readRepoFile('packages/platform/deploy/compose/compose.yaml')) as {
        services?: Record<string, ComposeService>;
      };
      for (const name of REQUIRED_SERVICES) {
      ...
      const evidence = {
        ...
        rollback_preflight: {
          ...
          lock: 'packages/platform/deploy/compose/image-lock.json',
