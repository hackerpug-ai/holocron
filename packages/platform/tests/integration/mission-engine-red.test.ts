 ...
 *   pnpm vitest run packages/platform/tests/integration/mission-engine-red.test.ts
 */
...
describe.sequential('Sprint 15 mission-5 RED suite — mission engine missing surfaces', () => {
  ...
  describe.sequential('Sprint 23 gate-3: fulcrum authorable-check seams', () => {
    const CLAIMS_FIXTURE = resolve(
      REPO_ROOT,
      'packages/platform/tests/fixtures/research/claims.json'
    );
    const EVIDENCE_GATE_PATH = resolve(
      REPO_ROOT,
      'packages/platform/src/research/evidence-gate.ts'
    );
