 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/tests/integration/sprint31-mission-trace-live.test.ts
 */
...
const RUNTIME_TS = resolve(REPO_ROOT, 'packages/platform/src/mission/runtime.ts');
...
describe('S31-07 AC-1 liveMissionEmitsTraceToLocalLangfuse', () => {
  ...
  itLive('liveMissionEmitsTraceToLocalLangfuse', async () => {
    ...
    const claimsFixture = resolve(
      REPO_ROOT,
      'packages/platform/tests/fixtures/research/claims-4.json'
    );
