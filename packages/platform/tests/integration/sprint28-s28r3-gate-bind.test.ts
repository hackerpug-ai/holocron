 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/tests/integration/sprint28-s28r3-gate-bind.test.ts
 *   pnpm vitest run packages/platform/tests/integration/sprint28-s28r3-gate-bind.test.ts
 */
...
describe('REDHAT-FIX-S28R3 gate-bind contracts (always)', () => {
  ...
  it('AC-6: credential inventory evidence records R2_RESTORE absence (lengths only)', () => {
    ...
    const secrets = resolve(
      REPO_ROOT,
      'packages/platform/tests/fixtures/sprint28/secrets-inventory-absent-restore.yaml'
    );
