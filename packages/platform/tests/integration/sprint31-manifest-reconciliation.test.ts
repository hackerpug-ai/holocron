 ...
 *     packages/platform/tests/integration/sprint31-manifest-reconciliation.test.ts
 */
...
const MANIFEST_LOADER_PATH = resolve(REPO_ROOT, 'packages/platform/src/mcp/manifest-loader.ts');
const GATEWAY_PATH = resolve(REPO_ROOT, 'packages/platform/src/mcp/gateway.ts');
...
describe('S31-MCP-04 manifest reconciliation', () => {
  ...
  it('AC-4 rate limiting is recorded as not_applicable with its scope citation', () => {
    ...
    const changedPaths = [
      resolve(REPO_ROOT, 'packages/platform/src/mcp/verify-manifest.ts'),
      resolve(REPO_ROOT, 'packages/platform/src/mcp/manifest-loader.ts'),
      MANIFEST_PATH,
    ];
