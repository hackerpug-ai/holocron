 ...
 *
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/tests/integration/redhat-fix-4-subscriptions-no-claims.test.ts
 */
...
describe.sequential('REDHAT-FIX-4 — bare standing subscriptions without --claims (H-2)', () => {
  ...
  itLive(
    ...
    async () => {
      ...
      const cmd =
        "bun run packages/platform/src/cli/holo.ts mission run subscriptions --topic 'AI agents' --json";
      const cli = runHolo(
