 ...
 *     bun test packages/platform/src/http/middleware/__tests__/scoped-key.test.ts
 */
...
describe('CLI manifest:resolve (integration via subprocess)', () => {
  function holoPath(): string {
    return join(resolveRepoRoot(), 'packages/platform/src/cli/holo.ts');
  }
...
/** Walk up from this file to the worktree/repo root that contains packages/platform. */
function resolveRepoRoot(): string {
  let dir = import.meta.dir;
  for (let i = 0; i < 16; i++) {
    if (existsSync(join(dir, 'packages/platform/src/cli/holo.ts'))) {
      return dir;
    }
