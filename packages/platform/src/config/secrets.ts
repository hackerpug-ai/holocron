 ...
 *   2. packages/platform/config/secrets.yaml (gitignored local file)
 *
...
/** Walk up from this module to the repo root that contains packages/platform. */
...
export function defaultSecretsPath(repoRoot = resolveRepoRoot()): string {
  return resolve(repoRoot, 'packages/platform/config/secrets.yaml');
}
...
export function resolveSecretsPathFromEnv(
  ...
  if (env.HOLO_HARNESS === '1') {
    ...
    const isProd = abs === prod || abs.endsWith('/packages/platform/config/secrets.yaml');
    const isEphemeral =
      abs === tmpRoot ||
...
export function defaultSecretsExamplePath(repoRoot = resolveRepoRoot()): string {
  return resolve(repoRoot, 'packages/platform/config/secrets.example.yaml');
}
...
export function secretsConfigDir(repoRoot = resolveRepoRoot()): string {
  return resolve(repoRoot, 'packages/platform/config');
}
