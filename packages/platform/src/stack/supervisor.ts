...
function ensureLangfuseUp(_cfg: StackConfig): { ok: boolean; detail: string } {
  ...
  return {
    ok: false,
    detail:
      'langfuse is unhealthy; start the canonical production compose (packages/platform/deploy/compose/compose.yaml) — standalone Langfuse lifecycle is retired',
  };
}
...
function startDirectMastra(cfg: StackConfig): {
  ...
  const holoCli = resolve(cfg.holoRoot, 'packages/platform/src/cli/holo.ts');
