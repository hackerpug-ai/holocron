 ...
 *
 * Operator:
 *   bun packages/platform/src/cli/holo.ts cutover:go-no-go [--json]
 *   bun packages/platform/src/cli/holo.ts cutover:go-no-go --json --output .tmp/D06-02/go-no-go-report.json
 */
...
export const DEFAULT_GATE_SPECS: readonly GateSpec[] = [
  ...
  {
    // Platform project scope — root tsconfig excludes packages/platform/**.
    ...
    name: 'typecheck',
    command: 'pnpm tsgo --noEmit -p packages/platform/tsconfig.json',
    argv: [
      'sh',
      '-c',
      [
        'status=0',
        'pnpm tsgo --noEmit -p packages/platform/tsconfig.json || status=$?',
        'FIXTURE="packages/platform/src/cutover/.tmp-gate-fixture.ts"',
        'if [ -f "$FIXTURE" ]; then',
  ...
  },
  {
    ...
    command: 'bun packages/platform/src/cli/holo.ts verify:no-convex-client',
    argv: ['bun', 'packages/platform/src/cli/holo.ts', 'verify:no-convex-client'],
    kind: 'plain',
  },
  {
    name: 'no-convex-env',
    command: 'bun packages/platform/src/cli/holo.ts verify-no-convex-env',
    argv: ['bun', 'packages/platform/src/cli/holo.ts', 'verify-no-convex-env'],
    kind: 'plain',
  },
] as const;
...
function createIsolatedIntegrationEnv(
  ...
  try {
    ...
    for (const key of [
      ...
      ISOLATED_DATABASE_OWNER_URL_ENV,
      ISOLATED_CONVEX_DEPLOYMENT_ENV,
      ISOLATED_CONVEX_URL_ENV,
      ISOLATED_CONVEX_SITE_URL_ENV,
      ISOLATED_FLEET_URL_ENV,
      ISOLATED_R2_PREFIX_ENV,
      ISOLATED_PG1_PATH_ENV,
