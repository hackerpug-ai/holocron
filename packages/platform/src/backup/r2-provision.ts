...
export async function resolveRuntimeScopedCredentials(options: {
  ...
  const envKey =
    ...
    getSecretValue('R2_SCOPED_ACCESS_KEY_ID', {
  ...
  const existingSession =
    getSecretValue('R2_SESSION_TOKEN', {
      secretsPath: options.secretsPath,
      env,
    })?.trim() || null;
...
export function renderPgbackrestConfig(cfg: {
  stanza: string;
  pg1Path: string;
  /** Postgres listener used for stanza validation/backup (default 5432). */
  pg1Port?: number;
  bucketName: string;
  endpointHost: string;
  repoPath: string;
  ...
  // Conf lives under packages/platform/config/pgbackrest/ (gitignored, 0600).
  ...
  const pg1Port =
    Number.isInteger(cfg.pg1Port) && (cfg.pg1Port ?? 0) > 0 ? (cfg.pg1Port ?? 5432) : 5432;
...
export function ensureArchiveCommandForCheck(options: {
  ...
  const showMode = run(psql, ['-d', 'holocron', '-tAc', 'SHOW archive_mode'], {
    env,
  });
  ...
  const modeAfter = run(psql, ['-d', 'holocron', '-tAc', 'SHOW archive_mode'], {
    env,
  });
...
function runTrustedPythonR2(
  ...
  // Resolve provider relative to repo root (this file lives under packages/platform/src/backup).
