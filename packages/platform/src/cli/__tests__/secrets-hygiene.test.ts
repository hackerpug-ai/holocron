 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/src/cli/__tests__/secrets-hygiene.test.ts
 */
...
describe('AC-2: consolidated secrets + Convex env alias detection (real CLI + real repo)', () => {
  ...
  itLive('consolidated secrets source is gitignored (secrets.yaml never committed)', () => {
    const configDir = resolve(REPO_ROOT, 'packages/platform/config');
    ...
    expect(existsSync(gitignore), 'packages/platform/config/.gitignore must exist').toBe(true);
    ...
    const check = runCmd('git', ['check-ignore', '-v', 'packages/platform/config/secrets.yaml'], {
      cwd: REPO_ROOT,
    });
    expect(
      check.status,
    ...
    if (existsSync(secrets)) {
      const tracked = runCmd(
        'git',
        ['ls-files', '--error-unmatch', 'packages/platform/config/secrets.yaml'],
        { cwd: REPO_ROOT }
      );
