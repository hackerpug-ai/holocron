 ...
 * Invoke: bun packages/platform/src/cli/holo.ts <args>
 */
import { spawnSync } from 'node:child_process';
import { homedir } from 'node:os';
import { resolve } from 'node:path';

/** Walk up from this file to the repo root that contains packages/platform. */
export function resolveRepoRoot(): string {
  // fixtures → __tests__ → cli → src → platform → packages → repo
  const parts = import.meta.dirname.split('/');
  const idx = parts.lastIndexOf('packages');
  if (idx > 0) return parts.slice(0, idx).join('/');
  const legacy = parts.lastIndexOf('services');
  if (legacy > 0) return parts.slice(0, legacy).join('/');
  return process.cwd();
}

export const REPO_ROOT = resolveRepoRoot();
export const HOLO_CLI = resolve(REPO_ROOT, 'packages/platform/src/cli/holo.ts');
