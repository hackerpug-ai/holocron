 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run packages/platform/src/cli/__tests__/launchd-and-embed-health.test.ts
 */
...
describe('AC-4: launchd units + fleet embed-route health (real launchd + real CLI)', () => {
  ...
  itLive('scheduler plist wires and enables the real worker (not /usr/bin/true)', () => {
    ...
    const template = resolve(
      ...
      'packages/platform/deploy/launchd/holocron-scheduler.plist'
    );
