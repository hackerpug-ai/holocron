...
Usage:
  bun packages/platform/src/cli/holo.ts <command> [options]
...
async function main(): Promise<void> {
  ...
  switch (args.command) {
    ...
    case 'mcp:verify-manifest': {
      ...
      const fixturesDir =
        args.fixturesDir ?? resolve(process.cwd(), 'packages/platform/tests/fixtures/mcp-manifest');
    ...
    case 'mcp:manifest-replay': {
      ...
      const manifest = loadManifest(args.manifestPath);
      const fixturesDir = resolve(process.cwd(), 'packages/platform/tests/fixtures/mcp-manifest');
    ...
    case 'verify:no-shells': {
      ...
      // repo root: packages/platform/src/cli/holo.ts → ../../../../
     ...
     * (packages/platform/src/research/progress.ts) — never advance-server.py.
     *
