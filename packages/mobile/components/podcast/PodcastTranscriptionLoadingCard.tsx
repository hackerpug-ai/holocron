 ...
 * Polls platform when configured; uses platform HTTP only so chat can
  ...
  return (
    <View className="flex-row p-4 rounded-xl my-2 bg-surface-variant">
      ...
      <View className="flex-1">
        <Text className="text-lg font-semibold text-on-surface-variant">Podcast Transcription</Text>
        <Text className="text-sm text-on-surface-variant opacity-70 mt-1 mb-2">{url}</Text>
        <View className="flex-row items-center">
          {status !== 'completed' && status !== 'failed' && <ActivityIndicator size="small" />}
          <Text className="text-xs text-on-surface-variant ml-2">{getStatusMessage()}</Text>
        </View>
      </View>
    </View>
  );
