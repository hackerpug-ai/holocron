...
export const HeadingRenderer = React.memo(({ node, children, testID }: HeadingProps) => {
  ...
  return (
    <Text
      variant={variant}
      testID={testID}
      style={[styles.heading, { marginTop: getMarginTop(), marginBottom: spacing.md }]}
    >
      {children}
    </Text>
  );
...
export const ParagraphRenderer = React.memo(({ children, testID }: ParagraphProps) => {
  const { spacing } = useTheme();
  const styles = useStyles();

  return (
    <Text variant="p" testID={testID} style={[styles.paragraph, { marginBottom: spacing.lg }]}>
      {children}
    </Text>
  );
});
...
export const ListItemRenderer = React.memo(
  ({ node: _node, index, parentOrdered, children, testID }: ListItemProps) => {
    ...
    return (
      <View testID={testID} style={[styles.listItem, { marginBottom: spacing.sm }]}>
        <Text variant="p" style={[styles.bullet, { color: colors.foreground }]}>
          {bulletText}
        </Text>
...
export const BlockquoteRenderer = React.memo(({ children, testID }: BlockquoteProps) => {
  const { spacing, colors } = useTheme();
...
export const ThematicBreakRenderer = React.memo(({ testID }: ThematicBreakProps) => {
  const { spacing, colors } = useTheme();
  const styles = useStyles();
