import type { ParsedFunctionCall, RealtimeEvent, RealtimeEventCallbacks } from './types';
...
export function createEventHandler(
  ...
  return (event: unknown) => {
    ...
    switch (parsed.type) {
      ...
      default:
        logger.debug(
          '[voice/event-handler] Unhandled event type:',
...
function handleOutputItemDone(
  ...
  try {
    ...
    logger.debug('[voice/event-handler] Failed to parse function call arguments:', rawArguments);
    callbacks.onError?.({
