...
type ToolSourceType = 'github' | 'npm' | 'pypi' | 'website' | 'cargo' | 'go' | 'other';
...
function inferSourceType(url: string): ToolSourceType {
  try {
    ...
    if (host.includes('npmjs.com') || host.includes('npmjs.org')) return 'npm';
    if (host.includes('pypi.org') || host.includes('pypi.python.org')) return 'pypi';
    if (host.includes('crates.io')) return 'cargo';
    if (host.includes('pkg.go.dev') || host.includes('golang.org')) return 'go';
    return 'website';
  } catch {
    return 'other';
  }
...
- language: if multi-language or unclear, leave empty`;
  ...
  try {
    ...
    const category: ToolCategory = VALID_CATEGORIES.includes(parsed.category)
...
export const addFromUrl = action({
  ...
  handler: async (ctx, { url, conversationId, loadingMessageId }): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(url, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; Holocron-Toolbelt/1.0)',
      ...
      if (!response.ok) {
        await postError(`Failed to fetch ${url}: HTTP ${response.status} ${response.statusText}`);
        return;
      }
      ...
      const { title: rawTitle, description: rawDescription } = extractHtmlMetadata(html);
      ...
      const description = (rawDescription || `Tool from ${new URL(url).hostname}`).slice(0, 500);

      // 3. Infer source type from URL
      const sourceType = inferSourceType(url);

      // 4. Classify via LLM (category, tags, useCases, language)
      const classification = await classifyTool(title, description, sourceType, url);

      // 5. Insert into toolbelt (reuses duplicate-detection logic)
      const result: { success: boolean; toolId: string; isNew: boolean } = await ctx.runMutation(
