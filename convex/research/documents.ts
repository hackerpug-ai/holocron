...
import type { FindingWithCitations, IterationWithStats } from './documentQueries';
import { type ConfidenceFilter, type FormattedFinding, generateConfidenceReport } from './output';
...
export const createResearchDocument = internalAction({
  ...
  handler: async (
    ctx,
    { sessionId }
  ): Promise<{ status: string; error?: string; reason?: string; documentId?: string }> => {
    // 1. Get session
    ...
    if (findings.length > 0) {
      ...
      stats = {
        ...
        highConfidenceCount: formattedFindings.filter((f) => f.confidenceLevel === 'HIGH').length,
        mediumConfidenceCount: formattedFindings.filter((f) => f.confidenceLevel === 'MEDIUM')
          .length,
        lowConfidenceCount: formattedFindings.filter((f) => f.confidenceLevel === 'LOW').length,
        averageConfidenceScore: Math.round(
          formattedFindings.reduce((s, f) => s + f.confidenceScore, 0) /
            (formattedFindings.length || 1)
        ),
        claimsWithMultipleSources: formattedFindings.filter((f) => f.sourceCount >= 3).length,
      };

      // Generate report
      const confidenceFilter = (session.outputConfidenceFilter || 'ALL') as ConfidenceFilter;
      content = generateConfidenceReport(session.topic, stats, formattedFindings, confidenceFilter);
    } else {
      ...
      if (iterations.length === 0) {
        ...
        if (allStats.length > 0) {
          stats = {
            totalClaims: allStats.reduce(
              (s: number, st: ConfidenceStatsType) => s + st.totalClaims,
              0
            ),
            ...
            averageConfidenceScore: Math.round(
              allStats.reduce(
                (s: number, st: ConfidenceStatsType) => s + st.averageConfidenceScore,
                0
              ) / allStats.length
            ),
    ...
    const result: { documentId: string; embeddingDimensions: number; embeddingStatus: string } =
      await ctx.runAction(api.documents.storage.createWithEmbedding, {
        title: session.topic,
        content,
        category: 'research',
    ...
    const findingMatches = content.match(
      /### \d+\.\s+(?:🟢|🟡|🔴)[\s\S]*?\*\*Sources\*\*[\s\S]*?\n\n/g
    );
    if (findingMatches) {
    ...
    const coverageScore =
      avgConfidence >= 75 ? 4 : avgConfidence >= 50 ? 3 : avgConfidence > 0 ? 2 : 1;
      ...
      cardData: {
        ...
        key_findings:
          keyFindings.length > 0
            ? keyFindings
            : [`Research completed with ${stats.totalClaims} findings`],
      },
    });
