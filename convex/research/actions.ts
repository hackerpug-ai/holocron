...
const processDeepResearchIteration = makeFunctionReference<'action', { sessionId: string }, null>(
  'research/scheduled:processDeepResearchIteration'
);

import { claudeFlash, claudePro } from '../lib/ai/anthropic_provider';
import { JinaError, jinaReader, jinaReaderBatch, jinaSearch } from '../lib/jina';
import {
...
import { generateFindingEmbedding, generateIterationEmbedding } from './embeddings';
...
import { executeParallelSearchWithRetry, executeParallelUrlRead } from './search';
import {
  DEFAULT_CRITERIA,
  type LoopMetrics,
...
export const executeApprovedResearchPlan = action({
  ...
  handler: async (
    ...
    try {
      const result = await ctx.runAction(internal.research.dispatcher.executePlanBasedResearch, {
        conversationId,
        sessionId,
        plan: plan.content,
        topic,
...
export async function runIterativeResearch(
  ...
  finalConfidenceStats?: ConfidenceStats;
}> {
  console.log(
  ...
  const checkTimeout = () => {
    ...
    if (elapsed >= ACTION_TIMEOUT_MS) {
      ...
      throw new Error(
        `RESEARCH_TIMEOUT: Research exceeded 10 minute limit (ran for ${elapsedMinutes} minutes)`
      );
    }
  };
  ...
  try {
    ...
    const totalCostUsd = 0; // Track cumulative cost
    const cumulativeConfidenceStats: ConfidenceStats = {
    ...
    while (true) {
      ...
      try {
        ...
        console.warn(
          `[runIterativeResearch] Step 2b.1: Failed to parse structured synthesis, using narrative only`
        );
      ...
      for (const finding of structuredFindings) {
        ...
        const caveats =
          confidenceLevel === 'MEDIUM' ? generateCaveats(factors, citationIds.length) : [];
        const warnings =
          confidenceLevel === 'LOW' ? generateWarnings(factors, citationIds.length) : [];
        ...
        await ctx.runMutation(api.research.mutations.createResearchFinding, {
          sessionId,
          iterationId,
      ...
      if (iterationFindingsForStats.length > 0) {
        await ctx.runMutation(api.research.mutations.updateIterationConfidenceStats, {
          iterationId,
      ...
      cumulativeConfidenceStats.claimsWithMultipleSources +=
        iterationStats.claimsWithMultipleSources;
      ...
      if (cumulativeConfidenceStats.totalClaims > 0) {
        ...
        cumulativeConfidenceStats.averageConfidenceScore = Math.round(
          (cumulativeConfidenceStats.averageConfidenceScore *
            (cumulativeConfidenceStats.totalClaims - iterationStats.totalClaims) +
      ...
      console.log(`[runIterativeResearch] Step 2c: Calling LLM (glm-4.7) for coverage review`);
      ...
      try {
        ...
        console.error(`[runIterativeResearch] Step 2c: Failed to parse review JSON:`, error);
        console.error(`[runIterativeResearch] Step 2c: Raw review text:`, reviewResult.text);
        review = {
          coverageScore: 3,
      ...
      if ((needsMoreCoverage || needsMoreConfidence) && iteration < maxIterations) {
        if (review.gaps.length > 0) {
          ...
          const keyFindingTexts = structuredFindings
            .slice(0, 10)
            .map((f: StructuredFinding) => f.claimText);
          try {
            ...
            if (parsed.refinedQuery && typeof parsed.refinedQuery === 'string') {
              ...
              console.log(`[runIterativeResearch] Step 2f: Fallback topic: "${currentTopic}"`);
            }
          } catch (err) {
            // Reasoning call failed — fall back to heuristic
            currentTopic = `${topic} - Focus on: ${review.gaps.slice(0, 2).join(', ')}`;
            console.warn(
              '[runIterativeResearch] Step 2f: Gap reasoning failed, using heuristic:',
              err
            );
          }
...
export function analyzeResearchStrategy(topic: string): ResearchStrategy {
  ...
  const isComparison =
    words.includes('vs') || words.includes('compare') || words.includes('versus');
  if (isComparison) {
    return 'parallel_fan_out';
  }
  ...
  const isDeepRequest =
    words.includes('deep') ||
    words.includes('comprehensive') ||
    words.includes('thorough') ||
    words.includes('detailed');
  if (isDeepRequest) {
    return 'single_pass';
  }
...
export const startSimpleResearch = action({
  ...
  handler: async (
    ...
    try {
      ...
      sessionId = await ctx.runMutation(api.research.mutations.createDeepResearchSession, {
...
- Be specific and actionable`;
      ...
      await ctx.runMutation(api.research.mutations.updateDeepResearchSession, {
        ...
        currentCoverageScore:
          synthesis.confidence === 'HIGH' ? 4 : synthesis.confidence === 'MEDIUM' ? 3 : 2,
      });
...
export async function executeSinglePassResearch(
  ...
  const searchPromises = variants.map((variant) =>
    executeParallelSearchWithRetry(variant.query, {}, [], {
      maxRetries: 2,
      timeoutMs: 12000,
  ...
  const searchSnippets = searchResults.map((r) => r.findings).join('\n\n---\n\n');

  // Step 9: Single synthesis with full content
  const synthesisPrompt = buildSinglePassSynthesisPrompt(topic, urlContents, searchSnippets, mode);
  ...
  if (
    report.toLowerCase().includes('confidence: high') ||
    report.toLowerCase().includes('**high**')
  ) {
    confidence = 'HIGH';
  } else if (
    report.toLowerCase().includes('confidence: low') ||
    report.toLowerCase().includes('**low**')
  ) {
    confidence = 'LOW';
  }
...
export const startSmartResearch = action({
  ...
  handler: async (
    ...
    if (strategy === 'parallel_fan_out') {
      ...
      const result = await runIterativeResearch(
        ctx,
        sessionId,
        effectiveConversationId,
        topic,
...
async function synthesize(
  ...
  try {
    const { text } = await generateText({
      ...
      // @ts-expect-error - signal is supported by generateText but not in the type definition
      signal: options.signal,
    });
 ...
 */
export async function findRecommendationsCore(
  args: FindRecommendationsArgs
): Promise<FindRecommendationsResult> {
  ...
  try {
    ...
    const parsed = synth
      ? RecommendationSynthesisSchema.safeParse(synth)
      : { success: false as const };
    console.log(
      `[findRec] parse result: ${parsed.success ? `ok, ${(parsed as any).data?.items?.length ?? 0} items` : 'failed'}`
    );
