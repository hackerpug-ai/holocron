...
import { exaSearchTool, jinaReaderTool, jinaSearchTool } from '../research/tools';
import {
  buildAnalysisPrompt,
...
import { DEFAULT_DIMENSION_SEQUENCE, INITIAL_DIMENSION_SCORES } from './validators';
...
export const processIteration = internalAction({
  ...
  handler: async (ctx, { sessionId }) => {
    try {
      // ── Load session ───────────────────────────────────────────────────────
      const session = await ctx.runQuery(internal.assimilate.queries.getSessionInternal, {
        sessionId,
      });
      ...
      // Guard: cost limit
      if ((session.estimatedCostUsd ?? 0) >= terminationCriteria.maxCostUsd) {
      ...
      if (currentIteration === 0) {
        ...
        try {
          ...
          console.error(`[processIteration] Failed to parse planning JSON:`, parseErr);
        ...
        await ctx.runMutation(internal.assimilate.mutations.updateSessionProgress, {
          sessionId,
          planContent,
          planSummary,
          coveragePlan,
      ...
      const dimension: AssimilationDimension =
        ...
        DEFAULT_DIMENSION_SEQUENCE[(currentIteration - 1) % DEFAULT_DIMENSION_SEQUENCE.length];
      ...
      const dimensionScores: DimensionScores = session.dimensionScores ?? {
      ...
      if (coveragePlan && typeof coveragePlan === 'object') {
        ...
        for (const filePath of keyFiles) {
          ...
          try {
            ...
            console.warn(`[processIteration] Failed to fetch key file ${filePath}:`, readErr);
          }
        }
      }
      ...
      try {
        ...
        console.error(`[processIteration] Failed to parse analysis JSON:`, parseErr);
      ...
      try {
        ...
        console.error(`[processIteration] Failed to parse evaluation JSON:`, parseErr);
      }

      // Estimate iteration cost (~$0.002 per iteration as rough approximation)
      const iterationCostUsd = 0.002;
      const newEstimatedCost = (session.estimatedCostUsd ?? 0) + iterationCostUsd;
      ...
      const saturatedDims = updatedFailureConstraints.filter((c) =>
      ...
      if (termination.continue && termination.trigger !== 'needs_synthesis') {
        ...
        console.log(`[processIteration] Proceeding to synthesis (trigger: ${termination.trigger})`);
        await ctx.scheduler.runAfter(0, internal.assimilate.scheduled.synthesizeAndSave, {
          sessionId,
        });
      }
    } catch (error) {
...
export const synthesizeAndSave = internalAction({
  ...
  handler: async (ctx, { sessionId }) => {
    try {
      ...
      // Load all iterations
      const iterations = await ctx.runQuery(internal.assimilate.queries.listIterations, {
        sessionId,
      });
      ...
      const allFindings = iterations
        .filter(
          (iter: { iterationType: string; findings?: string }) =>
            iter.iterationType !== 'plan' && iter.findings
        )
      ...
      await ctx.runMutation(internal.notifications.internal.create, {
        ...
        body: session.repositoryName
          ? `Assimilation of "${session.repositoryName}" has finished.`
          : 'Your assimilation session has finished.',
...
export const timeoutStuckSessions = internalAction({
  args: {},
  handler: async (ctx) => {
    const activeStatuses = ['planning', 'in_progress', 'synthesizing', 'pending_approval'];
    ...
    for (const session of allActiveSessions) {
      ...
      const maxDurationMs = session.terminationCriteria?.maxDurationMs ?? 15 * 60 * 1000;
    ...
    if (timedOutCount > 0) {
      console.log(`[timeoutStuckSessions] Timed out ${timedOutCount} session(s)`);
    }
  },
});
