...
import { formatTable } from '../lib/reportFormat';
...
function topRisk(evidence: Evidence[]): string {
  const sorted = [...evidence].sort((a, b) => {
    const statusRank = (e: Evidence) =>
      e.challengeStatus === 'refuted' ? 2 : e.challengeStatus === 'contested' ? 1 : 0;
...
function formatSummaryBlock(
  ...
  const payback = base?.paybackMonths !== undefined ? `${base.paybackMonths}` : '—';
...
function formatDvfTable(session: Session, evidence: Evidence[]): string {
  const rows: string[][] = [
    ...
    ['Viability', scoreLabel(session.viabilityScore), topClaimForDimension(evidence, 'viability')],
    [
      'Feasibility',
  ...
  return formatTable(['Dimension', 'Score', 'Key Evidence'], rows, 40);
}
...
function formatCompetitorsTable(competitors: Competitor[]): string {
  ...
  const rows = competitors.map((c) => [c.name, c.pricing ?? '—', c.differentiator ?? '—']);
...
function formatRisksSection(evidence: Evidence[]): string {
  const risks = evidence.filter(
    (e) => e.tier >= 3 || e.challengeStatus === 'contested' || e.challengeStatus === 'refuted'
  );
...
function formatEvidenceTable(evidence: Evidence[]): string {
  ...
  const rows = evidence.map((e) => [e.claim, `T${e.tier}`, e.sourceTitle ?? e.sourceUrl ?? '—']);
