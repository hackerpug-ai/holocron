...
async function executeSearchKnowledgeBase(
  ...
  ctx.scheduler.runAfter(0, internal.chat.toolActions.searchKnowledgeBaseAsync, {
    conversationId,
    query,
    limit,
  });
...
async function executeBrowseCategory(
  ...
  try {
    if (!category) {
      const categoryCounts: any = await ctx.runQuery(api.documents.queries.countByCategory, {});
...
async function executeKnowledgeBaseStats(ctx: ActionCtx): Promise<AgentResponse> {
  try {
    const categoryCounts: any = await ctx.runQuery(api.documents.queries.countByCategory, {});
...
async function executeSubscribe(ctx: ActionCtx, args: Record<string, any>): Promise<AgentResponse> {
...
async function executeListSubscriptions(
  ...
  try {
    const subscriptions: any[] = await ctx.runQuery(api.subscriptions.queries.list, {
...
async function executeWhatsNew(ctx: ActionCtx, args: Record<string, any>): Promise<AgentResponse> {
...
async function executeToolbeltSearch(
  ...
  try {
    const results: any[] = await ctx.runQuery(api.toolbelt.queries.fullTextSearch, {
      query,
      limit: 5,
    });
...
async function executeSaveDocument(
  ...
  await ctx.scheduler.runAfter(0, internal.chat.toolActions.saveDocumentAsync, {
    conversationId,
    title,
    content,
    category,
...
async function executeUpdateDocument(
  ...
  await ctx.scheduler.runAfter(0, internal.chat.toolActions.updateDocumentAsync, {
    conversationId,
    documentId,
    title,
    content,
...
async function executeAssimilate(
  ...
  try {
    const result: any = await ctx.runMutation(api.assimilate.mutations.startAssimilation, {
      repositoryUrl,
      profile,
      conversationId,
    });
...
export async function executeAgentTool(
  ...
  switch (toolName) {
    ...
    case 'find_recommendations': {
      ...
      const content = `Found ${result.items.length} recommendation${result.items.length === 1 ? '' : 's'} for "${query}":\n${itemSummary}`;
...
async function executeAddImprovement(
  ...
  const items: Array<{ description: string; sourceScreen?: string }> = args.items ?? [];
...
async function executeSearchImprovements(
  ...
  try {
    const results: any[] = await ctx.runAction(api.improvements.search.findSimilar, {
      description: query,
      limit,
    });
...
async function executeGetImprovement(
  ...
  try {
    ...
    if (result.agentDecision) {
      lines.push(
        `Agent decision: ${result.agentDecision.action} (confidence: ${(result.agentDecision.confidence * 100).toFixed(0)}%)`
      );
