...
import { REFINEMENT_LEXICON, startsWithRefinementPhrase } from './refinement';
...
describe('startsWithRefinementPhrase', () => {
  describe('lexicon coverage — each phrase triggers true', () => {
    ...
    it("returns true for 'refine...'", () => {
      expect(startsWithRefinementPhrase('refine the results to show only certified coaches')).toBe(
        true
      );
    });
    ...
    it("returns true for 'broaden...'", () => {
      expect(startsWithRefinementPhrase('broaden the search to include adjacent fields')).toBe(
        true
      );
    });
