...
# GATE-FIX-S28R3-QA26: values with embedded double-quotes (JSON policy) MUST be
# shell-quoted so `source restore-target.env` preserves them. Unquoted
# R2_CREDENTIAL_POLICY={"..."} drops JSON quotes under bash source and breaks
# isolation/prove-r2 canonical policy checks.
umask 077
_shell_quote() {
  /usr/bin/python3 -E -s -c 'import shlex,sys; print(shlex.quote(sys.argv[1]))' "$1"
}
{
  ...
  echo "# GATE-FIX-S28R3-QA26: shell-quoted values so JSON policy survives bash source."
  echo "MINI_HOST=$(_shell_quote "$MINI_HOST")"
  echo "MINI_PG_PORT=5432"
  echo "MINI_SSH_PORT=22"
  echo "MINI_IPV4=$(_shell_quote "${MINI_IPV4}")"
  echo "MINI_IPV6=$(_shell_quote "${MINI_IPV6}")"
  echo "MINI_TAILNET_IP=$(_shell_quote "${MINI_TAILNET_IP}")"
  echo "MINI_LAN_IP=$(_shell_quote "${MINI_LAN_IP}")"
  echo "MINI_DNS_ALIASES=$(_shell_quote "${MINI_DNS_ALIASES}")"
  echo "MINI_HOSTNAMES=$(_shell_quote "${MINI_HOSTNAMES}")"
  echo "TARGET_ATTESTED_IDENTITY=$(_shell_quote "$TARGET_ATTESTED_IDENTITY")"
  echo "MINI_ATTESTED_IDENTITY=$(_shell_quote "$MINI_ATTESTED_IDENTITY")"
  echo "REQUIRE_ATTESTED_IDENTITY=1"
  echo "R2_BUCKET_NAME=$(_shell_quote "$R2_BUCKET_NAME")"
  echo "R2_ENDPOINT=$(_shell_quote "$R2_ENDPOINT")"
  echo "R2_ACCESS_KEY_ID=$(_shell_quote "$R2_ACCESS_KEY_ID")"
  echo "R2_SECRET_ACCESS_KEY=$(_shell_quote "$R2_SECRET_ACCESS_KEY")"
  echo "R2_CREDENTIAL_KIND=$(_shell_quote "$R2_CREDENTIAL_KIND")"
  echo "R2_CREDENTIAL_POLICY=$(_shell_quote "$R2_CREDENTIAL_POLICY")"
  echo "PGBACKREST_PG1_PATH=$(_shell_quote "$CONTAINER_PGDATA")"
  echo "HOLOCRON_BLOB_RESTORE_PATH=$(_shell_quote "$CONTAINER_BLOB")"
  echo "PGDATA=$(_shell_quote "$CONTAINER_PGDATA")"
  if [[ -n "$R2_SESSION_TOKEN" ]]; then
    echo "R2_SESSION_TOKEN=$(_shell_quote "$R2_SESSION_TOKEN")"
  fi
  if [[ -n "$R2_ACCOUNT_ID" ]]; then
    echo "R2_ACCOUNT_ID=$(_shell_quote "$R2_ACCOUNT_ID")"
  fi
  # Propagate socket-isolation fixtures when the operator set them (local host PG sockets).
  if [[ -n "${MINI_SOCKET_DEFAULTS:-}" ]]; then
    echo "MINI_SOCKET_DEFAULTS=$(_shell_quote "$MINI_SOCKET_DEFAULTS")"
  fi
  if [[ -n "${MINI_UNIX_SOCKETS:-}" ]]; then
    echo "MINI_UNIX_SOCKETS=$(_shell_quote "$MINI_UNIX_SOCKETS")"
  fi
  if [[ -n "${R2_RESTORE_OBJECT_PREFIX:-}" ]]; then
    echo "R2_RESTORE_OBJECT_PREFIX=$(_shell_quote "$R2_RESTORE_OBJECT_PREFIX")"
  fi
  if [[ -n "${R2_PGBACKREST_PREFIX:-}" ]]; then
    echo "R2_PGBACKREST_PREFIX=$(_shell_quote "$R2_PGBACKREST_PREFIX")"
  fi
} >"$ENV_FILE"
chmod 600 "$ENV_FILE"
...
/bin/cat >"${RESTORE_CONF_DIR}/pgbackrest.conf" <<EOF
...
if [[ "$CONTAINER_STARTED" -eq 1 && "$SKIP_ISOLATION" -eq 0 ]]; then
  ...
  /bin/chmod 600 "$_envf"
  {
    # Docker --env-file: values are literal (not bash-sourced); JSON quotes OK unquoted.
    printf 'MINI_HOST=%s\n' "$MINI_HOST"
    printf 'R2_ACCESS_KEY_ID=%s\n' "$R2_ACCESS_KEY_ID"
    printf 'R2_SECRET_ACCESS_KEY=%s\n' "$R2_SECRET_ACCESS_KEY"
    printf 'R2_CREDENTIAL_KIND=%s\n' "$R2_CREDENTIAL_KIND"
    printf 'R2_CREDENTIAL_POLICY=%s\n' "$R2_CREDENTIAL_POLICY"
    printf 'R2_BUCKET_NAME=%s\n' "$R2_BUCKET_NAME"
    printf 'R2_ENDPOINT=%s\n' "$R2_ENDPOINT"
    # GATE-FIX-S28R3-QA26: alpine busybox tool paths + static R2 (host already ran live RO).
    printf 'GREP_BIN=/bin/grep\n'
    printf 'ENV_BIN=/usr/bin/env\n'
    printf 'TR_BIN=/usr/bin/tr\n'
    printf 'MKTEMP_BIN=/bin/mktemp\n'
    printf 'MINI_SOCKET_DEFAULTS=0\n'
    printf 'MINI_UNIX_SOCKETS=/tmp/.s.PGSQL.5432-container-absent\n'
    printf 'HOLO_ISOLATION_ALLOW_STATIC_R2=1\n'
    printf 'TARGET_ATTESTED_IDENTITY=%s\n' "$TARGET_ATTESTED_IDENTITY"
    printf 'MINI_ATTESTED_IDENTITY=%s\n' "$MINI_ATTESTED_IDENTITY"
    printf 'REQUIRE_ATTESTED_IDENTITY=1\n'
  } >"$_envf"
  "$DOCKER_BIN" exec --env-file "$_envf" \
    ...
    /usr/bin/env -u R2_PARENT_ACCESS_KEY_ID -u R2_PARENT_SECRET_ACCESS_KEY \
      ...
      /bin/bash /usr/local/bin/prove-isolation.sh --mini-host "$MINI_HOST"
  in_rc=$?
  /bin/rm -f "$_envf" 2>/dev/null || true
  set -e
  if [[ $in_rc -ne 0 ]]; then
