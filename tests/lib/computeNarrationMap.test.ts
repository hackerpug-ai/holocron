...
function imageOnlyNode() {
  return {
    type: 'paragraph',
    children: [{ type: 'image', url: 'https://example.com/img.png', alt: 'photo' }],
  };
}
...
describe('computeNarrationMap', () => {
  ...
  it('skips image-only paragraphs that produce no text', () => {
    const ast = makeAst([textNode('Before image.'), imageOnlyNode(), textNode('After image.')]);
    const backendParagraphs = [
  ...
  it('handles heading prefix stripping', () => {
    const ast = makeAst([headingNode('Introduction'), textNode('Content here.')]);
