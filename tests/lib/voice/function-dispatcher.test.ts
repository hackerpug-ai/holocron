...
import {
  ...
  getSpokenErrorMessage,
} from '@/lib/voice/function-dispatcher';
...
describe('server-side tool dispatch', () => {
  it('calls executeTool action with toolName, toolArgs, and conversationId', async () => {
    const mockResult = {
      success: true,
      content: JSON.stringify([{ _id: 'doc1' }]),
  ...
  it('routes create_note through server-side executeTool', async () => {
    const mockResult = {
      success: true,
      content: JSON.stringify({ id: 'new-doc-id' }),
      skipContinuation: false,
    };
    const deps = makeDeps({
      convex: {
...
describe('getSpokenErrorMessage', () => {
  ...
  it('returns permanent message with context', () => {
    expect(getSpokenErrorMessage('permanent', 'find that document')).toBe(
