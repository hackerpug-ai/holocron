...
describe('Summary Generation - AC-1: Each finding has summary field', () => {
  ...
  it('should generate summary between 80-150 characters', async () => {
    ...
    const finding = {
      ...
      content:
        'A revolutionary new AI framework that simplifies building agents with advanced capabilities.',
    };

    // Mock LLM response with exactly 100 characters
    const mockSummary =
      'This is a revolutionary new AI framework that simplifies building agents with advanced capabilities for developers.';
...
describe('Summary Generation - AC-2: Summary captures key insight', () => {
  ...
  it('should capture key technical insight accurately', async () => {
    ...
    const finding = {
      ...
      content:
        "Anthropic's Claude Code source code was accidentally leaked via .map file. The community is dissecting the internal safety architecture, prompt injection defenses, and agent loop design patterns.",
    };

    // Mock LLM response that captures the key insight
    const mockSummary =
      "Community dissecting Anthropic's internal safety architecture and agent loop design from leaked source code.";
    (generateText as any).mockResolvedValue({
      text: mockSummary,
    });
...
describe('Summary Generation - AC-4: Truncate long summaries', () => {
  ...
  it('should truncate summaries longer than 150 characters with ellipsis', async () => {
    ...
    const longSummary =
      'This is an extremely long summary that exceeds the maximum allowed length of one hundred fifty characters and therefore should be truncated with an ellipsis at the end to indicate it was cut off.';
    (generateText as any).mockResolvedValue({
      text: longSummary,
    });
...
describe('Summary Generation - AC-5: Video source handling', () => {
  ...
  it('should generate summary for video sources', async () => {
    ...
    const finding = {
      ...
      content:
        'This video covers the basics of building agent workflows with LangGraph, including state management, node design, and edge routing patterns.',
    };

    // Mock LLM response for video content
    const mockSummary =
      'Covers LangGraph agent workflow basics including state management, node design patterns, and edge routing for building production AI systems.';
    (generateText as any).mockResolvedValue({
      text: mockSummary,
    });
...
describe('Summary Generation - Edge Cases', () => {
  ...
  it('should handle findings with no content gracefully', async () => {
    ...
    const mockSummary =
      'A GitHub repository with relevant tools for AI engineers working on production systems.';
    (generateText as any).mockResolvedValue({
      text: mockSummary,
    });
