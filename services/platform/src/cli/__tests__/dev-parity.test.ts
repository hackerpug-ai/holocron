 ...
 * Config contract is real files under services/platform/config — not hardcoded mini paths.
 *
 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run services/platform/src/cli/__tests__/dev-parity.test.ts
 */
...
const CONFIG_DIR = resolve(REPO_ROOT, 'services/platform/config');
...
describe('AC-3: laptop dev-parity under portable config contract', () => {
  itLive(
    ...
    () => {
      expect(
        ...
        'services/platform/config must exist (consolidated config contract from D01-03/D01-04)'
      ).toBe(true);

      const { paths, blob } = collectConfigTexts();
      expect(
        paths.length,
        'portable config files must exist under services/platform/config (example + host profiles)'
      ).toBeGreaterThan(0);
  ...
  itLive(
    ...
    () => {
      ...
      const upLaptop = runCmd(
        ...
        [resolve(REPO_ROOT, 'services/platform/src/cli/holo.ts'), 'stack', 'up'],
        {
          cwd: REPO_ROOT,
          env: {
            ...process.env,
      ...
      const status = runCmd(
        process.env.BUN_BIN ?? 'bun',
        [resolve(REPO_ROOT, 'services/platform/src/cli/holo.ts'), 'stack', 'status'],
        {
          cwd: REPO_ROOT,
          env: {
            ...process.env,
  ...
  itLive('runtime parity: Darwin + bun available AND stack status works under portable env', () => {
    ...
    // Config contract must exist (pre-impl RED: services/platform/config absent)
    expect(
      existsSync(CONFIG_DIR),
      'services/platform/config required for portable runtime (D01-03/D01-04)'
    ).toBe(true);
