...
export function resolveHoloRoot(repoRoot: string, home: string): string {
  if (
    ...
    existsSync(resolve(process.env.HOLO_ROOT, 'services/platform/src/cli/holo.ts'))
  ) {
    return process.env.HOLO_ROOT;
  }
  const main = resolve(home, 'Projects/holocron');
  if (existsSync(resolve(main, 'services/platform/src/cli/holo.ts'))) {
    return main;
  }
  return repoRoot;
}
...
export function loadStackConfig(options?: {
  ...
  return {
    ...
    templateDir: resolve(repoRoot, 'services/platform/deploy/launchd'),
  };
}
