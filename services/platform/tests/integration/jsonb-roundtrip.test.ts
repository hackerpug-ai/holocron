...
describe('AC-3 jsonb round-trip integration (real Postgres)', () => {
  it('GREEN: card_data jsonb round-trip preserves nested structure', async () => {
  ...
  it('NEGATIVE: when card_data is not jsonb, probe fails closed (type casting / missing jsonb)', async () => {
  ...
  it('NEGATIVE: structural inequality is detected (round-trip must not lose nested fields)', async () => {
    ...
    try {
      ...
      if (!conversationId) {
        throw new Error('JSONB round-trip setup did not create a conversation');
      }
      ...
      const message = msg[0];
      if (!message) {
        throw new Error('JSONB round-trip setup did not create a message');
      }
      let read: unknown = message.card_data;
      ...
      await sql`DELETE FROM chat_messages WHERE id = ${message.id}`;
