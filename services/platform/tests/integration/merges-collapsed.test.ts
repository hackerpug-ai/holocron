...
describe('AC-4 merges collapsed integration (real Postgres)', () => {
  it(
    'GREEN: exactly 3 analysis_* + 3 research_* trios; discriminators present; no shells',
    async () => {
  ...
  it(
    'NEGATIVE: per-domain shell table makes verifyMerges fail (revenue_validation_sessions)',
    async () => {
      // would fail if verify reported ok while a forbidden shell exists
      const sql = createSql(DATABASE_URL);
      const shell = 'revenue_validation_sessions';
      expect(
        FORBIDDEN_SHELL_TABLES.includes(shell as (typeof FORBIDDEN_SHELL_TABLES)[number])
      ).toBe(true);
      let created = false;
      try {
        await sql.unsafe(`
  ...
  it(
    'NEGATIVE: deep_research_sessions shell is forbidden (research must stay merged)',
    async () => {
