 ...
 *     pnpm vitest run services/platform/tests/integration/sprint31-cron-schedule.test.ts
 */
...
describe('S31-02 AC-2: scheduleExpressionsDriveRealCadence', () => {
  ...
  itLive(
    ...
    async () => {
      ...
      const script = `
        import { parseSchedule, ScheduleParseError } from ${JSON.stringify(resolve(REPO_ROOT, 'services/platform/src/queue/schedule-parser.ts'))};
        try {
      ...
      const workerProbe = `
        import { parseSchedule } from ${JSON.stringify(resolve(REPO_ROOT, 'services/platform/src/queue/schedule-parser.ts'))};
