...
describe('AC-2 status CHECK integration (real Postgres)', () => {
  it(
    'GREEN: CHECK rejects in-progress and accepts in_progress; Zod enum aligned',
    async () => {
  ...
  it(
    'NEGATIVE: when tasks_status_check is missing, invalid status is accepted and probe fails',
    async () => {
      ...
      try {
        ...
        await sql`DELETE FROM tasks WHERE id = ${inserted[0]!.id}`;
