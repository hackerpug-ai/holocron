...
describe('AC-1 db:migrate integration (real Postgres)', () => {
  it('GREEN: 0-error migrate against live holocron (≥55 domain tables, 0 missing)', async () => {
  ...
  it('NEGATIVE: dead DATABASE_URL fails closed (Postgres not running / unreachable)', async () => {
  ...
  it('NEGATIVE: invalid SQL is rejected by real Postgres (migration error path has teeth)', async () => {
