 ...
 *
 * Run:
 *   PLATFORM_IT=1 pnpm vitest run services/platform/tests/integration/sprint31-fleet-client-bypass-guard.test.ts
 */
...
const SRC_ROOT = resolve(REPO_ROOT, 'services/platform/src');
...
`,
  ...
  itLive('noObservabilityModuleIsTestOnly — AC-5', () => {
    const graph = scanObservabilityModuleGraph({
      platformRoot: resolve(REPO_ROOT, 'services/platform/src'),
    });
