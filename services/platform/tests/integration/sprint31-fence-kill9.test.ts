 ...
 *     pnpm vitest run services/platform/tests/integration/sprint31-fence-kill9.test.ts
 */
...
describe('S31-03 AC-4: fake crash path cannot return', () => {
  itLive(
    ...
    () => {
      ensureEvidenceDirs();
      const durablePath = resolve(REPO_ROOT, 'services/platform/src/queue/durable-effect.ts');
      const srcTree = resolve(REPO_ROOT, 'services/platform/src');
      ...
      expect(crashAtHits, '0 crashAt under services/platform/src').toBe('');
      ...
      writeFileSync(
        probeFile,
        `
import { beginEffect } from '../../../services/platform/src/queue/durable-effect.ts';
