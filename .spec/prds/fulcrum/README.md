...
| **Role semantics** | Fleet-wide role definitions; a node cannot redefine a role | `fulcrum-assay`, `fulcrum-challenge`, `qwen3-embedding` mean the same thing everywhere |
...
**What changed for Fulcrum ([ADR-007](./09-technical-requirements/00-architecture-decisions.md) / [ADR-008](./09-technical-requirements/00-architecture-decisions.md)):** the coder models Fulcrum was drafted against (`reviewer`, `implementer`) are **out of its vocabulary entirely** — they were always a workaround for what the fleet happened to serve. Fulcrum now reasons about **research + embedding models only**, across two roles whose model binding is a `fleet.json` edit scored by a **deterministic oracle** the gate already produces (quote-check pass rate for extraction, refuting-claim gate-pass rate for challenge). Cross-model challenge survives intact, and no LLM ever grades an LLM.
...
| Functional Groups | 4 |
| Use Cases | 22 |
| Test Criteria | 102 (across 22 UCs) · AC coverage 110/110 verified |
...
| Fleet roles addressed | 3 — `fulcrum-assay`, `fulcrum-challenge` (chat, research-class), `qwen3-embedding` (1024-dim). **Zero coder roles.** |
...
| 3.0.0 | 2026-08-20 | **Fleet alignment.** Added hard dependency on the **Virtual Device Fleet** (config + launcher layers; not the VM tier). **ADR-007 added** — Fulcrum consumes inference as an ordinary fleet client on **loopback**, pinned to `inference1` + `inference2`; `FULCRUM_INFERENCE_BASE_URL` and the dev-laptop/prod-mini split are deleted; the laptop leaves the dependency set. **ADR-008 added** — Fulcrum's vocabulary is **research + embedding only**; the coder roles (`reviewer`/`implementer`/`orchestrator`/`qwen-coder`/`verifier`) are removed entirely; the two chat roles become fleet aliases whose model binding is swappable config scored by a **deterministic oracle** (quote-check pass rate / refuting-claim gate-pass rate). **ADR-005 re-affirmed** (embedder already live on both minis). **LIS group rewritten** (UC-LIS-03 tailnet-worker → swap-and-measure); CYC UC-05 re-specified on resolved model identity. **R1 re-scoped** (unmeasured research binding, now measurable), **R5 downgraded** (always-on minis), **R14–R17 added** (body-field model checks are false evidence; cutover can remove the live embedder; equal-weight reload churn; forbidden role substitution on retry). `06-external-dependencies.md` re-derived against the live fleet. Test criteria 93 → 102 (+9, all LIS); **the summary table's long-standing claim of "80" was drift and is corrected** — counts are now machine-derived, AC coverage 110/110 verified. MAJOR — CONSTITUTION ADRs + scope changed. | Fleet alignment (user directive) |
...
- **Fleet-side prerequisites (F1–F4)** — four `fleet.json` edits Fulcrum requires, recorded in [`09-technical-requirements/06-external-dependencies.md`](./09-technical-requirements/06-external-dependencies.md). **F2 is time-sensitive**: both minis serve `qwen3-embedding` today but no config declares it, so the fleet cutover would silently remove a capability Fulcrum depends on. File these into the fleet roadmap's deferred list.
- `/kb-prd-plan --edit .spec/prds/fulcrum` — re-derive the remaining TR sections (02–05, 08) against the live mk6 platform; they still carry `⚠️ Re-platform pending` banners. Sections 00, 01, 06, 07, 09 are current as of v3.0.0.
- `/review-red-hat .spec/prds/fulcrum` — adversarial review of this re-platformed, fleet-aligned PRD (out-of-band).
- `/kb-sprint-plan .spec/prds/fulcrum` — build the implementation roadmap **only after both `mk6-migration` and the fleet's config + launcher layers are live**. Every sprint's human testing gate draws `[human-gate]` criteria from [10-e2e-testing-criteria.md](./10-e2e-testing-criteria.md).
