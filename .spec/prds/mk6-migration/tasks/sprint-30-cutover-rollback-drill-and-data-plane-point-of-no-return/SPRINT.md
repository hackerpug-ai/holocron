...
### Human-Gate Remediation Cycle (plan only — GATE-FIX after partial 3/5 @ 54299bfc)

**Source:** real human-gate run `20260808T011038Z` against reviewed/deployed HEAD `54299bfc76fec6fc52468dae451ca293a6f104c4` (sourceRevision match; deployment mismatch fixed). Verdict **partial** — 3/5 steps passed; steps **1 and 2 failed**.

| Step | Result | Residual |
|------|--------|----------|
| 1 rollback-drill | **FAIL** | `DRILL_WRITE_SURFACES_NOT_BLOCKED` / `fence_armed=false`; app HTTP **201** + MCP accepted real writes |
| 2 zero-loss ledger | **FAIL** | Postgres `post_export_write_audit` `accepted_count=2`; `drill_lost_accepted_writes=1` |
| 3–5 pin/boot, enable-writes, post-PONR | pass | T-SYNC-014 latch can green while T-SYNC-013 zero-loss does **not** hold |

**Proven root cause:** durable `HOLO_MIGRATION_READ_ONLY: "0"` left by prior enable-writes; gate preflight dual-resets ledger+PONR but **never re-arms** the soak fence (`scripts/rearm-sprint30-cutover-control-plane.sh` exists, unwired); `runRollbackDrill` probes five write surfaces **without** fail-closed fence precondition, minting real accepted production writes and breaking UC-SYNC-04 / T-SYNC-013 zero-loss. Fence middleware **is** wired when armed — the gap is control-plane state + drill/gate precondition, not missing Hono/MCP/job/mission hooks.

| ID | Finding | Required remediation | Severity | Proposed by |
|----|---------|----------------------|----------|-------------|
| GATE-FIX-drill-fence-precondition | Product drill mints writes when fence disarmed | Fail-closed `DRILL_FENCE_NOT_ARMED` **before** `probeFiveWriteSurfaces`; live 423 after durable re-arm; strict `allFiveBlocked`; zero ledger mint under disarmed | CRITICAL | `mastra-planner` |
| GATE-FIX-gate-preflight-fence-rearm | Gate preflight never re-arms soak fence | Default-ON rearm via existing rearm script; live serving 423 oracle; dual-path pre-PONR clear; optional postgres soak plane restore; preserve step5 POST_PONR | HIGH | `devops-engineer` |
| GATE-FIX-zero-loss-t-sync-013 | Zero-loss / post-PONR oracles fakeable | Identity-bound zero-loss (not count-only); fail-closed if step1 mints writes; step5 `POST_PONR_INELIGIBLE` bound to this-run step4 `ponr_id`/`write_row_id` (reject residual aaaa); partial 3/5 is not T-SYNC-013 release | CRITICAL | `security-auditor` |

Task plan files (durable ACs + evidence requirements; **no implementation in this plan commit**):

- `GATE-FIX-drill-fence-precondition.md` — `mastra-planner` → implementer `mastra-implementer` · product drill fence precondition · CRITICAL · 6 AC / 11 TC · `red_first`
- `GATE-FIX-gate-preflight-fence-rearm.md` — `devops-engineer` · gate preflight re-arm + live 423 + pre-PONR soak · HIGH · 8 AC / 12 TC · `red_first`
- `GATE-FIX-zero-loss-t-sync-013.md` — `security-auditor` → implementer `devops-engineer` · T-SYNC-013/014 identity oracles · CRITICAL · 7 AC / 12 TC · `red_first`

**Specialist set (this cycle):** `mastra-planner` (drill product), `devops-engineer` (gate preflight), `security-auditor` (zero-loss/PONR oracles). Dispatched in a single wave. Orchestrator consolidated; it authored no task content beyond SPRINT index + sibling cross-links.

**Quality:** GATE-FIX remediation tasks with GWT ACs, TCs, anti-stub, evidence dirs, REQUIREMENT-CONTRACT v1 blocks, branch discipline. **Fakeability floor:** real HTTP 423 + body code (not CLI-only fence read); identity-bound accepted writes when count>0; step5 this-run `ponr_id`/`write_row_id` bind (not residual aaaa); no count-only/path-only/success-path-only.
