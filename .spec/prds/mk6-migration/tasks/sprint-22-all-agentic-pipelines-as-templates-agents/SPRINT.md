...
- REDHAT-FIX-3 through REDHAT-FIX-5 — pending `/kb-sprint-tasks-plan --only` expansion from `.spec/reviews/red-hat-sprint-22-20260721T183000Z.md`

pipes-1..5 avg quality ~100/115; fakeability audit: **0 CRITICAL / 0 HIGH**. REDHAT-FIX-1/2: `validate_scenario.py` exit 0. Topological order: pipes-4 → pipes-1 ∥ pipes-2 → pipes-3 → pipes-5 → REDHAT-FIX-*. Status remains 🟠 In Progress — red-hat remediation queued.
