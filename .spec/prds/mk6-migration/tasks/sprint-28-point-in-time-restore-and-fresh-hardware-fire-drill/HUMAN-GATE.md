...
`literal_cmd_sha256: b69c0cd67335140ca9cf62487fcb191b4fe15281c5fcf0e6a6daba51aea83102`

```bash
set -euo pipefail; test -f .tmp/REDHAT-FIX-S28R3/parity-report.json; jq -e '.LEDGER_CHECKSUM_MATCH == true and ((.ledger_sha256 // .ledgerDigest // .LEDGER_SHA256 // "") | tostring | test("^[a-f0-9]{64}$"; "i"))' .tmp/REDHAT-FIX-S28R3/parity-report.json
```

_Assertion notes:_ GATE-FIX-QA4: literal_cmd ends with jq -e which prints only the scalar true on success (exit 0). Domain tokens are proven by the frozen jq predicate + cmd_sha fidelity, not by stdout tokens that jq never emits. REDHAT-FIX-S28R3: report path is volume-bound step3 output.

### 5 — Restore blob storage on the fresh target — every object SHA-256 matches (BLOB_PARITY_PASS + matched_objects >= 1).

`literal_cmd_sha256: 935784d0dd45a3c831c1dcd98560447e71430bea384f52ca44fea51b777bdbed`

```bash
set -euo pipefail; test -f .tmp/REDHAT-FIX-S28R3/parity-report.json; jq -e '.BLOB_PARITY_PASS == true and (.matched_objects // .blob_matched_objects // 0) >= 1' .tmp/REDHAT-FIX-S28R3/parity-report.json
```
