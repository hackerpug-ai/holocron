...
> **[mastra-reviewer PASS (behavioral) / caveated]** Failure envelope is REAL: `run-degraded-no-hang.sh:104-137` `prove_fleet_unavailable_envelope` POSTs to the real `/api/chat-runs` and asserts the platform emits `ROLE_UNAVAILABLE` / `Local fleet unavailable`; chat-runs.ts:409-435 finalizes with `errorCode:'ROLE_UNAVAILABLE'`; `SURFACE_UNAVAILABLE_MESSAGE` is byte-equal across `degraded-mode-controller.ts:36` and `use-resumable-sse-stream.ts:157` (em-dash confirmed). CAVEAT: the fleet-down step has NO human-executable verb — `run-degraded-no-hang.sh:212` does `kill -9 $FLEET_PID` + a reaper loop (line 80-91). The SPRINT.md gate step 7 "Stop the local fleet" is not reproducible by a human without this harness. Backend signal: real. Gate ergonomics: fail.
