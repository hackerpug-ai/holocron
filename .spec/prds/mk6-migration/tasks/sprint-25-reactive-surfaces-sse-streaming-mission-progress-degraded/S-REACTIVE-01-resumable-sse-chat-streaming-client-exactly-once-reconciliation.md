...
- **THEN:** the thread shows EXACTLY ONE final assistant message matching the durable `chat_messages` row (`role='agent'`, content byte-equal via Maestro oracle `chat-content-byte-equal` / notVisible `chat-content-byte-mismatch` — REDHAT-FIX-11 PATH-A)
