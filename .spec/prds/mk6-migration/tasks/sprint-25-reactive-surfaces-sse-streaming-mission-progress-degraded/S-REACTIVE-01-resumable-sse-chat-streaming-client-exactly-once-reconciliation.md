...
> **[mastra-reviewer PARTIAL]** Backend SSE contract is REAL (hono-app.ts:256-284 streams `token`/`terminal`/`blocked`/`error` with monotonic `seq` as SSE `id`; `Last-Event-ID`→`afterSeq` at hono-app.ts:258-259; client hook use-resumable-sse-stream.ts:444-543 consumes the REAL event names). BUT the `seeded-streaming-conversation` fixture is FICTIONAL: `.validate-payloads/S-REACTIVE-01.json:4,7` claims a 'Streaming' conversation is seeded; `seed-e2e.ts:500` seeds only Alpha/Beta/Gamma + 'Sprint 20 reference conversation' — NO 'Streaming' row. Flow passes on conversation Alpha (id `...e1`, seed-e2e.ts:28). Behavioral: PASS on Alpha; Contract/fakeability: FAIL (fixture record false; validate_scenario.py passed a fictional fixture).
...
- **THEN:** the thread shows EXACTLY ONE final assistant bubble (`chat-assistant-message-latest` / `chat-assistant-bubble-count-1`). Content coordination with durable `chat_messages.content` is **deferred** (PATH-B / REDHAT-FIX-11): historical claim of content byte-equal is **downgraded** until follow-up `REDHAT-FIX-11-FOLLOWUP-MAESTRO-CONTENT-BYTE-EQUAL` greens Maestro `chat-content-byte-equal` against Zero durable content. Bubble-count==1 remains the verified primary oracle. (Oracle prep already shipped in ChatThread + yml.)
...
- **Scenario:** tier `visible` · test_tier `e2e` · topology `single-node`
  ...
  - **Evidence:** artifact `screenshot`, required_capture=True
  - **Case 1** — start_ref `seeded-streaming-conversation`: actor `user`; steps: send a message, wait for completion → MUST observe exactly `1` final bubble, the Zero query returns `1` row with `role='agent'`, the bubble count stays `1` after the `complete` transition; content equality vs durable row is **deferred** (PATH-B follow-up REDHAT-FIX-11-FOLLOWUP-MAESTRO-CONTENT-BYTE-EQUAL / historical claim downgraded); MUST NOT observe `0` bubbles (empty thread), `>1` final bubbles, or a spinner with no message (`0`)
...
|-------|-----|-------------|----------|----|
| 1 | FIX-S-REACTIVE-01-c1 | AC-1, AC-2, AC-3, AC-4, AC-5, TC-1, TC-2, TC-3, TC-4, TC-5, TC-6, TC-7 | product-manager+react-native-ui-reviewer | 2026-07-25T07:47:31Z |
## Reading List
...
<!--
{
  ...
  "requirements": [
    ...
    {"id":"AC-3","type":"acceptance_criterion","primary":true,"description":"GIVEN a chat reply completes streaming (terminal event received) WHEN the thread renders the final state THEN the thread shows EXACTLY ONE final assistant bubble (bubble-count==1). Content coordination with durable chat_messages is deferred (PATH-B / REDHAT-FIX-11 follow-up REDHAT-FIX-11-FOLLOWUP-MAESTRO-CONTENT-BYTE-EQUAL); historical content equality claim is downgraded until Maestro chat-content-byte-equal is green. Oracle prep shipped.","verify":"maestro test .maestro/reactive/exactly-one-final-message.yml","maps_to_ac":null,"scenario":{"tier":"visible","test_tier":"e2e","verification_service":"Zero+seeded Postgres","topology":"single-node","negative_control":{"would_fail_if":["disconnect — durable row not written to Postgres","empty — Zero query returns no row for the message","stub — thread shows a hardcoded string with no durable row","mock — terminal event faked, no real chat_messages row written"]},"evidence":{"artifact_type":"screenshot","required_capture":true},"cases":[{"start_ref":"seeded-streaming-conversation","action":{"actor":"user","steps":["send a message from the chat composer","wait for the streaming reply to complete (terminal event received)"]},"end_state":{"must_observe":["`1` final assistant message bubble rendered (count exactly `1`)","the Zero query returns `1` row with `role='agent'`","the bubble count stays `1` after the state transitions to `complete`"],"must_not_observe":["`0` assistant message bubbles (empty thread)","more than `1` final assistant bubble for the reply","a spinner with no message (count `0` messages)"]}}]}},
