...
| REDHAT-FIX-H1 | Close the as-of audit-forgery path by preventing app-role insertion of closed historical beliefs while preserving authorized seed/revision writes (fresh red-hat H1: INSERT as-of forgery) | mastra-implementer | 120 min |
| REDHAT-FIX-H2 | Bind the default runtime/CLI database pool to the least-privilege app role and prove owner-pool DML bypass is unavailable on product paths (fresh red-hat H2: DATABASE_URL bypass) | mastra-implementer | 150 min |
| REDHAT-FIX-H3 | Make the documented evidence seed-to-belief human path product-true without gate-only scaffold inserts (fresh red-hat H3: HT-1→HT-2 broken) | mastra-implementer | 120 min |
...
Generated by /kb-sprint-tasks-plan on 2026-07-15T20:14:47Z (proposed by: mastra-planner [ledger-1..5 original; REDHAT-FIX-H1/H2/H3 remediation expansion]).
Avg quality score: ~115/115 (115-point rubric, min 80). Fakeability audit: **0 CRITICAL / 0 HIGH** — `validate_scenario` clean on every behavioral AC across REDHAT-FIX-H1/H2/H3.
Topological order for remediation: REDHAT-FIX-H1 (closed-history INSERT authenticity) ∥ REDHAT-FIX-H2 (app-role pool bind) → REDHAT-FIX-H3 (product seed→belief HT-1→HT-2).
...
- REDHAT-FIX-H1-close-as-of-audit-forgery-prevent-app-role-closed-history-insert.md
- REDHAT-FIX-H2-bind-default-runtime-cli-pool-to-app-role-block-owner-dml-bypass.md
- REDHAT-FIX-H3-seed-to-belief-product-path-without-gate-scaffold.md
