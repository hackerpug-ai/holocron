...
**Success state:** S33-OPS-03 keeps its one-file implementation scope and mounted-secret design, but its prose and embedded contract now prove nonempty FLEET_URL from the container environment, nonempty FLEET_KEY from `/proc/1/environ`, absence of FLEET_KEY in a separate `docker exec` child, and exact healthy status without printing either runtime value.
...
- Assert FLEET_URL through the configured container environment, assert FLEET_KEY through a quiet nonempty record match in `/proc/1/environ`, and assert a separate `docker exec` child has FLEET_KEY absent.
...
- **THEN** AC-1, TC-1, and TC-2 decode to shell-valid fail-closed commands; AC-1 proves the combined process contract and exact healthy state; TC-1 proves `FLEET_URL_PRESENT`, `FLEET_KEY_PID1_PRESENT`, and `FLEET_KEY_CHILD_ABSENT` without printing runtime values; TC-2 proves exact healthy state; the old child-side nonempty FLEET_KEY check is absent; and WRITE-ALLOWED remains exactly one scheduler-block Compose path.
- **Verify:** `target=.spec/prds/mk6-migration/tasks/sprint-33-fleet-routing-and-deployed-service-restoration/S33-OPS-03-wire-the-scheduler-compose-service-to-fleeturl-fleetkey-so-b.md; python3 -c 'import hashlib,json,re,subprocess,sys; t=open(sys.argv[1],encoding="utf-8").read(); end="--"+">"; marker="<!-- REQUIREMENT-CONTRACT v1 "+end; matches=re.findall(re.escape(marker)+r"\s*<!--\s*(\{.*?\})\s*"+re.escape(end),t,re.S); assert len(matches)==1 and t.count(end)==2; c=json.loads(matches[0]); req={r["id"]:r for r in c["requirements"]}; ac=re.search(r"### AC-1.*?\n- \*\*Verify:\*\* `(.*?)`",t,re.S).group(1); table=dict(re.findall(r"^\| (TC-\d+) \|.*?\| AC-\d+ \| `(.*?)` \|$",t,re.M)); assert [r["id"] for r in c["requirements"]]==["AC-1","TC-1","TC-2"]; assert ac==req["AC-1"]["verify"] and table["TC-1"]==req["TC-1"]["verify"] and table["TC-2"]==req["TC-2"]["verify"]; assert hashlib.sha256(ac.encode()).hexdigest()=="effb0b3df8144d71d6f91ff3230788fbf199425b64653abd34a1b5f147d37e80"; assert hashlib.sha256(table["TC-1"].encode()).hexdigest()=="bab58037c003c77d9ef661f9ece32742b76781cb5aca6c1e662996e3a9196aa3"; assert hashlib.sha256(table["TC-2"].encode()).hexdigest()=="fd682546e464e62c03153f6b2cb17fc494235a478d899d8ed3a4c10860f99b09"; [subprocess.run(["bash","-n","-c",req[x]["verify"]],check=True) for x in ("AC-1","TC-1","TC-2")]; allowed=re.search(r"\*\*WRITE-ALLOWED\*\*\s*(.*?)\s*\*\*WRITE-PROHIBITED\*\*",t,re.S).group(1); assert [x.strip() for x in allowed.splitlines() if x.strip()]==["- services/platform/deploy/compose/compose.yaml (MODIFY - scheduler service block only)"]; required=["through `/proc/1/environ`","docker exec` child MUST have FLEET_KEY absent","does not inherit FLEET_KEY","FLEET_URL_PRESENT","FLEET_KEY_PID1_PRESENT","FLEET_KEY_CHILD_ABSENT","grep -zq \"^FLEET_KEY=.\" /proc/1/environ","test -z \"${FLEET_KEY:-}\""]; assert not [x for x in required if x not in t]; assert "[ -n \"$FLEET_KEY\" ]" not in t; assert "printenv" not in req["AC-1"]["verify"] and "echo \"$FLEET_KEY\"" not in req["AC-1"]["verify"]; assert req["AC-1"]["maps_to_ac"] is None and req["TC-1"]["maps_to_ac"]=="AC-1" and req["TC-2"]["maps_to_ac"]=="AC-1"; print("target_contract_ok=1 commands_bash_n=3 target_write_allowed=1")' "$target"`
...
### TC-1 — The repaired target contract is exact, shell-valid, and bounded
...
- **THEN** its commands, hashes, process-boundary predicates, maps, and one-entry implementation WRITE-ALLOWED list match exactly.
- **Verify:** `target=.spec/prds/mk6-migration/tasks/sprint-33-fleet-routing-and-deployed-service-restoration/S33-OPS-03-wire-the-scheduler-compose-service-to-fleeturl-fleetkey-so-b.md; python3 -c 'import hashlib,json,re,subprocess,sys; t=open(sys.argv[1],encoding="utf-8").read(); end="--"+">"; marker="<!-- REQUIREMENT-CONTRACT v1 "+end; matches=re.findall(re.escape(marker)+r"\s*<!--\s*(\{.*?\})\s*"+re.escape(end),t,re.S); assert len(matches)==1 and t.count(end)==2; c=json.loads(matches[0]); req={r["id"]:r for r in c["requirements"]}; ac=re.search(r"### AC-1.*?\n- \*\*Verify:\*\* `(.*?)`",t,re.S).group(1); table=dict(re.findall(r"^\| (TC-\d+) \|.*?\| AC-\d+ \| `(.*?)` \|$",t,re.M)); assert [r["id"] for r in c["requirements"]]==["AC-1","TC-1","TC-2"]; assert ac==req["AC-1"]["verify"] and table["TC-1"]==req["TC-1"]["verify"] and table["TC-2"]==req["TC-2"]["verify"]; assert hashlib.sha256(ac.encode()).hexdigest()=="effb0b3df8144d71d6f91ff3230788fbf199425b64653abd34a1b5f147d37e80"; assert hashlib.sha256(table["TC-1"].encode()).hexdigest()=="bab58037c003c77d9ef661f9ece32742b76781cb5aca6c1e662996e3a9196aa3"; assert hashlib.sha256(table["TC-2"].encode()).hexdigest()=="fd682546e464e62c03153f6b2cb17fc494235a478d899d8ed3a4c10860f99b09"; [subprocess.run(["bash","-n","-c",req[x]["verify"]],check=True) for x in ("AC-1","TC-1","TC-2")]; allowed=re.search(r"\*\*WRITE-ALLOWED\*\*\s*(.*?)\s*\*\*WRITE-PROHIBITED\*\*",t,re.S).group(1); assert [x.strip() for x in allowed.splitlines() if x.strip()]==["- services/platform/deploy/compose/compose.yaml (MODIFY - scheduler service block only)"]; required=["through `/proc/1/environ`","docker exec` child MUST have FLEET_KEY absent","does not inherit FLEET_KEY","FLEET_URL_PRESENT","FLEET_KEY_PID1_PRESENT","FLEET_KEY_CHILD_ABSENT","grep -zq \"^FLEET_KEY=.\" /proc/1/environ","test -z \"${FLEET_KEY:-}\""]; assert not [x for x in required if x not in t]; assert "[ -n \"$FLEET_KEY\" ]" not in t; assert "printenv" not in req["AC-1"]["verify"] and "echo \"$FLEET_KEY\"" not in req["AC-1"]["verify"]; assert req["AC-1"]["maps_to_ac"] is None and req["TC-1"]["maps_to_ac"]=="AC-1" and req["TC-2"]["maps_to_ac"]=="AC-1"; print("target_contract_ok=1 commands_bash_n=3 target_write_allowed=1")' "$target"`
...
1. **Target task contract:** Run the exact AC-1 verifier. Expected: `target_contract_ok=1 commands_bash_n=3 target_write_allowed=1`.
  ...
  "requirements": [
    {
      ...
      "primary": true,
      "description": "GIVEN FLEET_KEY is securely exported only into scheduler PID 1 WHEN the S33-OPS-03 contract is repaired THEN its exact shell-valid commands prove configured FLEET_URL, nonempty PID-1 FLEET_KEY, exec-child FLEET_KEY absence, exact healthy state, no runtime-value output, and the unchanged one-file implementation scope.",
      "verify": "target=.spec/prds/mk6-migration/tasks/sprint-33-fleet-routing-and-deployed-service-restoration/S33-OPS-03-wire-the-scheduler-compose-service-to-fleeturl-fleetkey-so-b.md; python3 -c 'import hashlib,json,re,subprocess,sys; t=open(sys.argv[1],encoding=\"utf-8\").read(); end=\"--\"+\">\"; marker=\"<!-- REQUIREMENT-CONTRACT v1 \"+end; matches=re.findall(re.escape(marker)+r\"\\s*<!--\\s*(\\{.*?\\})\\s*\"+re.escape(end),t,re.S); assert len(matches)==1 and t.count(end)==2; c=json.loads(matches[0]); req={r[\"id\"]:r for r in c[\"requirements\"]}; ac=re.search(r\"### AC-1.*?\\n- \\*\\*Verify:\\*\\* `(.*?)`\",t,re.S).group(1); table=dict(re.findall(r\"^\\| (TC-\\d+) \\|.*?\\| AC-\\d+ \\| `(.*?)` \\|$\",t,re.M)); assert [r[\"id\"] for r in c[\"requirements\"]]==[\"AC-1\",\"TC-1\",\"TC-2\"]; assert ac==req[\"AC-1\"][\"verify\"] and table[\"TC-1\"]==req[\"TC-1\"][\"verify\"] and table[\"TC-2\"]==req[\"TC-2\"][\"verify\"]; assert hashlib.sha256(ac.encode()).hexdigest()==\"effb0b3df8144d71d6f91ff3230788fbf199425b64653abd34a1b5f147d37e80\"; assert hashlib.sha256(table[\"TC-1\"].encode()).hexdigest()==\"bab58037c003c77d9ef661f9ece32742b76781cb5aca6c1e662996e3a9196aa3\"; assert hashlib.sha256(table[\"TC-2\"].encode()).hexdigest()==\"fd682546e464e62c03153f6b2cb17fc494235a478d899d8ed3a4c10860f99b09\"; [subprocess.run([\"bash\",\"-n\",\"-c\",req[x][\"verify\"]],check=True) for x in (\"AC-1\",\"TC-1\",\"TC-2\")]; allowed=re.search(r\"\\*\\*WRITE-ALLOWED\\*\\*\\s*(.*?)\\s*\\*\\*WRITE-PROHIBITED\\*\\*\",t,re.S).group(1); assert [x.strip() for x in allowed.splitlines() if x.strip()]==[\"- services/platform/deploy/compose/compose.yaml (MODIFY - scheduler service block only)\"]; required=[\"through `/proc/1/environ`\",\"docker exec` child MUST have FLEET_KEY absent\",\"does not inherit FLEET_KEY\",\"FLEET_URL_PRESENT\",\"FLEET_KEY_PID1_PRESENT\",\"FLEET_KEY_CHILD_ABSENT\",\"grep -zq \\\"^FLEET_KEY=.\\\" /proc/1/environ\",\"test -z \\\"${FLEET_KEY:-}\\\"\"]; assert not [x for x in required if x not in t]; assert \"[ -n \\\"$FLEET_KEY\\\" ]\" not in t; assert \"printenv\" not in req[\"AC-1\"][\"verify\"] and \"echo \\\"$FLEET_KEY\\\"\" not in req[\"AC-1\"][\"verify\"]; assert req[\"AC-1\"][\"maps_to_ac\"] is None and req[\"TC-1\"][\"maps_to_ac\"]==\"AC-1\" and req[\"TC-2\"][\"maps_to_ac\"]==\"AC-1\"; print(\"target_contract_ok=1 commands_bash_n=3 target_write_allowed=1\")' \"$target\"",
      "maps_to_ac": null,
      "scenario": {
        "id": "AC-1",
        ...
        "negative_control": {
          "would_fail_if": [
            "the old impossible child-side secret check remains",
        ...
        "cases": [
          {
            ...
            "action": {
              "actor": "planner",
              "steps": [
                "replace the impossible exec-child FLEET_KEY assertion with distinct configured-container, PID-1, and exec-child-isolation predicates",
            ...
            "end_state": {
              "must_observe": [
                ...
                "target AC-1 verify SHA-256 == 'effb0b3df8144d71d6f91ff3230788fbf199425b64653abd34a1b5f147d37e80'",
                "target TC-1 verify SHA-256 == 'bab58037c003c77d9ef661f9ece32742b76781cb5aca6c1e662996e3a9196aa3'"
              ],
              "must_not_observe": [
    ...
    },
    {
      "id": "TC-1",
      "type": "test_criterion",
      "description": "The target task's prose and embedded requirements encode the exact PID-1 secret boundary and retain exactly one implementation WRITE-ALLOWED path.",
      "maps_to_ac": "AC-1",
      "verify": "target=.spec/prds/mk6-migration/tasks/sprint-33-fleet-routing-and-deployed-service-restoration/S33-OPS-03-wire-the-scheduler-compose-service-to-fleeturl-fleetkey-so-b.md; python3 -c 'import hashlib,json,re,subprocess,sys; t=open(sys.argv[1],encoding=\"utf-8\").read(); end=\"--\"+\">\"; marker=\"<!-- REQUIREMENT-CONTRACT v1 \"+end; matches=re.findall(re.escape(marker)+r\"\\s*<!--\\s*(\\{.*?\\})\\s*\"+re.escape(end),t,re.S); assert len(matches)==1 and t.count(end)==2; c=json.loads(matches[0]); req={r[\"id\"]:r for r in c[\"requirements\"]}; ac=re.search(r\"### AC-1.*?\\n- \\*\\*Verify:\\*\\* `(.*?)`\",t,re.S).group(1); table=dict(re.findall(r\"^\\| (TC-\\d+) \\|.*?\\| AC-\\d+ \\| `(.*?)` \\|$\",t,re.M)); assert [r[\"id\"] for r in c[\"requirements\"]]==[\"AC-1\",\"TC-1\",\"TC-2\"]; assert ac==req[\"AC-1\"][\"verify\"] and table[\"TC-1\"]==req[\"TC-1\"][\"verify\"] and table[\"TC-2\"]==req[\"TC-2\"][\"verify\"]; assert hashlib.sha256(ac.encode()).hexdigest()==\"effb0b3df8144d71d6f91ff3230788fbf199425b64653abd34a1b5f147d37e80\"; assert hashlib.sha256(table[\"TC-1\"].encode()).hexdigest()==\"bab58037c003c77d9ef661f9ece32742b76781cb5aca6c1e662996e3a9196aa3\"; assert hashlib.sha256(table[\"TC-2\"].encode()).hexdigest()==\"fd682546e464e62c03153f6b2cb17fc494235a478d899d8ed3a4c10860f99b09\"; [subprocess.run([\"bash\",\"-n\",\"-c\",req[x][\"verify\"]],check=True) for x in (\"AC-1\",\"TC-1\",\"TC-2\")]; allowed=re.search(r\"\\*\\*WRITE-ALLOWED\\*\\*\\s*(.*?)\\s*\\*\\*WRITE-PROHIBITED\\*\\*\",t,re.S).group(1); assert [x.strip() for x in allowed.splitlines() if x.strip()]==[\"- services/platform/deploy/compose/compose.yaml (MODIFY - scheduler service block only)\"]; required=[\"through `/proc/1/environ`\",\"docker exec` child MUST have FLEET_KEY absent\",\"does not inherit FLEET_KEY\",\"FLEET_URL_PRESENT\",\"FLEET_KEY_PID1_PRESENT\",\"FLEET_KEY_CHILD_ABSENT\",\"grep -zq \\\"^FLEET_KEY=.\\\" /proc/1/environ\",\"test -z \\\"${FLEET_KEY:-}\\\"\"]; assert not [x for x in required if x not in t]; assert \"[ -n \\\"$FLEET_KEY\\\" ]\" not in t; assert \"printenv\" not in req[\"AC-1\"][\"verify\"] and \"echo \\\"$FLEET_KEY\\\"\" not in req[\"AC-1\"][\"verify\"]; assert req[\"AC-1\"][\"maps_to_ac\"] is None and req[\"TC-1\"][\"maps_to_ac\"]==\"AC-1\" and req[\"TC-2\"][\"maps_to_ac\"]==\"AC-1\"; print(\"target_contract_ok=1 commands_bash_n=3 target_write_allowed=1\")' \"$target\""
    }
  ]
