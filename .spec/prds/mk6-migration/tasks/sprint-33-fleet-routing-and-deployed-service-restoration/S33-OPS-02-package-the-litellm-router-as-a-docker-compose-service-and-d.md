...
- Before creating an evidence directory, making any remote call, or mutating the router, `health-flip` must byte-compare its evidence path against an exact two-entry mode/control allowlist: normal mode accepts only `.tmp/S33-OPS-02/health-flip`; `--negative-control fail-after-stop` accepts only `.tmp/S33-OPS-02/health-flip-negative`. Every other absolute, relative, normalized, traversing, or symlink-alias path must fail closed before mutation.
...
- Both normal and negative-control evidence must preserve raw `health.pre.json`, `health.degraded.json`, `health.restored.json`, `production-containers.pre.json`, `production-containers.post.json`, `remote-primary.pre.json`, and `remote-primary.post.json`. The health files are untouched endpoint bodies; each production-container file is raw Docker inspect JSON for exactly `postgres`, `mastra`, `scheduler`, and `zero-cache`; each remote-primary file is a direct read-only sentinel object with nonempty `head`, `status_sha256`, and `tracked_hash`. Summary booleans never substitute for independently recomputing the five identity, PID, uptime, production-container, and protected-primary invariants from these raw pre/post captures.
...
| cleanup negative control bites and independently restores | `set -o pipefail; neg=.tmp/S33-OPS-02/health-flip-negative; if bash scripts/verify-s33-router-capacity.sh --mode health-flip --holocron-host holocron --remote-compose-file /Users/holocron/Projects/holocron/.kb-run-sprint/worktrees/S33-OPS-02/services/platform/deploy/compose/router.compose.yaml --remote-docker-bin /usr/local/bin/docker --router-url http://holocron.tail011a51.ts.net:4545 --health-url https://holocron.tail011a51.ts.net:44111/health --negative-control fail-after-stop --evidence-dir .tmp/S33-OPS-02/health-flip-negative; then exit 1; fi && jq -e '.ok == false and .mode == "health-flip" and .negative_control == "fail-after-stop" and .intentional_failure_observed == true and .cleanup_restore_armed == true and .cleanup_restore_attempted == true and .restore_succeeded == true and .restored_health.status == "ok" and .restored_health.fleet.ready == true and .restored_health.failing_dependency == null and .deployment_identity_unchanged == true and .deployment_pid_unchanged == true and .deployment_uptime_monotonic == true and .production_service_identities_unchanged == true and .remote_primary_unchanged == true and .final_router.state == "running" and .final_router.health == "healthy"' "$neg/failure.json" >/dev/null && jq -s -e 'def ident: .deployment.identity | {host,runtime,imageDigest,sourceRevision,composeGeneration}; (.[0] | ident) as $pre | (.[1] | ident) as $degraded | (.[2] | ident) as $restored | ($pre | all(.[]; type == "string" and length > 0)) and ($pre == $degraded) and ($pre == $restored) and ((.[0].deployment.identity.pid | type) == "number") and (.[0].deployment.identity.pid > 0) and (.[0].deployment.identity.pid == .[1].deployment.identity.pid) and (.[0].deployment.identity.pid == .[2].deployment.identity.pid) and ((.[0].deployment.identity.uptimeMs | type) == "number") and (.[1].deployment.identity.uptimeMs >= .[0].deployment.identity.uptimeMs) and (.[2].deployment.identity.uptimeMs >= .[1].deployment.identity.uptimeMs)' "$neg/health.pre.json" "$neg/health.degraded.json" "$neg/health.restored.json" >/dev/null && jq -s -e 'def identities: map({service: .Config.Labels["com.docker.compose.service"], id: .Id, image: .Image, started_at: .State.StartedAt}) | sort_by(.service); (.[0] | identities) as $pre | (.[1] | identities) as $post | (($pre | length) == 4) and (($pre | map(.service)) == ["mastra","postgres","scheduler","zero-cache"]) and ($pre | all(.[]; (.id | type) == "string" and (.id | length) > 0 and (.image | type) == "string" and (.image | length) > 0 and (.started_at | type) == "string" and (.started_at | length) > 0)) and ($pre == $post)' "$neg/production-containers.pre.json" "$neg/production-containers.post.json" >/dev/null && jq -s -e '.[0] as $pre | .[1] as $post | (($pre | keys | sort) == ["head","status_sha256","tracked_hash"]) and ($pre | all(.[]; type == "string" and length > 0)) and ($pre == $post)' "$neg/remote-primary.pre.json" "$neg/remote-primary.post.json" >/dev/null && curl --fail --silent --show-error --connect-timeout 5 --max-time 20 https://holocron.tail011a51.ts.net:44111/health | jq -e '.status == "ok" and .fleet.ready == true and .failing_dependency == null' >/dev/null && ssh -o BatchMode=yes -o ConnectTimeout=10 -o ServerAliveInterval=5 -o ServerAliveCountMax=2 holocron '/usr/local/bin/docker inspect "$(/usr/local/bin/docker compose -f /Users/holocron/Projects/holocron/.kb-run-sprint/worktrees/S33-OPS-02/services/platform/deploy/compose/router.compose.yaml ps -q litellm-router)"' | jq -e 'length == 1 and .[0].State.Status == "running" and .[0].State.Health.Status == "healthy"' >/dev/null` | Intentional verifier exit is nonzero; failure.json asserts cleanup plus all five invariants; independent raw three-health/pre-post recomputation proves Mastra identity/PID/uptime, all four production identities, and the remote-primary sentinel; explicitly bounded fresh HTTP and absolute-Docker SSH prove final restoration |
          ...
          },
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "persist untouched health.pre.json plus raw production-containers.pre.json for exactly postgres, mastra, scheduler, and zero-cache and raw remote-primary.pre.json containing nonempty head/status_sha256/tracked_hash; require pre-stop status=ok, fleet.ready=true, failing_dependency=null, and executable /usr/local/bin/docker through non-interactive ssh holocron",
                ...
                "poll and persist untouched health.degraded.json until status=degraded, fleet.ready=false, and failing_dependency=fleet are observed",
                "restore litellm-router through the armed cleanup path, poll until the router is running and healthy, and persist untouched health.restored.json, raw production-containers.post.json, and raw remote-primary.post.json before emitting ok=true",
                "in the focused integration test, rerun health-flip with --negative-control fail-after-stop and exact .tmp/S33-OPS-02/health-flip-negative; assert intentional nonzero and every failure.json predicate, independently recompute all five invariants from the raw named pre/post artifacts, then re-read /health with bounded curl and inspect the router with bounded SSH plus /usr/local/bin/docker before continuing"
              ]
            },
            "end_state": {
              "must_observe": [
                ...
                "the fail-after-stop invocation exits nonzero after writing failure.json with intentional_failure_observed==true, cleanup_restore_attempted==true, restore_succeeded==true, all five unchanged/monotonic booleans true, and final router state=='running'/health=='healthy'",
                "the focused integration test independently recomputes the Mastra five-field deployment tuple, PID, monotonic uptimeMs, exactly four production Docker identities, and protected-primary head/status_sha256/tracked_hash equality from raw pre/post files; bounded fresh HTTP and SSH oracles then confirm restored ok/true/null and router running/healthy"
              ],
              "must_not_observe": [
                ...
                "the test trusts failure.json booleans without recomputing any of the five invariants from raw named artifacts",
