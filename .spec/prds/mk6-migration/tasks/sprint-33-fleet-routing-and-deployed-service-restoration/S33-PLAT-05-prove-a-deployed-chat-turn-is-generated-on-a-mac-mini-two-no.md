...
- **AND** every `negative_controls[]` item for direct argument/environment exposure and indirect parameter/name/length, command-substitution, `printenv`, runtime `getenv`, or artifact retrieval records `actual_process_spawned:true`, integer `actual_pid > 1`, observer equality `observed_pid === actual_pid`, exact `observed_surface === expected_surface`, `observed_canary_matches >= 1`, nonzero exit, `CREDENTIAL_CANARY_EXPOSURE_DETECTED`, `public_request_issued:false`, and `external_side_effect_performed:false`; private control artifacts are removed
- **AND** aggregate success requires exactly six expected control detections and six fail-closed exits, recomputed from actual observations, while `unexpected_exposure_count === 0`; zeroing a required detection or relabeling a PID/surface fails, and no hand-authored counter is authoritative
...
| TC-14 | The positive verifier path has zero canary matches outside actual stdin/config lifecycle, while six distinct bounded real leak-control subprocesses each bind actual/observed PID and expected/observed surface, observe at least one intentional match, fail nonzero with `CREDENTIAL_CANARY_EXPOSURE_DETECTED`, issue no public request, and contribute only to expected-detection aggregates. | AC-11 | `PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'` |
...
- six `negative_controls[]` actual subprocesses exercise direct argv/environment plus indirect parameter/name/length, command-substitution, `printenv`, runtime `getenv`, and artifact retrieval; each binds its actual PID and expected surface to observer output with at least one canary match, then fails closed without a public request or external side effect
- expected control detections are counted separately from `unexpected_exposure_count`; actual observation is recomputed and hand-authored counters cannot pass
...
- Static source and command scanning never substitutes for the runtime canary gate. The positive path uses an internally generated synthetic value, actual verifier/curl processes, platform argv/environment observation, a real temporary file, and a real public Hono HTTP request; its zero counters exclude six separately identified negative controls. Every control's real PID, exact surface, positive match count, nonzero fail-closed exit, no-request fact, and cleanup are recomputed from observation; zeroed detections or relabeled PID/surface fail.
...
- RUNTIME CREDENTIAL CANARY: `positive_path` carries a synthetic in-memory value through the actual verifier, private stdin, mode-0600 config, curl child, filesystem, and public Hono boundary with zero matches on every forbidden surface and verified cleanup. Separate `negative_controls[]` each bind a bounded real subprocess's actual PID to observer PID, expected surface to observed surface, at least one intentional match, nonzero `CREDENTIAL_CANARY_EXPOSURE_DETECTED`, no public request/external side effect, and cleanup. Aggregate success counts expected detections separately from zero unexpected exposure; hand-authored counters, missing matches, PID/surface relabeling, static-only, mocked, stubbed, recorded, skipped-observer, or live-secret-bearing evidence is invalid.
...
<!--
{
  ...
  "fixtures": {
    ...
    "credential_canary_transport": {
      ...
      "records": [
        ...
        "six negative_controls actual subprocesses exercise direct argv/environment and indirect parameter, constructed-name, command-substitution, printenv, runtime-getenv, and artifact retrieval; each binds actual PID to observer PID and expected surface to observed surface, sees at least one canary match, then fails CREDENTIAL_CANARY_EXPOSURE_DETECTED before any public request or external side effect",
        "negative_controls_expected_detection_count=6 and negative_controls_failed_closed_count=6 are recomputed from actual observations while unexpected_exposure_count=0; zeroed detections, relabeled PID/surface, or hand-authored counters reject",
  ...
  "requirements": [
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN a unique synthetic canary generated only in integration-test memory with live credential values unloaded WHEN the actual positive verifier receives it over private stdin, uses an actual curl child plus a real quoted mode-0600 config and public Hono request, and the platform observer inspects owned-process argv/environment THEN positive_path has zero matches on every forbidden surface and verified cleanup, while six separate negative_controls each bind actual/observed PID and expected/observed surface, observe at least one intentional match, exit nonzero CREDENTIAL_CANARY_EXPOSURE_DETECTED before public request or external side effect, and contribute to recomputed expected-detection aggregates rather than unexpected exposure; zeroed detections, PID/surface relabeling, hand-authored counters, mocks, stubs, skips, and static-only proof reject.",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'",
      "scenario": {
        ...
        "topology": "multi-node",
        "negative_control": {
          "would_fail_if": ["mocked_process", "stubbed_http", "static_only", "observer_skipped", "canary_in_argv", "canary_in_environment", "canary_in_output", "temp_config_leak", "live_secret_loaded", "network_mutation"]
        },
        "evidence": {
          "artifact_type": "stdout",
          "required_capture": true
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "Run six bounded real negative_controls subprocesses for direct argument, direct environment, indirect parameter/constructed-name output, printenv command-substitution argv, runtime-getenv output, and private artifact exposure.",
                "For every negative control recompute actual_process_spawned=true, actual_pid > 1, observed_pid === actual_pid, observed_surface === expected_surface, observed_canary_matches >= 1, exit_code != 0, error_code === CREDENTIAL_CANARY_EXPOSURE_DETECTED, public_request_issued=false, external_side_effect_performed=false, and applicable artifact cleanup.",
                "Require negative_controls_expected_detection_count=6 and negative_controls_failed_closed_count=6 while unexpected_exposure_count=0; reject a zeroed expected detection, PID/surface relabeling, or hand-authored counter.",
            ...
            "end_state": {
              "must_observe": [
                ...
                "negative_controls | length === 6 and every control has actual_process_spawned === true, actual_pid > 1, observed_pid === actual_pid, observed_surface === expected_surface, observed_canary_matches >= 1, exit_code != 0, error_code === CREDENTIAL_CANARY_EXPOSURE_DETECTED, public_request_issued === false, and external_side_effect_performed === false",
                "negative_controls_expected_detection_count === 6, negative_controls_failed_closed_count === 6, unexpected_exposure_count === 0, and observation_recomputed === true",
              ...
              "must_not_observe": [
                ...
                "a negative control with observed_canary_matches === 0, observed_pid !== actual_pid, or observed_surface !== expected_surface",
                "a hand-authored expected-detection counter or a negative control that issues a public request or external side effect",
    ...
    },
    {
      "id": "TC-14",
      "type": "test_criterion",
      "description": "The positive verifier path records zero forbidden-surface matches outside actual stdin/config lifecycle, while six distinct bounded real leak-control subprocesses each bind actual/observed PID and expected/observed surface, observe at least one intentional match, fail nonzero CREDENTIAL_CANARY_EXPOSURE_DETECTED, issue no public request or external side effect, and contribute only to recomputed expected-detection aggregates; zeroing a control detection or relabeling PID/surface rejects.",
      "maps_to_ac": "AC-11",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'"
    }
  ]
