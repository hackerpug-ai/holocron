...
PUBLIC AUTH AND SECRET HYGIENE: only `HOLO_KEY_RN` may authorize the public Hono chat POST/GET routes; `HOLO_KEY_MCP` is forbidden for every public request. Secret values are forbidden in process argv, stdout, stderr, JSON receipts, and retained artifacts. POSITIVE CREDENTIAL GRAMMAR: literal credential-name references are allowed in prose and credential-name lists, but command-bearing contract text may contain a shell expansion of `HOLO_KEY_RN` only on either of the two exact standalone lines below. The first line reads presence without expanding the value; the second puts the value only in the target verifier's private stdin. Any other unbraced, braced, or parameter-variant value expansion fails closed independent of wrapper, prefix, absolute-path command, inline assignment, curl option, or positional argv. The verifier must turn that private stdin into a mode-0600 temporary curl config, quote and escape every URL/header value, and trap-remove the config on every exit; it must never echo or persist the value.

```bash
test "${HOLO_KEY_RN+x}" = x
PLATFORM_IT=1 S33_REQUEST_HOST=inference1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode post-chat-invalid-stream --json <<<"${HOLO_KEY_RN}" >"$receipt"
```

In the deployed container, the telemetry/trace CLI is `/app/src/cli/holo.ts`; its command must bootstrap `DATABASE_URL` from `/run/secrets/database_url` inside the container without exposing the value. Once the public POST has been attempted, every subsequent error receipt must truthfully retain `chat_request_issued:true`; only failures before the POST may report `false`.

RUNTIME CREDENTIAL CANARY: static source scanning is defense in depth, not proof of secret-safe execution. The integration gate must generate a unique synthetic canary value only in test-process memory, keep every live credential value unloaded, and pipe the canary to the actual verifier over private stdin. The actual positive verifier must create and use the real quoted mode-0600 temporary curl config and an actual curl child against the real public Hono HTTP boundary. With a deterministic readiness barrier, the gate must inspect the owned verifier/curl process tree's observable argv and environment using the platform process observer, scan captured stdout/stderr/receipt/evidence plus retained files, inspect the temporary config only while it exists, and then prove trap cleanup removed it. `positive_path` records zero canary matches in argv, environment, stdout, stderr, receipt, evidence, artifact, and retained-file surfaces; its only allowed matches are private stdin plus the temporary config during that bounded lifecycle.

Intentional leak controls are a distinct `negative_controls[]` evidence surface and are never folded into the positive zero counters. Each control must spawn one bounded real subprocess, record and observer-bind its actual PID, declare an exact expected surface (`argv`, `environment`, `stdout`, `stderr`, or `artifact`), observe at least one canary match on that same surface, then require the target gate to return nonzero `CREDENTIAL_CANARY_EXPOSURE_DETECTED`. Each control forbids a public request and any external side effect; a private control artifact, when applicable, must be removed. Aggregate success requires all expected detections and fail-closed exits while `unexpected_exposure_count` remains zero. Receipt counters are recomputed from actual process/output/filesystem observations and cannot be hand-authored. If argv/environment observation is unavailable, the gate fails closed rather than skipping. Direct or indirect parameter, command-substitution, runtime-environment, or dynamically constructed-name retrieval controls must execute as real subprocesses; source regex alone cannot satisfy this gate.

The canonical negative-control manifest is the following exact ID-sorted set; every `id`, `kind`, `method`, and `expected_surface` is unique as a full tuple, and no row may be duplicated, omitted, added, or relabeled:

| id | kind | method | expected_surface |
|---|---|---|---|
| `artifact-write` | `artifact` | `private-artifact-write` | `artifact` |
| `command-substitution-printenv` | `command-substitution` | `printenv-command-substitution-into-argv` | `argv` |
| `direct-argv` | `direct-argv` | `direct-positional-argv` | `argv` |
| `direct-environment` | `direct-environment` | `direct-child-environment` | `environment` |
| `indirect-parameter-name-length` | `indirect-parameter` | `indirect-shell-parameter-name-length-to-stdout` | `stdout` |
| `runtime-language-getenv` | `runtime-getenv` | `runtime-language-getenv-to-stdout` | `stdout` |

Each manifest row owns exactly one real bounded spawned process instance. Its stable `actual_process_instance_id` must be pairwise distinct across the six controls even if the operating system reuses a PID; `actual_pid`/`actual_process_instance_id` must equal the observer's PID/instance fields. The expected surface has at least one canary match while every non-expected surface has zero, the detector exit is nonzero, and public-request/external-side-effect counters are zero. Aggregates are recomputed from this exact map, not from submitted counts.
...
### AC-10 — A real post-POST parser failure stays fail closed and proves secret-safe public auth

- **GIVEN** the public Hono route is authorized only with `HOLO_KEY_RN`, carried over private SSH stdin into a quoted mode-0600 temporary curl config that is trap-removed, and never with `HOLO_KEY_MCP`
- **WHEN** `scripts/verify-s33-mini-served-turn.sh --mode post-chat-invalid-stream` successfully issues a real public POST, captures the real public stream, makes a private temporary copy, deterministically truncates the first SSE JSON payload in that copy, and sends the malformed copy through the same verifier response parser used by the positive path
- **THEN** the target verifier itself exits nonzero with `CHAT_STREAM_PARSE_FAILED`, `chat_request_issued:true`, a successful 2xx POST plus non-empty chat-run ID, `failure_stage=stream-response-parse`, `stream_capture_source=real-public-stream`, `receipt_source=scripts/verify-s33-mini-served-turn.sh`, and `synthetic:false`
- **AND** the receipt proves the control used the target verifier path rather than a hand-authored receipt; the response mutation is confined to the private captured-response copy and performs no service, route, DNS, interface, Wi-Fi, Tailscale, or other network mutation
- **AND** the credential contract reports public key name `HOLO_KEY_RN`, MCP public-key use false, quoted curl-config values true, private SSH-stdin/mode-0600-temp-config transport, temp-config removal true, and secret presence false for argv, stdout, stderr, receipt, and retained artifact
- **AND** source and command review applies the positive credential grammar: the only allowed expansions are the exact names-only presence line and exact verifier private-stdin line above; every other value expansion rejects regardless of curl option, wrapper, prefix, or absolute path, while names-only literal prose remains valid
- **Verify:** `set -o pipefail; receipt="$(mktemp)"; chmod 600 "$receipt"; trap 'rm -f "$receipt"' EXIT; if PLATFORM_IT=1 S33_REQUEST_HOST=inference1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode post-chat-invalid-stream --json >"$receipt"; then exit 1; fi; jq -e '.ok == false and .error_code == "CHAT_STREAM_PARSE_FAILED" and .chat_request_issued == true and .public_post_succeeded == true and (.public_post_http_status >= 200 and .public_post_http_status < 300) and (.chat_run_id | type == "string" and length > 0) and .failure_stage == "stream-response-parse" and .stream_capture_source == "real-public-stream" and .response_mutation == "truncate-first-sse-json-in-private-copy" and .receipt_source == "scripts/verify-s33-mini-served-turn.sh" and .verifier_mode == "post-chat-invalid-stream" and .synthetic == false and .network_mutation_performed == false and .literal_disconnect_claimed == false and .credential_contract.public_key_name == "HOLO_KEY_RN" and .credential_contract.mcp_key_used_for_public_request == false and .credential_contract.curl_config_values_quoted == true and .credential_contract.secret_transport == "ssh-stdin-private-0600-temp-curl-config" and .credential_contract.private_temp_config_removed == true and .credential_contract.secret_in_argv == false and .credential_contract.secret_in_stdout == false and .credential_contract.secret_in_stderr == false and .credential_contract.secret_in_receipt == false and .credential_contract.secret_in_artifact == false' "$receipt"`
- **Tier:** e2e · **Service:** deployed-holocron-chat + target verifier parser · **Flow:** UC-INFER-01
- **Scenario:** topology `multi-node` · evidence `stdout` · negative control: hand-authored receipt, pre-POST failure, synthetic stream, secret exposure, MCP public auth, network mutation

### AC-11 — A synthetic canary proves the real verifier transport is secret-safe at runtime

- **GIVEN** the integration test creates a unique synthetic canary only in its own memory, removes all live credential-bearing values from the verifier/curl child environment, and retains credential names only in diagnostics
- **WHEN** it pipes the canary through the actual verifier's private stdin, holds the actual verifier/curl child behind a bounded readiness barrier, observes the owned OS process tree's argv and environment using the platform-supported observer, and drives actual curl through a real mode-0600 temporary config to the real public Hono HTTP boundary
- **THEN** `positive_path` records the canary only in private stdin and the temporary config while it exists; its argv, environment, stdout, stderr, receipt, evidence, artifact, and retained-file match counters are all zero; the config is quoted, mode `0600`, and absent after trap cleanup
- **AND** `negative_controls[]` equals the exact six-row ID-sorted manifest above with no duplicate, missing, extra, or relabeled ID/kind/method/surface, and every row records exactly one real bounded process, integer `actual_pid > 1`, pairwise-distinct `process_instance_id`, equality of actual/observed PID and process-instance identity, `observed_surface === expected_surface`, `expected_surface_canary_matches >= 1`, `non_expected_surface_canary_matches === 0`, nonzero `CREDENTIAL_CANARY_EXPOSURE_DETECTED`, `public_request_count === 0`, `external_side_effect_count === 0`, and applicable artifact cleanup
- **AND** aggregate success requires exactly six manifest matches, expected detections, and fail-closed exits, recomputed from the exact map, while duplicate/missing/extra counts and `unexpected_exposure_count` are zero; zeroing a required detection, reusing a process identity, relabeling a kind/PID/surface, or submitting a hand-authored aggregate fails
- **AND** the gate uses the actual verifier, actual processes, real filesystem, and real HTTP request with no mocks, stubs, recorded response, hand-authored receipt, static-only source scan, service/network mutation, or live secret value in output or artifacts
- **Verify:** `PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'`
- **Tier:** integration · **Service:** actual verifier process + filesystem + public Hono HTTP · **Flow:** UC-INFER-01
- **Scenario:** topology `multi-node` · evidence `stdout` · negative control: real direct/indirect canary exposure subprocesses and trap-cleanup assertion
...
| TC-13 | The target verifier issues a real public 2xx POST, then its production parser rejects a deterministically truncated private copy of the real SSE stream, exits nonzero with `chat_request_issued:true`, and proves RN-only public auth plus zero secret exposure or network mutation. | AC-10 | `set -o pipefail; receipt="$(mktemp)"; chmod 600 "$receipt"; trap 'rm -f "$receipt"' EXIT; if PLATFORM_IT=1 S33_REQUEST_HOST=inference1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode post-chat-invalid-stream --json >"$receipt"; then exit 1; fi; jq -e '.ok == false and .error_code == "CHAT_STREAM_PARSE_FAILED" and .chat_request_issued == true and .public_post_succeeded == true and (.public_post_http_status >= 200 and .public_post_http_status < 300) and (.chat_run_id | type == "string" and length > 0) and .failure_stage == "stream-response-parse" and .stream_capture_source == "real-public-stream" and .response_mutation == "truncate-first-sse-json-in-private-copy" and .receipt_source == "scripts/verify-s33-mini-served-turn.sh" and .verifier_mode == "post-chat-invalid-stream" and .synthetic == false and .network_mutation_performed == false and .literal_disconnect_claimed == false and .credential_contract.public_key_name == "HOLO_KEY_RN" and .credential_contract.mcp_key_used_for_public_request == false and .credential_contract.curl_config_values_quoted == true and .credential_contract.secret_transport == "ssh-stdin-private-0600-temp-curl-config" and .credential_contract.private_temp_config_removed == true and .credential_contract.secret_in_argv == false and .credential_contract.secret_in_stdout == false and .credential_contract.secret_in_stderr == false and .credential_contract.secret_in_receipt == false and .credential_contract.secret_in_artifact == false' "$receipt"` |
| TC-14 | The positive verifier path has zero canary matches outside actual stdin/config lifecycle, while the exact six-row ID-sorted negative-control manifest has one distinct real bounded process instance per unique ID/kind/method mapping; every row binds actual/observed identity and expected surface, has positive expected-surface and zero off-surface matches, fails nonzero, has zero request/side effects, and contributes to recomputed exact-map aggregates. | AC-11 | `PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'` |
...
- POST https://holocron.tail011a51.ts.net:44111/api/chat-runs with Authorization Bearer HOLO_KEY_RN only; HOLO_KEY_MCP is forbidden; carry the secret over private SSH stdin into a mode-0600 temporary curl config, quote/escape every curl-config URL/header value, trap-remove the config, and expose no secret value in argv/stdout/stderr/receipt/artifact
...
- the public chat request is executed from `inference1` with `HOLO_KEY_RN` only and never `HOLO_KEY_MCP`; auth is delivered over private SSH stdin into a mode-0600 temporary curl config, URL/header curl-config values are quoted and escaped, a trap removes the config on every exit, and secret values never enter argv, stdout, stderr, JSON receipts, or retained artifacts
...
**`post_chat_invalid_stream_control`** — One real public 2xx POST followed by the target verifier's production parser consuming a deterministically truncated private copy of the real SSE response. _(seed: public_api)_

- the verifier, not a fixture author, emits the fail-closed JSON receipt after it obtains a non-empty real chat-run ID
- only the private mode-0600 response copy is truncated; the live service, request, route, DNS, interface, Wi-Fi, Tailscale, and network remain unchanged
- expected receipt fields include `CHAT_STREAM_PARSE_FAILED`, `chat_request_issued:true`, `public_post_succeeded:true`, `stream_capture_source:real-public-stream`, `receipt_source:scripts/verify-s33-mini-served-turn.sh`, and `synthetic:false`
- credential evidence names `HOLO_KEY_RN`, denies public `HOLO_KEY_MCP`, proves quoted curl config plus private stdin/temp-file lifecycle, and denies secret values in argv/stdout/stderr/receipt/artifact
- source review rejects secret-bearing curl `-H`/`-d`/`-u` separated, clustered, attached, and equals spellings, equivalent long/positional argv channels, and inline secret assignments while allowing names-only presence checks and private stdin/config-file paths

**`credential_canary_transport`** — A unique synthetic value generated only in test memory and sent through actual verifier stdin, actual temporary-file handling, actual child processes, and the real public Hono HTTP boundary. _(seed: cli)_

- live credential values remain unloaded; logs, receipts, evidence, and retained artifacts contain credential names and boolean/count facts only
- an owned-process readiness barrier permits platform observation of the actual verifier/curl child argv and environment; unavailable observation fails the gate
- the canary exists only in private stdin and the quoted mode-0600 curl config during the request, then the trap removes the config
- `positive_path` has zero argv/environment/output/receipt/evidence/artifact/retained-file matches and preserves the private stdin plus temporary-config lifecycle
- the six `negative_controls[]` rows equal the exact ID-sorted manifest; unique ID/kind/method tuples map `artifact-write→artifact`, `command-substitution-printenv→argv`, `direct-argv→argv`, `direct-environment→environment`, `indirect-parameter-name-length→stdout`, and `runtime-language-getenv→stdout`
- each manifest row owns exactly one real bounded process, binds actual/observed PID plus robust process-instance identity, has positive expected-surface and zero off-surface matches, fails closed, issues no public request or side effect, and cleans applicable artifacts
- exact-map matches, expected detections, and fail-closed exits are counted separately from zero duplicate/missing/extra/wrong-surface/unexpected-exposure counts; actual observation is recomputed and hand-authored counters cannot pass
- no mock process, mock filesystem, mocked HTTP client, recorded response, stub, hand-authored receipt, or static-only source scan can satisfy this fixture
...
- The credential-canary integration case generates its synthetic value in test memory, sends it only through the actual positive verifier's stdin, uses an actual curl child and real public Hono request, and synchronizes with the owned process tree through a bounded readiness barrier so the platform observer can inspect real argv/environment. Six separate intentional-leak subprocess controls bind observed PID/surface/match evidence and fail before any public request; expected detections never contaminate positive-path zero counters. An unsupported observer is a blocking failure, not a skip.
...
| credential canary runtime safety | `PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'` | Exit 0 only after actual verifier/process/filesystem/public-HTTP execution proves the synthetic canary existed solely in private stdin and the temporary mode-0600 curl config, then cleanup removed it; direct and indirect exposure controls fail closed |
...
- Only HOLO_KEY_RN may authorize public Hono chat requests; HOLO_KEY_MCP is categorically forbidden on those requests. Never place HOLO_KEY_RN, HOLO_KEY_MCP, FLEET_KEY, DATABASE_URL, or any SSH credential value in argv, stdout, stderr, JSON receipts, or retained artifacts — node identities, credential names, and endpoint hosts only.
- Public-request auth crosses SSH only through private stdin, is written only to a mode-0600 temporary curl config whose URL/header values are quoted and escaped, and is removed by a trap on every exit.
- Apply the positive credential grammar to every target-contract occurrence: allow names-only literal prose plus the canonical presence-check and verifier-private-stdin lines declared above; reject every other shell expansion independent of command prefix or wrapper. The verifier-owned private stdin becomes a quoted mode-0600 temporary curl config that is trap-removed; non-secret private file paths used for stdin or `curl --config` remain valid.
- Static source and command scanning never substitutes for the runtime canary gate. The positive path uses an internally generated synthetic value, actual verifier/curl processes, platform argv/environment observation, a real temporary file, and a real public Hono HTTP request; its zero counters exclude the exact six-row ID-sorted negative-control manifest. Every unique ID/kind/method row owns one distinct real bounded process instance and recomputes PID/instance binding, its pinned expected surface, positive expected and zero off-surface matches, nonzero fail-closed exit, zero request/side-effect counters, and cleanup. Duplicate/missing/extra rows, wrong surfaces, reused process identity, or submitted aggregates fail.
...
- PUBLIC AUTH TRANSPORT: public Hono requests use only `HOLO_KEY_RN`; `HOLO_KEY_MCP` is forbidden. Secrets travel through private SSH stdin to a mode-0600 temporary curl config with quoted/escaped URL and header values, are trap-removed on every exit, and never appear in argv, stdout, stderr, receipts, or retained artifacts.
- SECRET-EXPANSION GRAMMAR: names-only literals are valid, and exactly 2 standalone command forms are valid — the names-only presence check and target-verifier private-stdin transport declared above. Every other `HOLO_KEY_RN` shell expansion fails closed regardless of wrapper/prefix/absolute path; private non-secret stdin/config path arguments remain valid.
- RUNTIME CREDENTIAL CANARY: `positive_path` carries a synthetic in-memory value through the actual verifier, private stdin, mode-0600 config, curl child, filesystem, and public Hono boundary with zero matches on every forbidden surface and verified cleanup. `negative_controls[]` equals the exact canonical ID-sorted six-row ID/kind/method/surface map; each row owns one distinct real bounded process instance, binds actual to observer PID/instance, has positive expected-surface and zero off-surface matches, exits nonzero `CREDENTIAL_CANARY_EXPOSURE_DETECTED`, and has zero public-request/external-side-effect counts plus cleanup. Aggregate success is recomputed from the exact map. Duplicate/missing/extra/relabelled rows, reused process identity, submitted counters, static-only, mocked, stubbed, recorded, skipped-observer, or live-secret-bearing evidence is invalid.
- POST-CHAT FAILURE TRUTH: `post-chat-invalid-stream` first completes a real public 2xx POST and obtains its chat-run ID, then truncates only a private copy of the real SSE response and exercises the same target response parser. The target verifier must exit nonzero with `chat_request_issued=true`; a hand-authored receipt, synthetic stream, pre-POST failure, or network mutation cannot satisfy the control.
...
- LIVE VERIFIER EXECUTION: public Hono auth is `HOLO_KEY_RN` only and never `HOLO_KEY_MCP`; auth crosses private SSH stdin into a trap-removed mode-0600 temporary curl config with quoted/escaped URL/header values, and secret values stay absent from argv/stdout/stderr/receipts/artifacts; in-container telemetry commands use `/app/src/cli/holo.ts` and privately bootstrap `DATABASE_URL` from `/run/secrets/database_url`; any error after POST issuance reports `chat_request_issued:true`.
...
<!--
{
  ...
  "fixtures": {
    "nonce_chat_turn": {
      ...
      "records": [
        "POST https://holocron.tail011a51.ts.net:44111/api/chat-runs with Authorization Bearer HOLO_KEY_RN only; HOLO_KEY_MCP is forbidden; carry auth over private SSH stdin into a trap-removed mode-0600 temporary curl config with quoted and escaped URL/header values and no secret value in argv, stdout, stderr, receipt, or artifact",
        "body: requestId s33-<uuid>, msg 'S33 nonce <uuid>: reply with one short sentence.'",
        "captured: run id, conversation id, request start timestamp in ISO-8601, nonce"
      ]
    },
    ...
    "deployed_router_topology": {
      ...
      "records": [
        ...
        "the public chat request is executed from inference1 with HOLO_KEY_RN only and never HOLO_KEY_MCP; auth arrives over private SSH stdin in a trap-removed mode-0600 temporary curl config whose URL/header values are quoted and escaped while secret values remain absent from argv, stdout, stderr, JSON receipts, and retained artifacts",
    ...
    "post_chat_invalid_stream_control": {
      "description": "One real public 2xx POST followed by the target verifier production parser consuming a deterministically truncated private copy of the real SSE response; the verifier emits the failure receipt itself.",
      "seed_method": "public_api",
      "records": [
        "the target verifier obtains a non-empty real chat-run ID before inducing the downstream parse failure",
        "only a private mode-0600 copy of the real public SSE response is truncated; no live service or network state is mutated",
        "the target verifier exits nonzero with CHAT_STREAM_PARSE_FAILED, chat_request_issued=true, public_post_succeeded=true, stream_capture_source=real-public-stream, receipt_source=scripts/verify-s33-mini-served-turn.sh, and synthetic=false",
        "HOLO_KEY_RN is the only public credential name; HOLO_KEY_MCP public use is false; quoted curl config, private stdin/temp lifecycle, trap removal, and zero argv/stdout/stderr/receipt/artifact secret exposure are mandatory",
        "a positive grammar allows names-only literal references, exact names-only presence check, and exact target-verifier private-stdin transport only; every other HOLO_KEY_RN shell expansion rejects independent of wrapper, prefix, absolute path, curl option, or positional argv while private non-secret stdin/config path arguments remain valid"
      ]
    },
    "credential_canary_transport": {
      "description": "A unique synthetic value generated only in integration-test memory traverses actual verifier stdin, actual child processes, a real temporary curl config, and the real public Hono HTTP boundary without loading any live credential value.",
      "seed_method": "cli",
      "records": [
        "the actual verifier and curl child receive the canary only through private stdin and a quoted mode-0600 temporary curl config; the config is absent after trap cleanup",
        "a bounded readiness barrier permits the platform process observer to inspect the owned verifier/curl process tree actual argv and environment; unavailable observation fails rather than skips",
        "positive_path scans prove zero canary matches in every observed argv/environment, captured stdout/stderr, JSON receipt, evidence object, artifact, and retained file while private stdin/config lifecycle matches remain separate",
        "negative_controls equals the exact ID-sorted manifest: artifact-write/artifact/private-artifact-write/artifact; command-substitution-printenv/command-substitution/printenv-command-substitution-into-argv/argv; direct-argv/direct-argv/direct-positional-argv/argv; direct-environment/direct-environment/direct-child-environment/environment; indirect-parameter-name-length/indirect-parameter/indirect-shell-parameter-name-length-to-stdout/stdout; runtime-language-getenv/runtime-getenv/runtime-language-getenv-to-stdout/stdout",
        "each unique manifest row owns exactly one bounded real process with a pairwise-distinct robust process-instance identity, binds actual to observer PID/instance, records positive expected-surface and zero non-expected-surface matches, fails CREDENTIAL_CANARY_EXPOSURE_DETECTED, and has zero public requests or external side effects",
        "manifest matches, expected detections, and fail-closed exits are recomputed from the exact map while duplicate/missing/extra/wrong-surface/unexpected-exposure counts are zero; relabeling or reusing process identity and hand-authored aggregates reject",
        "the HTTP request reaches the real public Hono boundary; mocks, stubs, recorded responses, hand-authored receipts, and static-only scanning are forbidden",
        "artifacts contain live credential names only, never live credential values, and network_mutation_performed=false"
      ]
    },
  ...
  "requirements": [
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "Record the request-window start timestamp and request_origin=inference1, then execute POST /api/chat-runs from inference1 with the nonce prompt and HOLO_KEY_RN only, never HOLO_KEY_MCP; carry auth over private SSH stdin into a trap-removed mode-0600 temporary curl config, quote and escape each URL/header value, and place no secret value in argv, stdout, stderr, JSON receipt, or retained artifact.",
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN RN-only public auth and private quoted curl-config secret handling WHEN the target verifier successfully issues a real public 2xx POST and then drives its production response parser with a deterministically truncated private copy of the real SSE stream THEN it exits nonzero with truthful post-POST issuance, real stream provenance, target-verifier receipt provenance, zero secret exposure, and zero network mutation.",
      "verify": "set -o pipefail; receipt=\"$(mktemp)\"; chmod 600 \"$receipt\"; trap 'rm -f \"$receipt\"' EXIT; if PLATFORM_IT=1 S33_REQUEST_HOST=inference1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode post-chat-invalid-stream --json >\"$receipt\"; then exit 1; fi; jq -e '.ok == false and .error_code == \"CHAT_STREAM_PARSE_FAILED\" and .chat_request_issued == true and .public_post_succeeded == true and (.public_post_http_status >= 200 and .public_post_http_status < 300) and (.chat_run_id | type == \"string\" and length > 0) and .failure_stage == \"stream-response-parse\" and .stream_capture_source == \"real-public-stream\" and .response_mutation == \"truncate-first-sse-json-in-private-copy\" and .receipt_source == \"scripts/verify-s33-mini-served-turn.sh\" and .verifier_mode == \"post-chat-invalid-stream\" and .synthetic == false and .network_mutation_performed == false and .literal_disconnect_claimed == false and .credential_contract.public_key_name == \"HOLO_KEY_RN\" and .credential_contract.mcp_key_used_for_public_request == false and .credential_contract.curl_config_values_quoted == true and .credential_contract.secret_transport == \"ssh-stdin-private-0600-temp-curl-config\" and .credential_contract.private_temp_config_removed == true and .credential_contract.secret_in_argv == false and .credential_contract.secret_in_stdout == false and .credential_contract.secret_in_stderr == false and .credential_contract.secret_in_receipt == false and .credential_contract.secret_in_artifact == false' \"$receipt\"",
      "scenario": {
        "id": "S33-PLAT-05/AC-10",
        "tier": "visible",
        "test_tier": "e2e",
        "verification_service": "deployed-holocron-chat + target verifier parser",
        "topology": "multi-node",
        "negative_control": {
          "would_fail_if": ["hand_authored_receipt", "pre_post_failure", "synthetic_stream", "empty", "static", "mcp_public_auth", "secret_exposure", "network_mutation"]
        },
        "evidence": {
          "artifact_type": "stdout",
          "required_capture": true
        ...
        "cases": [
          {
            "start_ref": "post_chat_invalid_stream_control",
            "action": {
              "actor": "operator invoking the target verifier from inference1 against the public Hono service",
              "steps": [
                "Carry HOLO_KEY_RN over private SSH stdin into a trap-removed mode-0600 curl config, quote and escape each URL/header value, forbid HOLO_KEY_MCP, and retain no secret value in argv, stdout, stderr, receipt, or artifact.",
                "Apply the positive credential grammar across the target contract: accept exactly the canonical names-only presence line and canonical target-verifier private-stdin line plus literal names in prose, and reject every other HOLO_KEY_RN shell expansion without interpreting command prefixes.",
                "Use the target verifier to complete a real public 2xx POST and retain its non-empty chat-run ID with chat_request_issued=true.",
                "Capture the real public SSE stream, create a private temporary copy, truncate the first SSE JSON payload only in that copy, and invoke the same production response-parser path used by the positive verifier.",
                "Require the target verifier itself to emit the JSON failure receipt and exit nonzero; do not hand-author a receipt or substitute a synthetic stream.",
                "Assert the full failure, credential-lifecycle, provenance, and Network Continuity fields with jq -e."
              ]
            },
            "end_state": {
              "must_observe": [
                "a real public 2xx POST and non-empty chat-run ID precede the response parse failure",
                "CHAT_STREAM_PARSE_FAILED and chat_request_issued === true are emitted by scripts/verify-s33-mini-served-turn.sh while that script exits nonzero",
                "stream_capture_source === real-public-stream, response_mutation === truncate-first-sse-json-in-private-copy, and synthetic === false",
                "public_key_name === HOLO_KEY_RN, MCP public-key use is false, quoted private curl-config lifecycle is complete, and all five secret-exposure flags are false",
                "exactly 2 canonical expansion-bearing command forms pass while wrapper, prefix, absolute-path, curl option, positional argument, inline assignment, and parameter-variant expansions reject",
                "network_mutation_performed === false and literal_disconnect_claimed === false"
              ],
              "must_not_observe": [
                "a hand-authored, empty, static, or success-exit receipt",
                "chat_request_issued=false after the POST",
                "HOLO_KEY_MCP on a public request",
                "a secret value in argv, stdout, stderr, JSON receipt, or retained artifact",
                "secret expansion through curl short/long options, positional argv, or an inline environment assignment",
                "a synthetic/recorded stream or any live service/network mutation"
              ]
            }
          }
        ]
    ...
    },
    {
      "id": "TC-13",
      "type": "test_criterion",
      "description": "The target verifier completes a real public POST, then its production parser rejects a deterministically truncated private copy of the real SSE stream, exits nonzero with truthful issuance, and proves RN-only public auth plus zero secret exposure and zero network mutation.",
      "maps_to_ac": "AC-10",
      "verify": "set -o pipefail; receipt=\"$(mktemp)\"; chmod 600 \"$receipt\"; trap 'rm -f \"$receipt\"' EXIT; if PLATFORM_IT=1 S33_REQUEST_HOST=inference1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode post-chat-invalid-stream --json >\"$receipt\"; then exit 1; fi; jq -e '.ok == false and .error_code == \"CHAT_STREAM_PARSE_FAILED\" and .chat_request_issued == true and .public_post_succeeded == true and (.public_post_http_status >= 200 and .public_post_http_status < 300) and (.chat_run_id | type == \"string\" and length > 0) and .failure_stage == \"stream-response-parse\" and .stream_capture_source == \"real-public-stream\" and .response_mutation == \"truncate-first-sse-json-in-private-copy\" and .receipt_source == \"scripts/verify-s33-mini-served-turn.sh\" and .verifier_mode == \"post-chat-invalid-stream\" and .synthetic == false and .network_mutation_performed == false and .literal_disconnect_claimed == false and .credential_contract.public_key_name == \"HOLO_KEY_RN\" and .credential_contract.mcp_key_used_for_public_request == false and .credential_contract.curl_config_values_quoted == true and .credential_contract.secret_transport == \"ssh-stdin-private-0600-temp-curl-config\" and .credential_contract.private_temp_config_removed == true and .credential_contract.secret_in_argv == false and .credential_contract.secret_in_stdout == false and .credential_contract.secret_in_stderr == false and .credential_contract.secret_in_receipt == false and .credential_contract.secret_in_artifact == false' \"$receipt\""
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN a unique synthetic canary generated only in integration-test memory with live credential values unloaded WHEN the actual positive verifier receives it over private stdin, uses an actual curl child plus a real quoted mode-0600 config and public Hono request, and the platform observer inspects owned-process argv/environment THEN positive_path has zero matches on every forbidden surface and verified cleanup, while negative_controls equals the exact ID-sorted six-row unique ID/kind/method/surface manifest and every row owns one distinct bounded real process instance, binds actual/observed PID and robust instance identity, records positive expected-surface plus zero off-surface matches, exits nonzero CREDENTIAL_CANARY_EXPOSURE_DETECTED with zero request/side-effect counts, and contributes to exact-map recomputed aggregates; duplicate/missing/extra/relabelled rows, reused process identity, submitted aggregates, mocks, stubs, skips, and static-only proof reject.",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'",
      "scenario": {
        "id": "S33-PLAT-05/AC-11",
        "tier": "visible",
        "test_tier": "integration",
        "verification_service": "actual verifier process + filesystem + public Hono HTTP",
        "topology": "multi-node",
        "negative_control": {
          "would_fail_if": ["mocked_process", "stubbed_http", "static_only", "observer_skipped", "canary_in_argv", "canary_in_environment", "canary_in_output", "temp_config_leak", "duplicate_control_kind", "missing_control_kind", "extra_control_kind", "wrong_control_surface", "reused_process_identity", "aggregate_not_recomputed", "live_secret_loaded", "network_mutation"]
        },
        "evidence": {
          "artifact_type": "stdout",
          "required_capture": true
        ...
        "cases": [
          {
            "start_ref": "credential_canary_transport",
            "action": {
              "actor": "integration test controlling the actual target verifier and its owned child processes",
              "steps": [
                "Generate a unique synthetic canary only in test memory, unload every live credential value from child environments, and pipe the canary to the actual verifier private stdin.",
                "Use a bounded readiness barrier and the platform-supported process observer to capture the actual verifier/curl child PID lineage, argv, and environment; fail if observation is unavailable.",
                "Inspect the actual quoted mode-0600 curl config during the request, drive actual curl to the real public Hono HTTP boundary, then require trap cleanup to remove the config.",
                "Record positive_path separately: scan observed argv/environment, captured stdout/stderr, JSON receipt, evidence objects, artifacts, and retained files and require zero matches outside private stdin and the temporary config lifecycle.",
                "Instantiate exactly the ID-sorted six-row negative_control_manifest: artifact-write/artifact/private-artifact-write/artifact; command-substitution-printenv/command-substitution/printenv-command-substitution-into-argv/argv; direct-argv/direct-argv/direct-positional-argv/argv; direct-environment/direct-environment/direct-child-environment/environment; indirect-parameter-name-length/indirect-parameter/indirect-shell-parameter-name-length-to-stdout/stdout; runtime-language-getenv/runtime-getenv/runtime-language-getenv-to-stdout/stdout.",
                "For each unique manifest row spawn exactly one bounded real process and recompute actual_process_spawned=true, spawned_process_count=1, actual_pid > 1, observed_pid === actual_pid, actual_process_instance_id === observed_process_instance_id, observed_surface === expected_surface, observed_canary_matches === expected_surface_canary_matches, expected_surface_canary_matches >= 1, non_expected_surface_canary_matches=0, exit_code != 0, error_code === CREDENTIAL_CANARY_EXPOSURE_DETECTED, public_request_count=0, external_side_effect_count=0, and applicable artifact cleanup.",
                "Require six pairwise-distinct actual_process_instance_id values and exact-manifest/unique-ID/unique-kind/unique-method counts of six; recompute duplicate/missing/extra/wrong-surface counts as zero from the exact manifest rather than trusting receipt fields.",
                "Require negative_controls_expected_detection_count=6 and negative_controls_failed_closed_count=6 while unexpected_exposure_count=0; reject a zeroed expected detection, duplicated/missing/extra kind, wrong surface, reused process identity/PID binding, or aggregate_not_recomputed.",
                "Retain only credential names and boolean/count facts; perform no service or network mutation."
              ]
            },
            "end_state": {
              "must_observe": [
                "actual_verifier_executed === true, actual_curl_child_count >= 1, real_filesystem_observed === true, public_hono_http_reached === true, mock_count === 0, stub_count === 0, and recorded_response_count === 0",
                "private_stdin_canary_matches === 1 and temp_config_canary_matches_during_request === 1 while temp_config_mode === 0600",
                "positive_path.argv_canary_matches === 0, positive_path.environment_canary_matches === 0, positive_path.stdout_canary_matches === 0, positive_path.stderr_canary_matches === 0, positive_path.receipt_canary_matches === 0, positive_path.evidence_canary_matches === 0, positive_path.artifact_canary_matches === 0, and positive_path.retained_file_canary_matches === 0",
                "private_temp_config_removed === true, retained_temp_config_count === 0, and process_observer_skipped === false",
                "negative_control_manifest === [{id:artifact-write,kind:artifact,method:private-artifact-write,expected_surface:artifact},{id:command-substitution-printenv,kind:command-substitution,method:printenv-command-substitution-into-argv,expected_surface:argv},{id:direct-argv,kind:direct-argv,method:direct-positional-argv,expected_surface:argv},{id:direct-environment,kind:direct-environment,method:direct-child-environment,expected_surface:environment},{id:indirect-parameter-name-length,kind:indirect-parameter,method:indirect-shell-parameter-name-length-to-stdout,expected_surface:stdout},{id:runtime-language-getenv,kind:runtime-getenv,method:runtime-language-getenv-to-stdout,expected_surface:stdout}] and negative_control_manifest_is_id_sorted === true",
                "negative_controls | length === 6 and every control has actual_process_spawned === true, spawned_process_count === 1, bounded_process === true, actual_pid > 1, observed_pid === actual_pid, actual_process_instance_id === observed_process_instance_id, observed_surface === expected_surface, observed_canary_matches === expected_surface_canary_matches, expected_surface_canary_matches >= 1, non_expected_surface_canary_matches === 0, exit_code != 0, error_code === CREDENTIAL_CANARY_EXPOSURE_DETECTED, public_request_count === 0, external_side_effect_count === 0, and control_artifact_removed === true when expected_surface === artifact",
                "negative_control_manifest_match === true, negative_control_unique_id_count === 6, negative_control_unique_kind_count === 6, negative_control_unique_method_count === 6, negative_control_unique_process_instance_count === 6, negative_control_duplicate_count === 0, negative_control_missing_count === 0, negative_control_extra_count === 0, negative_control_wrong_surface_count === 0",
                "negative_controls_expected_detection_count === 6, negative_controls_failed_closed_count === 6, unexpected_exposure_count === 0, observation_recomputed === true, and aggregate_recomputed_from_exact_manifest === true",
                "live_credential_values_loaded === 0, live_credential_values_printed === 0, and network_mutation_performed === false"
              ],
              "must_not_observe": [
                "a canary value in positive_path argv, environment, stdout, stderr, JSON receipt, evidence, artifact, or retained files",
                "a duplicate, missing, extra, or relabeled negative-control ID/kind/method/surface; a reused process instance; or an actual/observed PID/instance mismatch",
                "a negative control with expected_surface_canary_matches === 0, non_expected_surface_canary_matches > 0, public_request_count > 0, external_side_effect_count > 0, or nonzero retained artifact count",
                "a hand-authored expected-detection counter or aggregate not recomputed from the exact manifest",
                "a remaining temporary curl config",
                "an observer skip or unobserved owned child process",
                "a mock, stub, recorded response, hand-authored receipt, or static-only proof",
                "a live credential value or service/network mutation",
                "an empty (0), static, or start-only success receipt"
              ]
            }
          }
        ]
    ...
    },
    {
      "id": "TC-14",
      "type": "test_criterion",
      "description": "The positive verifier path records zero forbidden-surface matches outside actual stdin/config lifecycle, while the exact ID-sorted six-row unique ID/kind/method/surface manifest owns one distinct bounded real process instance per row, binds actual/observed PID and robust instance identity, records positive expected-surface plus zero off-surface/request/side-effect counts, fails nonzero CREDENTIAL_CANARY_EXPOSURE_DETECTED, and contributes only to exact-map recomputed aggregates; duplicate/missing/extra kinds, wrong surfaces, reused identity, zeroed detection, relabeling, or submitted aggregates reject.",
      "maps_to_ac": "AC-11",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'"
    }
  ]
