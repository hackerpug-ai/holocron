...
RUNTIME CREDENTIAL CANARY: static source scanning is defense in depth, not proof of secret-safe execution. The integration gate must generate a unique synthetic canary value only in test-process memory, keep every live credential value unloaded, and pipe the canary to the actual verifier over private stdin. The actual verifier must create and use the real quoted mode-0600 temporary curl config and an actual curl child against the real public Hono HTTP boundary. With a deterministic readiness barrier, the gate must inspect the owned verifier/curl process tree's observable argv and environment using the platform process observer, scan captured stdout/stderr/receipt/evidence plus retained files, inspect the temporary config only while it exists, and then prove trap cleanup removed it. The canary may appear only in private stdin and that temporary config during its bounded lifecycle. If argv/environment observation is unavailable, the gate fails closed rather than skipping. Direct or indirect parameter, command-substitution, runtime-environment, or dynamically constructed-name retrieval controls must execute as real subprocesses and fail `CREDENTIAL_CANARY_EXPOSURE_DETECTED`; source regex alone cannot satisfy this gate.
...
- **THEN** the canary is observed only in private stdin and the temporary config while it exists; it is absent from every observed argv/environment, captured stdout/stderr, JSON receipt, evidence object, and retained file; the config is quoted, mode `0600`, and absent after trap cleanup
- **AND** actual subprocess controls for direct argument/environment exposure and indirect parameter/name/length, command-substitution, `printenv`, and runtime `getenv` retrieval each exit nonzero with `CREDENTIAL_CANARY_EXPOSURE_DETECTED`; inability to observe owned process argv/environment is a failure, never a skip
...
| TC-14 | A unique synthetic canary traverses only actual verifier stdin and its real mode-0600 curl config while actual OS-process, output, receipt, evidence, retained-file, HTTP, and cleanup observations prove zero other exposure; real direct/indirect exposure controls fail closed. | AC-11 | `PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'` |
...
- actual negative subprocesses exercise direct argv/environment plus indirect parameter/name/length, command-substitution, `printenv`, and runtime `getenv` retrieval; every exposure is detected and fails closed
...
- The credential-canary integration case generates its synthetic value in test memory, sends it only through the actual verifier's stdin, uses an actual curl child and real public Hono request, and synchronizes with the owned process tree through a bounded readiness barrier so the platform observer can inspect real argv/environment. An unsupported observer is a blocking failure, not a skip.
...
- Static source and command scanning never substitutes for the runtime canary gate. That gate uses an internally generated synthetic value, actual verifier/curl processes, platform argv/environment observation, a real temporary file, and a real public Hono HTTP request; it scans outputs/evidence/retained files in memory, removes the temp config, and never loads or prints a live credential value.
...
- RUNTIME CREDENTIAL CANARY: a synthetic value generated only in test memory must traverse the actual verifier, real private stdin, real mode-0600 quoted curl config, actual child processes, real filesystem, and public Hono HTTP boundary. Platform observation must find it nowhere in owned-process argv/environment, outputs, receipts, evidence, or retained files; the config must be absent after cleanup. Actual direct and indirect retrieval controls fail `CREDENTIAL_CANARY_EXPOSURE_DETECTED`. Static-only, mocked, stubbed, recorded, skipped-observer, or live-secret-bearing evidence is invalid.
...
<!--
{
  ...
  "fixtures": {
    ...
    "credential_canary_transport": {
      ...
      "records": [
        ...
        "in-memory scans prove the canary is absent from every observed argv/environment, captured stdout/stderr, JSON receipt, evidence object, and retained file",
        "actual negative subprocesses exercise direct argv/environment and indirect parameter, constructed-name, command-substitution, printenv, and runtime-getenv retrieval; each fails CREDENTIAL_CANARY_EXPOSURE_DETECTED",
  ...
  "requirements": [
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN a unique synthetic canary generated only in integration-test memory with live credential values unloaded WHEN the actual verifier receives it over private stdin, uses actual child processes plus a real quoted mode-0600 curl config and public Hono HTTP request, and the platform observer inspects owned-process argv/environment THEN the canary exists only in private stdin/config during the bounded lifecycle, is absent from all other process/output/evidence/retained-file surfaces, the config is removed, and real direct/indirect retrieval subprocess controls fail CREDENTIAL_CANARY_EXPOSURE_DETECTED without mocks, stubs, skips, or static-only proof.",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'",
      "scenario": {
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "Scan observed argv/environment, captured stdout/stderr, JSON receipt, evidence objects, and retained files for the in-memory canary and require zero matches outside private stdin and the temporary config lifecycle.",
                "Run actual direct-argument, direct-environment, indirect parameter/constructed-name, command-substitution, printenv, and runtime-getenv exposure subprocess controls and require CREDENTIAL_CANARY_EXPOSURE_DETECTED from each.",
            ...
            "end_state": {
              "must_observe": [
                ...
                "argv_canary_matches === 0, environment_canary_matches === 0, stdout_canary_matches === 0, stderr_canary_matches === 0, receipt_canary_matches === 0, evidence_canary_matches === 0, and retained_file_canary_matches === 0",
                "private_temp_config_removed === true, retained_temp_config_count === 0, and process_observer_skipped === false",
                "direct_and_indirect_control_failure_count === 6 and every control exit != 0 with error_code === CREDENTIAL_CANARY_EXPOSURE_DETECTED",
                "live_credential_values_loaded === 0, live_credential_values_printed === 0, and network_mutation_performed === false"
              ],
              "must_not_observe": [
                "a canary value in argv, environment, stdout, stderr, JSON receipt, evidence, or retained files",
    ...
    },
    {
      "id": "TC-14",
      "type": "test_criterion",
      "description": "A unique in-memory synthetic canary traverses only actual verifier stdin and its real mode-0600 curl config while actual process, filesystem, public-HTTP, output, evidence, retained-file, and cleanup observations prove zero other exposure; real direct and indirect retrieval subprocess controls fail closed.",
      "maps_to_ac": "AC-11",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts --testNamePattern='credential canary transport safety'"
    }
  ]
