...
PUBLIC AUTH AND SECRET HYGIENE: only `HOLO_KEY_RN` may authorize the public Hono chat POST/GET routes; `HOLO_KEY_MCP` is forbidden for every public request. Secret values are forbidden in process argv, stdout, stderr, JSON receipts, and retained artifacts. CREDENTIAL EXPANSION BAN: an expanded secret value must never enter curl short options `-H`, `-d`, or `-u` (separated, clustered, attached, or equals forms), any equivalent long option or positional argv channel, or an inline environment assignment preceding curl or another process. Names-only presence checks are allowed only when they neither expand a secret value into process argv nor print it. The caller must carry auth over private SSH stdin into a private mode-0600 temporary curl config, render every URL/header value with curl-config quoting and escaping so spaces and punctuation survive parsing, and remove the temporary config with a trap on every exit. In the deployed container, the telemetry/trace CLI is `/app/src/cli/holo.ts`; its command must bootstrap `DATABASE_URL` from `/run/secrets/database_url` inside the container without exposing the value. Once the public POST has been attempted, every subsequent error receipt must truthfully retain `chat_request_issued:true`; only failures before the POST may report `false`.
...
- **AND** source and command review rejects secret expansion through curl `-H`, `-d`, `-u` in separated/clustered/attached/equals spellings, equivalent long/positional argv channels, and inline environment assignment; names-only presence checks and private file-to-stdin/config transport remain valid
...
- Reject any expanded credential value in curl `-H`, `-d`, `-u` (including clustered, attached, and equals spellings), long-form header/data/user options, positional argv, inline environment assignments, or equivalent process argv. A names-only presence test and a private file redirected to stdin or named as `curl --config` are not secret-value exposure.
...
- SECRET-EXPANSION SCANNER: fail closed on secret-bearing curl `-H`, `-d`, or `-u` in separated/clustered/attached/equals forms, equivalent long or positional argv, and inline secret assignments before a process. Do not reject names-only presence checks or private non-secret stdin/config path arguments.
  ...
  "fixtures": {
    ...
    "post_chat_invalid_stream_control": {
      ...
      "records": [
        ...
        "source review rejects expanded secret values in curl -H, -d, or -u separated/clustered/attached/equals forms, equivalent long/positional argv channels, and inline environment assignments while allowing names-only presence checks and private non-secret stdin/config path arguments"
      ]
    },
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "Statically reject secret expansion through curl -H, -d, or -u separated/clustered/attached/equals forms, equivalent long or positional argv channels, and inline environment assignment while retaining legitimate names-only checks and private stdin/config path transport.",
            ...
            "end_state": {
              "must_observe": [
                ...
                "exactly 4 argv bypass classes (short option, long option, positional argument, inline assignment) are rejected while exactly 2 safe controls (names-only presence check, private stdin/config path) pass",
                "network_mutation_performed === false and literal_disconnect_claimed === false"
              ],
              "must_not_observe": [
