...
- **AND** success requires `fleetRequests >= 1`, `cloudRequests === 0`, `unknownRequests === 0`, and `modelRequests === fleetRequests + cloudRequests + unknownRequests`
...
- **AND** it requires exactly one appended completion across both nodes plus serving-header/telemetry/node agreement
- **AND** a missing node receipt, inode change, truncation, unreadable range, zero or multiple completions, missing header, or header/telemetry disagreement fails `AMBIGUOUS_MINI_CORRELATION`
- **Verify:** `PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts`
...
- **GIVEN** an orchestrator-authorized implementation base
- **WHEN** source-predeploy and final-lineage receipts are verified
...
- **Verify:** `set -o pipefail; : "${S33_LINEAGE_RECEIPT:?}"; bash scripts/verify-s33-mini-served-turn.sh --mode final-lineage --receipt "$S33_LINEAGE_RECEIPT" --json`
...
| TC-10 | No-mini mode performs real bounded SSH/read attempts against the intentionally invalid path on both healthy minis and returns two non-synthetic receipts. | AC-7 | `set -o pipefail; : "${S33_EXPECTED_MAIN_SHA:?}"; : "${S33_RELEASE_LOCK:?}"; receipt="$(mktemp)"; if PLATFORM_IT=1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode no-mini-evidence --expected-main-sha "$S33_EXPECTED_MAIN_SHA" --release-lock "$S33_RELEASE_LOCK" --json >"$receipt"; then rm -f "$receipt"; exit 1; fi; jq -e '.ok == false and .error_code == "MINI_EVIDENCE_UNAVAILABLE" and .chat_request_issued == false and ([.attempts[].node] | sort) == ["inference1","inference2"] and ([.attempts[] | select(.actual_ssh_attempted == true and .actual_read_attempted == true and .synthetic == false and .canonical_precheck_exit == 0 and .read_path == "/dev/null/omlx-mini-8003.log" and .read_exit != 0 and .receipt_source == "ssh" and (.started_at | type == "string" and length > 0) and (.finished_at | type == "string" and length > 0) and (.started_epoch_ms | type == "number") and (.finished_epoch_ms | type == "number") and .finished_epoch_ms > .started_epoch_ms and (.finished_epoch_ms - .started_epoch_ms) <= 15000 and (.stdout_sha256 | test("^[0-9a-f]{64}$")) and (.stderr_sha256 | test("^[0-9a-f]{64}$")) and .bounded_ssh_options == ["BatchMode=yes","ConnectTimeout=10","ServerAliveCountMax=2","ServerAliveInterval=5"])] | length) == 2 and .network_mutation_performed == false and .literal_disconnect_claimed == false and .remote_host == "holocron@holocron"' "$receipt"; status=$?; rm -f "$receipt"; exit $status` |
| TC-11 | Positive correlation explicitly denies nonce binding, while missing/discontinuous/multiple/disagreeing temporary receipt mutations all fail AMBIGUOUS_MINI_CORRELATION. | AC-8 | `PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts` |
| TC-12 | Static Git/source verification plus temporary lineage-receipt mutations prove test-only RED, exact ancestry, authorized platform typecheck comparison, source-review-before-land, same-identity deploy/proof, and final-review-after-proof. | AC-9 | `set -o pipefail; : "${S33_LINEAGE_RECEIPT:?}"; bash scripts/verify-s33-mini-served-turn.sh --mode final-lineage --receipt "$S33_LINEAGE_RECEIPT" --json` |
...
<!--
{
  ...
  "requirements": [
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN the real public chat-runs agent.stream path WHEN a real Hono/Postgres/fleet turn runs THEN request/run-scoped accounting is attached before streaming, observes at least one request, terminalizes, reconciles fleet/cloud/unknown, and fails on missing, bypassed, cloud/OpenAI, unknown, or inconsistent accounting.",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts",
      "scenario": {
        "id": "S33-PLAT-05/AC-5",
        "tier": "visible",
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "Assert request/run binding, attached and complete status, modelRequests at least one, total reconciliation, fleetRequests at least one, cloudRequests zero, and unknownRequests zero.",
                "On regular temporary source and receipt copies, remove the public-boundary wrapper, insert direct api.openai.com and unknown requests, and corrupt totals; run the same oracle for each."
              ]
            },
            "end_state": {
              "must_observe": [
                "modelRequests >= 1 and modelRequests === fleetRequests + cloudRequests + unknownRequests",
                "fleetRequests >= 1, cloudRequests === 0, and unknownRequests === 0",
                "exactly 4 real-filesystem bypass, cloud, unknown, and counter mutations exit nonzero"
              ],
              "must_not_observe": [
                "missing instrumentation reported as cloudRequests === 0 success",
                "a global fetch monkey patch treated as request-scoped proof",
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN healthy real minis WHEN no-mini-evidence mode runs THEN canonical reads succeed, real bounded SSH/read attempts against the invalid path execute on both nodes, the verifier returns nonzero with two non-synthetic timestamp/hash receipts, and no chat, remote, service, or network mutation occurs.",
      "verify": "set -o pipefail; : \"${S33_EXPECTED_MAIN_SHA:?}\"; : \"${S33_RELEASE_LOCK:?}\"; receipt=\"$(mktemp)\"; if PLATFORM_IT=1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode no-mini-evidence --expected-main-sha \"$S33_EXPECTED_MAIN_SHA\" --release-lock \"$S33_RELEASE_LOCK\" --json >\"$receipt\"; then rm -f \"$receipt\"; exit 1; fi; jq -e '.ok == false and .error_code == \"MINI_EVIDENCE_UNAVAILABLE\" and .chat_request_issued == false and ([.attempts[].node] | sort) == [\"inference1\",\"inference2\"] and ([.attempts[] | select(.actual_ssh_attempted == true and .actual_read_attempted == true and .synthetic == false and .canonical_precheck_exit == 0 and .read_path == \"/dev/null/omlx-mini-8003.log\" and .read_exit != 0 and .receipt_source == \"ssh\" and (.started_at | type == \"string\" and length > 0) and (.finished_at | type == \"string\" and length > 0) and (.started_epoch_ms | type == \"number\") and (.finished_epoch_ms | type == \"number\") and .finished_epoch_ms > .started_epoch_ms and (.finished_epoch_ms - .started_epoch_ms) <= 15000 and (.stdout_sha256 | test(\"^[0-9a-f]{64}$\")) and (.stderr_sha256 | test(\"^[0-9a-f]{64}$\")) and .bounded_ssh_options == [\"BatchMode=yes\",\"ConnectTimeout=10\",\"ServerAliveCountMax=2\",\"ServerAliveInterval=5\"])] | length) == 2 and .network_mutation_performed == false and .literal_disconnect_claimed == false and .remote_host == \"holocron@holocron\"' \"$receipt\"; status=$?; rm -f \"$receipt\"; exit $status",
      "scenario": {
        "id": "S33-PLAT-05/AC-7",
        "tier": "visible",
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                "Perform bounded canonical-log readability prechecks on both nodes.",
                "Perform bounded real reads of /dev/null/omlx-mini-8003.log over SSH on both nodes.",
                "Hash and emit each attempt stdout/stderr with timestamps and exit codes."
              ]
            },
            "end_state": {
              "must_observe": [
                "underlying verifier exit code !== 0",
                "actual SSH/read receipt count === 2, each canonical precheck exit === 0, and each invalid-read exit !== 0",
                "network_mutation_performed === false and literal_disconnect_claimed === false"
              ],
              "must_not_observe": [
                "a skipped, synthetic, replayed, or canned node row",
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN the mini log has no nonce WHEN request/run telemetry, bounded append windows, and serving header are correlated THEN the receipt explicitly denies nonce binding, requires exactly one appended completion and node agreement, and fails closed on every ambiguity.",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts",
      "scenario": {
        "id": "S33-PLAT-05/AC-8",
        "tier": "visible",
        ...
        "topology": "multi-node",
        "negative_control": {
          "would_fail_if": ["false_nonce_binding", "missing_node", "inode_change", "truncation", "ambiguous_cardinality", "header_telemetry_disagreement", "static"]
        },
        "evidence": {
          "artifact_type": "stdout",
          "required_capture": true
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "Require exactly one appended completion and serving-header/request-telemetry agreement.",
                "Set method bounded_append_window_header_and_run_telemetry, nonceLogBinding=false, and an explicit not-nonce-binding claim.",
                "Mutate regular temporary receipt copies for every ambiguity input and rerun the oracle."
              ]
            },
            "end_state": {
              "must_observe": [
                "appended completion count across both nodes === 1",
                "`servingHeaderNode === telemetryNode === matchingNode`",
                "nonceLogBinding === false and claim explicitly says not nonce binding",
                "exactly 7 ambiguity mutations fail with `AMBIGUOUS_MINI_CORRELATION`"
              ],
              "must_not_observe": [
                ...
                "success with zero or multiple appended completions",
    ...
    },
    {
      ...
      "type": "acceptance_criterion",
      "primary": false,
      "description": "GIVEN an orchestrator-authorized base WHEN source-predeploy and final-lineage receipts are checked THEN a real-path test-only RED and descendant GREEN precede authorized platform typecheck comparison and mandatory product-manager plus mastra-reviewer source review, then orchestrator landing, same-main package/deploy/proof, and mandatory final product-manager plus mastra-reviewer review.",
      "verify": "set -o pipefail; : \"${S33_LINEAGE_RECEIPT:?}\"; bash scripts/verify-s33-mini-served-turn.sh --mode final-lineage --receipt \"$S33_LINEAGE_RECEIPT\" --json",
      "scenario": {
        "id": "S33-PLAT-05/AC-9",
        "tier": "visible",
        ...
        "cases": [
          {
            ...
            "action": {
              "actor": "integrator",
              "steps": [
                "Prove RED parent and sole diff path plus real-path expected failure, then GREEN ancestry.",
                ...
                "Require mandatory product-manager and mastra-reviewer source approvals before orchestrator landing.",
                "Require package/deploy/proof to share the landed main SHA, image digest, and Compose hash.",
                "Require mandatory product-manager and mastra-reviewer final approvals after proof; treat test-quality-reviewer as additional only.",
                "Mutate regular temporary lineage copies for every order and identity invariant."
              ]
            },
            "end_state": {
              "must_observe": [
                "`red.parentSha === authorizedBaseSha`, `red.diffPaths === [services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts]`, and `green.descendsFromRed === true`",
                ...
                "`package.sourceRevision === deploy.sourceRevision === proof.expectedMainSha === landedMainSha` and digest/Compose values are equal",
                "final reviewer list === `[product-manager,mastra-reviewer]` and both review timestamps are greater than `proof.finishedAt`"
              ],
              "must_not_observe": [
    ...
    },
    {
      "id": "TC-10",
      "type": "test_criterion",
      "description": "No-mini mode performs two real bounded invalid-path SSH reads and emits two non-synthetic receipts while returning nonzero.",
      "maps_to_ac": "AC-7",
      "verify": "set -o pipefail; : \"${S33_EXPECTED_MAIN_SHA:?}\"; : \"${S33_RELEASE_LOCK:?}\"; receipt=\"$(mktemp)\"; if PLATFORM_IT=1 S33_HOLOCRON_HOST=holocron@holocron bash scripts/verify-s33-mini-served-turn.sh --mode no-mini-evidence --expected-main-sha \"$S33_EXPECTED_MAIN_SHA\" --release-lock \"$S33_RELEASE_LOCK\" --json >\"$receipt\"; then rm -f \"$receipt\"; exit 1; fi; jq -e '.ok == false and .error_code == \"MINI_EVIDENCE_UNAVAILABLE\" and .chat_request_issued == false and ([.attempts[].node] | sort) == [\"inference1\",\"inference2\"] and ([.attempts[] | select(.actual_ssh_attempted == true and .actual_read_attempted == true and .synthetic == false and .canonical_precheck_exit == 0 and .read_path == \"/dev/null/omlx-mini-8003.log\" and .read_exit != 0 and .receipt_source == \"ssh\" and (.started_at | type == \"string\" and length > 0) and (.finished_at | type == \"string\" and length > 0) and (.started_epoch_ms | type == \"number\") and (.finished_epoch_ms | type == \"number\") and .finished_epoch_ms > .started_epoch_ms and (.finished_epoch_ms - .started_epoch_ms) <= 15000 and (.stdout_sha256 | test(\"^[0-9a-f]{64}$\")) and (.stderr_sha256 | test(\"^[0-9a-f]{64}$\")) and .bounded_ssh_options == [\"BatchMode=yes\",\"ConnectTimeout=10\",\"ServerAliveCountMax=2\",\"ServerAliveInterval=5\"])] | length) == 2 and .network_mutation_performed == false and .literal_disconnect_claimed == false and .remote_host == \"holocron@holocron\"' \"$receipt\"; status=$?; rm -f \"$receipt\"; exit $status"
    },
    {
      "id": "TC-11",
      "type": "test_criterion",
      "description": "Positive correlation denies nonce binding and real-filesystem ambiguity mutations all fail AMBIGUOUS_MINI_CORRELATION.",
      "maps_to_ac": "AC-8",
      "verify": "PLATFORM_IT=1 pnpm test:integration services/platform/tests/integration/s33-plat-05-mini-served-turn.test.ts"
    },
    {
