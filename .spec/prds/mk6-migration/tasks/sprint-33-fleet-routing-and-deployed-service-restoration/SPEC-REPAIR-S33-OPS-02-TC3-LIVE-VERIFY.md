...
**Success state:** the human-readable and embedded S33-OPS-02 TC-3 verifiers decode to the same `health-flip` command; the old local-path probe is absent; remote Docker uses exact `/usr/local/bin/docker`; the scenario requires pre-stop `ok/true/null`, post-stop `degraded/false/fleet`, restored `ok/true/null`, unchanged Mastra identity and PID, monotonic uptime, unchanged production-service and protected-primary sentinels, and a final running/healthy router; the exact `fail-after-stop` negative control exits nonzero only after independently provable cleanup, validates all five summary predicates, independently reasserts the exact health triplet from the three raw bodies, rejects nonnumeric or non-finite uptime values before ordering all three, and recomputes the remaining invariants from fixed raw artifacts using explicitly bounded HTTP/SSH; normal and control evidence paths are exact pre-mutation allowlist entries; only the existing verifier, focused integration test, and evidence paths are writable.
...
- Require both the exact negative-control gate and focused integration test to derive `ok/true/null`, `degraded/false/fleet`, and `ok/true/null` directly from `health.pre.json`, `health.degraded.json`, and `health.restored.json`, and to establish that all three `deployment.identity.uptimeMs` values are finite JSON numbers before monotonic comparison.
      ...
      "scenario": {
        ...
        "negative_control": {
          "would_fail_if": [
            ...
            "the cleanup, timeout, exact evidence-path allowlist, absolute Docker, intentional-failure, raw health-triplet, all-three finite-numeric uptime, raw-artifact recomputation, identity, or final-router oracle is omitted"
          ]
        },
        "evidence": {
        ...
        "cases": [
          {
            ...
            "action": {
              ...
              "steps": [
                ...
                "align the S33-OPS-02 scenario, exact timeout and evidence-path allowlist, absolute remote Docker requirement, fail-after-stop interface, raw health-triplet and all-three finite-numeric uptime predicates, raw pre/post evidence schema, verifier/test recomputation contract, and WRITE-ALLOWED list",
            ...
            "end_state": {
              "must_observe": [
                ...
                "the exact fail-after-stop gate has jq -s -e recomputation count==3, raw health triplet==ok/true/null,degraded/false/fleet,ok/true/null, finite_numeric_uptime_count==3 before ordering, failure.json has all five invariant predicates==true, and the test reads exactly 7 named raw invariant artifacts before independently observing the restored router",
                "canonical scenario validation returns ok==true and scenario_count==1"
              ],
              "must_not_observe": [
                "the old docker compose down plus grep command remains",
                "an empty cleanup, timeout, evidence-allowlist, absolute-Docker, intentional-failure, raw-health-triplet, all-three finite-numeric uptime, raw-recomputation, identity, or final-router assertion set",
