...
| scope compliance | `actual=$(git diff --name-only HEAD^..HEAD | LC_ALL=C sort); expected=$(printf '%s\n' '.spec/prds/mk6-migration/tasks/sprint-33-fleet-routing-and-deployed-service-restoration/S33-OPS-02-package-the-litellm-router-as-a-docker-compose-service-and-d.md' '.spec/prds/mk6-migration/tasks/sprint-33-fleet-routing-and-deployed-service-restoration/SPEC-REPAIR-S33-OPS-02-EVIDENCE-RUNS.md' | LC_ALL=C sort); test "$actual" = "$expected"` | Exit 0 only when the committed diff contains exactly the two WRITE-ALLOWED planning files |
