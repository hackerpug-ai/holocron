...
| 16 | — | [Sprint 16: Public /article/ Endpoint on Hono](#sprint-16-public-article-endpoint-on-hono) | `/article/{token}` returns byte-comparable HTML; non-public token → 404 | 4 | 4, 14 | ✅ Completed | `mk6-public-article` | — |
| 17 | — | [Sprint 17: Deterministic pi-free Research Engine](#sprint-17-deterministic-pi-free-research-engine) | Research does NOT terminate on thin evidence; only on the pure-TS evidence gate | 4 | 7, 8, 9, 10, 15 | ✅ Completed | `mk6-research-engine` | — |
| 18 | — | [Sprint 18: Chat Redesign — Native Tool Loop and Resumable SSE](#sprint-18-chat-redesign--native-tool-loop-and-resumable-sse) | `POST /api/chat-runs` streams sequenced tokens from a fleet specialist; replay idempotent | 5 | 4, 5, 8 | ✅ Completed | `mk6-chat` | — |
| 19 | — | [Sprint 19: MCP Gateway Rehost — 44 Tools on Postgres](#sprint-19-mcp-gateway-rehost--44-tools-on-postgres) | All 44 MCP tools over stdio on seeded Postgres match manifest; zero Convex calls | 8 | 3, 4, 5 | ✅ Completed | `mk6-mcp-rehost` | — |
| 20 | — | [Sprint 20: E2E Maestro Harness and Cold-Boot Reference Flow](#sprint-20-e2e-maestro-harness-and-cold-boot-reference-flow) | Cold-boot chat round-trips fleet→Postgres→Zero-sync green on real Maestro harness | 10 | 4, 6, 13, 18 | 🟠 In flight | `mk6-reference-flow` | — |
| 21 | — | [Sprint 21: Client Data Contract](#sprint-21-client-data-contract) | `holo verify:client-contract` — all 105 call sites mapped to a live target | 3 | 4, 5, 18 | 🟠 In Progress | `mk6-client-contract` | — |
| 22 | — | [Sprint 22: All Agentic Pipelines as Templates/Agents](#sprint-22-all-agentic-pipelines-as-templatesagents) | Each pipeline produces former output from a shared template; no per-domain shells | 5 | 8, 9, 10, 12, 15, 17 | ✅ Completed | `mk6-pipelines` | — |
| 23 | — | [Sprint 23: Deterministic Human Gate, Steering and Fulcrum Seams](#sprint-23-deterministic-human-gate-steering-and-fulcrum-seams) | Verdicts deterministically reject uncited kill / 2nd WIP / unprobed advance | 5 | 7, 8, 15, 17 | ✅ Completed | `mk6-human-gate` | — |
| 24 | — | [Sprint 24: Full RN App Rewrite off Convex onto Zero](#sprint-24-full-rn-app-rewrite-off-convex-onto-zero) | Rewritten app shows 3 seeded conversations via Zero; no Convex hook on the path | 6 | 4, 16, 20, 21 | 🟠 In flight | `mk6-app-rewrite` | — |
| 25 | — | [Sprint 25: Reactive Surfaces — SSE Streaming, Mission Progress, Degraded](#sprint-25-reactive-surfaces--sse-streaming-mission-progress-degraded) | Disconnect mid-stream → reconciles to exactly one final message, no dup tokens | 5 | 8, 15, 18, 24 | ✅ Completed | `mk6-reactive-surfaces` | — |
...
| 33 | — | [Sprint 33: Fleet Routing and Deployed-Service Restoration](#sprint-33-fleet-routing-and-deployed-service-restoration) | With the laptop off the tailnet, `/health` is 200 with fleet ready, a document reads from Postgres not 410, and a chat turn is generated on a Mac mini | 13 | 6, 8, 29 | 🟠 In flight | `mk6-fleet-routing` | — |
...
**Gate:** An Agent Client invoking all 44 MCP tools over stdio and Streamable HTTP against seeded Postgres receives manifest-bound results with zero Convex calls; replay, auth/origin, cancellation, and no-sampling are proven.
...
**Progress note (2026-07-18):** The fail-closed Maestro harness, real zero-cache bootstrap, deterministic `holo namespace reset`, Expo Zero schema/provider seam, stable chat selectors, and a Hono/Zero reference-chat route are implemented and committed (`4d56349`, `9109e32`, `23e425b`, `ad63aad`, `6b3ee1a`, `610846b`). The boundary proof passes against real Postgres, the Expo iOS bundle exports, and the reset seeds the deterministic conversation. The cold-boot flow is not yet green: a real Expo development build on the named simulator, real fleet completion, and end-to-end Zero sync artifacts remain unproven; Sprint 20 stays In Progress.
...
Generated by /kb-sprint-tasks-plan on 2026-07-18T21:44:20Z (specialist proposals: red-test-generator, react-native-ui-planner, devops-engineer, ghactions-planner). Avg quality target ≥80/115; fakeability audit: 0 CRITICAL/HIGH after consolidator hardening — `validate_scenario.py` green for all 10 tasks.
...
Generated by /kb-sprint-tasks-plan on 2026-07-21T05:02:44Z (specialist proposal: mastra-planner). Avg quality ~100/115; fakeability audit: 0 CRITICAL / 0 HIGH — `validate_scenario.py` exit 0 on every task.
...
Generated by /kb-sprint-tasks-plan on 2026-07-21T20:13:41Z.
...
Generated by /kb-sprint-tasks-plan on 2026-07-23T02:16:55Z (specialist proposal: react-native-ui-planner). Avg quality ≥80/115; fakeability audit **0 CRITICAL / 0 HIGH** — `validate_scenario.py` exit 0 on every task. Topological order: S-REWRITE-01 ∥ 02 ∥ 03 ∥ 04 (the four screen clusters) → S-REWRITE-05 (no-convex gate) → S-REWRITE-06 (review).
...
Generated by /kb-sprint-tasks-plan on 2026-07-24T19:52:54Z (specialist proposal: react-native-ui-planner; backend-contract enrichments: mastra-planner). Avg quality ≥80/115; fakeability audit 0 CRITICAL / 0 HIGH — `validate_scenario.py` exit 0 on every behavioral AC of all 5 tasks. Topological order: S-REACTIVE-01 ∥ 02 ∥ 04 → S-REACTIVE-03 → S-REACTIVE-05.
...
**Sequence:** 26
**Timeline:** Phase 5 — Client Rewrite
**Status:** Deferred (user-directed; runtime gate incomplete)
**Proposed by:** react-native-ui-planner
**Milestone:** — (`sprint-26`)
**Branch:** `mk6-uploads`
**PR:** —
...
Generated by /kb-sprint-tasks-plan on 2026-07-24T20:52:41Z
...
> Standing platform capability (CAP-BAK-01) that runs in parallel with feature work and **outlasts** the migration — it is not a cutover-only safety net. It gates the final decommission (Sprint 32), not the feature build.
...
- Blocks: Sprint 28, Sprint 31, Sprint 32
...
Generated by /kb-sprint-tasks-plan on 2026-07-28T01:16:22Z — SPRINT.md + 6 task files at [`tasks/sprint-28-point-in-time-restore-and-fresh-hardware-fire-drill/`](tasks/sprint-28-point-in-time-restore-and-fresh-hardware-fire-drill/); avg 109/115, **0 CRITICAL** fakeability (`validate_scenario.py` clean on every behavioral AC).
...
Generated by `/kb-sprint-tasks-plan 30` on 2026-08-02T06:20:00Z — avg quality **115/115**, fakeability audit clean on **all 33 behavioral ACs**.

- `D07-01-red-rollback-recovers-zero-loss-ponr-write-closes-rollback-path.md` (`mastra-planner`)
- `D07-02-keep-convex-live-pin-the-convex-pointing-fallback-app-build-through-soak.md` (`devops-engineer`)
- `D07-03-run-the-rollback-drill-sev-1-trigger-config-re-point-zero-loss-verification.md` (`devops-engineer`)
- `D07-04-record-the-data-plane-point-of-no-return-first-accepted-postgres-write.md` (`mastra-planner`)
- `D07-05-security-review-rollback-config-switch-ponr-immutability.md` (`security-auditor`)
...
**Proposed by:** mastra-planner + devops-engineer + react-native-ui-planner + mcp-planner + convex-planner + frontend-designer
**Milestone:** — (`sprint-31`)
**Branch:** `mk6-integrity-remediation`
**PR:** —

#### Closure record — 2026-08-09

Sprint 31 is administratively closed with the operator-approved exceptions below. All implementation task tips are landed on `main`; the original acceptance criteria remain authoritative and are not rewritten as green where their live proof was deferred or failed closed.

- **S31-CX-02:** live export fidelity comparison is deferred; Convex remains retained. A read-only export is archived on `/Volumes/Archives` for later comparison.
- **S31-OPS-01:** old R2-key revocation and the old-key negative control are deferred by operator approval; new-key validation remains follow-up work.
- **S31-OPS-02:** the production fixture purge is complete (5 rows deleted after dump; 0 fixture-like rows remain). Healthy-chain verification remains an explicit exception because 6 retained real jobs are overdue and `ALERT_WEBHOOK_URL` is unset.

The sprint evidence remains `met:false` until those exceptions are resolved or the sprint contract is formally amended. Sprint 32's Convex-decommission gate remains blocked by this documented state.

> **Why this sprint exists.** A 29-sprint adversarial audit found that several capabilities the roadmap marked ✅ Completed are not implemented, and that five verifier commands are structurally incapable of failing — which is the mechanism by which those claims shipped as green. Sprint 32 deletes `convex/` and the Convex cloud deployment irreversibly; any capability still implemented only in Convex becomes unrecoverable at that moment. This sprint makes the migration's own claims true before that door closes. Scope was set by the dispatched specialist SET against the PRD's locked scope: 15 findings were rejected as out-of-scope (recorded in [`01-scope.md`](./01-scope.md)) and 29 recorded as risks (recorded in [`10-technical-requirements/08-technical-risks.md`](./10-technical-requirements/08-technical-risks.md)) rather than becoming tasks.
...
- Blocks: Sprint 32 (no capability may be deleted before it has a working non-Convex replacement)
- Dependent on: Sprint 30
...
- Dependent on: Sprint 28, Sprint 30, Sprint 31 (deletion is hard-blocked on the decommission-readiness inventory reporting zero sole-implementation capabilities)
...
- [`D08-01-red-grep-clean-build-without-convex-deps-is-the-acceptance-oracle.md`](tasks/sprint-32-convex-decommission-code-deps-and-cloud-deletion/D08-01-red-grep-clean-build-without-convex-deps-is-the-acceptance-oracle.md)
- [`D08-02-remove-convex-code-deps-delete-dead-clients-archive-ratatui-playground.md`](tasks/sprint-32-convex-decommission-code-deps-and-cloud-deletion/D08-02-remove-convex-code-deps-delete-dead-clients-archive-ratatui-playground.md)
- [`D08-03-re-run-the-fresh-hardware-fire-drill-restore-as-the-final-pre-deletion-gate.md`](tasks/sprint-32-convex-decommission-code-deps-and-cloud-deletion/D08-03-re-run-the-fresh-hardware-fire-drill-restore-as-the-final-pre-deletion-gate.md)
- [`D08-04-author-the-decommission-runbook-ordered-gated-checklist.md`](tasks/sprint-32-convex-decommission-code-deps-and-cloud-deletion/D08-04-author-the-decommission-runbook-ordered-gated-checklist.md)
- [`D08-05-delete-the-convex-cloud-deployment-operator-executed-irreversible.md`](tasks/sprint-32-convex-decommission-code-deps-and-cloud-deletion/D08-05-delete-the-convex-cloud-deployment-operator-executed-irreversible.md)
...
- **Test-step normalization.** Sprint 32's decommission gate/step 1 is rendered as the operator wrapper command (`holo verify:no-convex`) the sprint's own tasks build, rather than a raw `grep` invocation, matching the wrapper convention used across the roadmap and the TEST-STEPS field guide.
- **Sprint 31 insertion (2026-08-07).** Sprint 31 (Migration Integrity Remediation) was inserted between the rollback drill and the decommission, and the former Sprint 31 (Convex Decommission) renumbered to Sprint 32. Its content was authored by the full dispatched specialist SET — `mastra-planner`, `devops-engineer`, `react-native-ui-planner`, `mcp-planner`, `convex-planner`, `frontend-designer` — reviewing every audit finding against the PRD's locked scope. `frontend-designer` returned no design surface (the failure states reuse the existing degraded-banner pattern verbatim), which is recorded rather than padded into tasks. Findings judged to be drift were written to [`01-scope.md`](./01-scope.md) as Out of Scope or to [`10-technical-requirements/08-technical-risks.md`](./10-technical-requirements/08-technical-risks.md) as risks, never converted into sprint work.

## Sprint 33: Fleet Routing and Deployed-Service Restoration

**Status:** In Progress

**Gate:** With the laptop off the tailnet, the deployed holocron service reports healthy, serves a real
document out of Postgres instead of a 410, and answers a chat turn with tokens generated on a Mac mini.

**Why this sprint exists.** The MK-VI platform is deployed on `holocron` and answering, but `/health` is
**503 degraded**. Three deploy-time faults, none of which shipping `main` would fix: the container's fleet
endpoint is `host.docker.internal:4545` (nothing serves 4545 on that Mac); the runtime still declares
`data_plane: convex` while `convex/` was deleted in an ancestor of the running image, so document reads
hard-fail 410; and the scheduler container receives no `FLEET_URL`/`FLEET_KEY` at all.

Specialist expansion additionally surfaced three verifiers that cannot fail — the same class of defect
Sprint 31 was created to remediate:

| Where | What it lets pass |
|---|---|
| `inference/resolve-model.ts` never reads `FLEET_URL` | green `/health` over a dead inference path |
| `probeFleet` = `res.ok` on `/v1/models` | an empty router reads as healthy |
| `compat/cells/agent.ts:80` loopback-only counter | "zero cloud calls" passes when zero calls happened |

**Scope (human-directed):** a fleet that does not depend on the operator's laptop — a LiteLLM router
packaged onto `holocron`, role models resident on both Mac minis, capacity routing across them.

**Proposed by:** dispatched specialist SET — `devops-engineer` (S33-OPS-*), `mastra-planner` (S33-PLAT-*),
`mcp-planner` (S33-MCP-*). Consolidated, sequenced, audited and rendered by the orchestrator.

**Tasks (13)** — see [`tasks/sprint-33-fleet-routing-and-deployed-service-restoration/SPRINT.md`](tasks/sprint-33-fleet-routing-and-deployed-service-restoration/SPRINT.md).

**Blocks:** `S33-PLAT-04` and `S33-MCP-03` depend on **MK6-DATA-001** (in-flight sprint
`imp-mk6-functional-completeness-1786837297`), which is BLOCKED on the retained Convex export. The other
eleven tasks are unblocked. Overlaps `MK6-FLEET-001` on routing — Sprint 33 owns provisioning, that task
owns in-service config; reconcile before both land.

**Known gap:** `qwen3-reranker` exists nowhere on the fleet, so the manifest's fail-closed `rerank` role
cannot resolve. Recorded, not silently downgraded.
