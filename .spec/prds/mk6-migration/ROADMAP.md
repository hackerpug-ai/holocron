...
**Progress note (2026-07-18):** The fail-closed Maestro harness, real zero-cache bootstrap, deterministic `holo namespace reset`, Expo Zero schema/provider seam, and stable chat selectors are implemented and committed (`4d56349`, `9109e32`, `35bcbd5`). The cold-boot flow is not yet green: the remaining blocker is migrating the thin chat send/read path off Convex and proving it on the named simulator with a real Expo development build.
