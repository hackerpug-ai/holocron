...
| 8 | — | [Sprint 08: Role Router, Local-First and Degraded Modes](#sprint-08-role-router-local-first-and-degraded-modes) | Normal mission routes every call to the fleet; zero Anthropic on default path | 5 | 1, 5 | 🟠 In Progress | `mk6-inference-router` | — |
...
| 11 | — | [Sprint 11: Scheduler and Durable Queue](#sprint-11-scheduler-and-durable-queue) | kill-9 at each boundary → exactly one observable effect + dedupe record | 5 | 1, 4, 5 | 🔵 Planned | `mk6-scheduler-queue` | — |
...
| 15 | — | [Sprint 15: Mission Engine — Durable Resumable Templates](#sprint-15-mission-engine--durable-resumable-templates) | `holo mission run` produces typed output; idempotency replay returns stored result | 5 | 4, 5, 11 | 🔵 Planned | `mk6-mission-engine` | — |
...
**Test Steps:**
1. Run `holo etl:run --export ./export` against real Postgres — loads all tables in FK-dependency order, status normalized.
2. Run `holo etl:reconcile` — every table's expected-target formula matches; zero unexplained variance.
3. Run `holo etl:fk-audit` — all FK constraints enforce clean; the NULL-FK audit returns 0 orphans.
4. Run `holo etl:vectors` — every regenerated passage vector is 1024-dim and non-null (regenerated, never copied).
5. Migrate a narration MP3 + a file object and run `holo blob:verify` — SHA-256/byte-length/MIME match; Range read returns correct bytes.
6. Re-run `holo etl:run` from the immutable archive — no duplicate rows or blobs (convex_id_map stable).
7. Run `holo upload:init/put/finalize` for an image + a voice artifact — hash verified, idempotent attach, no orphan row/object.
...
| etl-6 | Verify ETL export completeness + review integrity | convex-reviewer | 150 min |

#### Dependencies

- Blocks: Sprint 16, Sprint 26, Sprint 29
- Dependent on: Sprint 02, Sprint 04, Sprint 07, Sprint 10

#### PRD Coverage

- UC-DATA-05
- T-DATA-016, T-DATA-017, T-DATA-018, T-DATA-019, T-DATA-021
...
**Gate:** Running `holo mission run <template> --goal '…'` from a declarative contract row against real Postgres produces the contract's typed output with template/compiler/executor/schema versions persisted, and replaying the same idempotency key returns that stored result rather than re-executing.

**Test Steps:**
1. Register a template and run `holo mission run <t> --goal 'X'` — commits a typed output row with template/compiler/executor/schema versions.
2. Submit an arbitrary executable stage definition — rejected before run creation (closed DSL only).
3. Run `holo mission run <t>` and SIGKILL mid-run — `holo mission resume <id>` restarts from the last committed step with the pinned executor.
4. Reference an unknown/incompatible stage version — rejected before start.
5. Kill-9 mid-COMMIT — no partial rows; replaying the idempotency key returns the stored result.
6. Run a mission that exceeds its token/wall-ms budget — records `budget_exceeded`, not a silent non-commit.

#### Tasks

| ID | Title | Agent | Estimate |
|----|-------|-------|----------|
| mission-1 | Mission template record + MissionTemplateSchema (Zod) + closed DSL executor/schema registry | mastra-implementer | 300 min |
| mission-2 | Durable workflow runtime (@mastra/pg): run-state, suspend snapshots, pinned-executor resume | mastra-implementer | 300 min |
| mission-3 | All-or-nothing cycle COMMIT + idempotency-key replay + per-run budget enforcement | mastra-implementer | 240 min |
| mission-4 | RED tests: typed output, reject-arbitrary-stage, SIGKILL-resume, kill-9-commit no-partial, budget | red-test-generator | 240 min |
| mission-5 | Review durability + DSL closedness | mastra-reviewer | 90 min |

#### Dependencies

- Blocks: Sprint 17, Sprint 22, Sprint 23, Sprint 25
- Dependent on: Sprint 04, Sprint 05, Sprint 11
