...
| C-2 | **Mastra Server (Hono)** | The one backend process. Hosts agents, tools, workflows, processors; exposes `/api/*`, SSE, the MCP mount, `/article/`, `/blobs/`, `/health` over Tailscale. | Service (on mini) |
...
| C-6 | **Workflow Runtime + State** (`@mastra/pg`) | Durable run state, suspend snapshots, resume-from-last-committed-step. Survives kill-9. Replaces both the Convex scheduler and `@convex-dev/workflow`. | Module |
| C-7 | **Scheduler + Durable Queue** | Mastra native `schedule` for cadence + **graphile-worker / pg-boss** (Postgres `SKIP LOCKED`, retries/backoff, DLQ, priorities). Replaces all 16 crons + `scheduler.runAfter` chaining. | Module + PG |
| C-8 | **Reactive Bridge** | Postgres `LISTEN/NOTIFY` + an outbox table → enqueues a queue job → mission run. Implements the *reactive* trigger class exactly-once (raw NOTIFY is lossy). | Module + PG |
...
| C-12 | **MCP Gateway** (`@mastra/mcp`) | The 44-tool server, rehosted to call in-process Mastra tools (no Convex proxy), stateless streamable-HTTP, no server→client sampling. | Module |
| C-13 | **Blob Store** | Content-addressed MP3/image storage on the mini FS behind a `BlobStore` interface (`put/get/stream/url/delete`); Hono serves `/blobs/:id` with Range support. MinIO is a one-adapter swap. | Module + FS |
| C-14 | **Processors / Guardrails** | Input/output/error processors; TokenLimiter, output validation, tripwire handling at every call site. | Module |
| C-15 | **Observability + Budget Ledger** | Langfuse (self-host) via OTel exporter; per-call inference telemetry (tokens/wall-ms/endpoint/role) to Postgres; the escape-hatch budget ledger; eval scorers (local judge). | Module + Service |
