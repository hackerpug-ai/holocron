...
| 2 | `pnpm-workspace.yaml` contains `packages/*` and `pnpm list -r --depth 0` prints `@holocron/web` | AC-3 | `rg -n 'packages/\*' pnpm-workspace.yaml && pnpm install && pnpm list -r --depth 0 --parseable \| rg '@holocron/web'` | [ ] TRUE [ ] FALSE |
