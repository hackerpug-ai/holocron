...
- [ ] AC-2: `PLATFORM_IT=1 MK6_DATA_NEGATIVE=count-equal-content-corrupt bash scripts/verify-mk6-data-plane-truth.sh --json` — A count-equal/content-corrupt isolated Postgres target exits non-zero with `CONTENT_DIGEST_MISMATCH`; cross-surface retired/wrong/disconnected variants belong to MK6-DATA-002.
...
<!--
{
  ...
  "fixtures": {
    "real_export": {
      ...
      "description": "retained immutable real Convex export with one known sentinel",
      "records": [
  ...
  "requirements": [
    {
      ...
      "primary": true,
      "description": "GIVEN a retained real export WHEN reconciliation runs THEN direct Postgres and external content probe return the same non-empty sentinel hash",
      "verify": "PLATFORM_IT=1 bash scripts/verify-mk6-data-plane-truth.sh --json",
      "maps_to_ac": null,
      "scenario": {
        "id": "data-foundation",
        ...
        "negative_control": {
          "would_fail_if": [
            "the sentinel is deleted or content is hardcoded"
          ]
        },
        "evidence": {
        ...
        "cases": [
          {
            "start_ref": "real_export",
            "action": {
              "steps": [
                "reconcile the retained export and fetch mk6-data-sentinel-1 through the external API"
              ]
            },
            "end_state": {
              "must_observe": [
    ...
    },
    {
      "id": "AC-2",
      "type": "acceptance_criterion",
      "description": "Count-equal content corruption fails with CONTENT_DIGEST_MISMATCH",
      "verify": "PLATFORM_IT=1 MK6_DATA_NEGATIVE=count-equal-content-corrupt bash scripts/verify-mk6-data-plane-truth.sh --json",
      "maps_to_ac": null,
      "scenario": {
        "id": "data-content-corrupt",
        "test_tier": "integration",
        "tier": "visible",
        "verification_service": "postgres-reconcile",
        "negative_control": {
          "would_fail_if": [
            "content digest validation is removed or corruption is ignored"
          ]
        },
        "evidence": {
          "artifact_type": "db_query",
        ...
        "cases": [
          {
            ...
            "action": {
              "steps": [
                "change sentinel bytes in isolated Postgres without changing row count"
              ]
            },
            "end_state": {
              "must_observe": [
    ...
    },
    {
      "id": "TC-1",
      "type": "test_criterion",
      "description": "The real export and Postgres sentinel bytes agree",
      "verify": "PLATFORM_IT=1 bash scripts/verify-mk6-data-plane-truth.sh --json",
      "maps_to_ac": "AC-1"
    },
    {
