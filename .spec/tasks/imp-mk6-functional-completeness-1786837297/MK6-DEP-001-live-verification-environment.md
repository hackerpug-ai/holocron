...
| TC-3 | The five-case matrix enumerates and fails Postgres, fleet, Mastra, scheduler, and Zero independently. | AC-2 | `PLATFORM_IT=1 MK6_NEGATIVE_DEPENDENCY=core-matrix bash scripts/verify-mk6-live-dependencies.sh --provision-isolated --json` |
...
<!--
{
  ...
  "fixtures": {
    "isolated_stack": {
      "seed_method": "cli",
      "description": "isolated real Postgres, Mastra, scheduler, Zero and fleet namespace",
      "records": [
        "mk6-dep-sentinel-1"
      ]
    }
  ...
  "requirements": [
    {
      ...
      "primary": true,
      "description": "GIVEN an isolated namespace WHEN live dependency provisioning runs THEN one real sentinel crosses Postgres, Mastra, scheduler, Zero and fleet",
      "verify": "PLATFORM_IT=1 bash scripts/verify-mk6-live-dependencies.sh --provision-isolated --json",
      "maps_to_ac": null,
      "scenario": {
        "id": "dep-stack",
        "test_tier": "integration",
        "tier": "visible",
        "verification_service": "mk6-live-stack",
        "negative_control": {
          "would_fail_if": [
            "Zero is disconnected or Postgres is replaced by an in-memory stub"
          ]
        },
        "evidence": {
        ...
        "cases": [
          {
            "start_ref": "isolated_stack",
            "action": {
              "steps": [
                "provision the isolated real services and submit sentinel mk6-dep-sentinel-1"
              ]
            },
            "end_state": {
              "must_observe": [
    ...
    },
    {
      "id": "AC-2",
      "type": "acceptance_criterion",
      "description": "Dependency absence is named and manual-only prerequisites remain blocked",
      "verify": "PLATFORM_IT=1 MK6_NEGATIVE_DEPENDENCY=core-matrix bash scripts/verify-mk6-live-dependencies.sh --provision-isolated --json",
      "maps_to_ac": null,
      "scenario": {
        "id": "dep-core-matrix",
        "test_tier": "integration",
        "tier": "visible",
        "verification_service": "mk6-dependency-preflight",
        "negative_control": {
          "would_fail_if": [
            "any of Postgres, fleet, Mastra, scheduler, or Zero is disconnected but readiness still succeeds"
          ]
        },
        "evidence": {
        ...
        "cases": [
          {
            ...
            "action": {
              "steps": [
                "enumerate and run isolated Postgres, fleet, Mastra, scheduler, and Zero unavailable cases"
              ]
            },
            "end_state": {
              "must_observe": [
                "enumeratedDependencyCount: 5",
                "namedDependencyFailureCount: 5",
                "readyCount: 0"
              ],
              "must_not_observe": [
                "readyCount: 1",
    ...
    },
    {
      "id": "TC-1",
      "type": "test_criterion",
      "description": "The real isolated stack emits non-empty identities and one sentinel",
      "verify": "PLATFORM_IT=1 bash scripts/verify-mk6-live-dependencies.sh --provision-isolated --json",
      "maps_to_ac": "AC-1"
    },
    {
      "id": "TC-2",
      "type": "test_criterion",
      "description": "The Zero-negative run exits non-zero and cannot report ready",
      "verify": "PLATFORM_IT=1 MK6_NEGATIVE_DEPENDENCY=zero bash scripts/verify-mk6-live-dependencies.sh --provision-isolated --json",
      "maps_to_ac": "AC-2"
    },
    {
      "id": "TC-3",
      "type": "test_criterion",
      "description": "Five dependency variants fail independently",
      "verify": "PLATFORM_IT=1 MK6_NEGATIVE_DEPENDENCY=core-matrix bash scripts/verify-mk6-live-dependencies.sh --provision-isolated --json",
      "maps_to_ac": "AC-2"
    }
  ]
